...
def verify_feature_order_file():
    ...
    if len(feature_list) != expected_count:
        logger.error(
            f"❌ Feature list length mismatch: expected {expected_count}, "
    ...
    logger.info(
        f"✅ feature_order.json verified: {actual_count} features including momentum_14"
    )
    return True
...
def verify_feature_generation(config: dict):
    ...
    try:
        ...
        if actual_features != expected_features:
            logger.warning(
                f"⚠️ Feature count: expected {expected_features}, got {actual_features}"
            )
        ...
        if ordered_features is not None:
            logger.info(
                f"✅ Feature ordering successful: {ordered_features.shape[1]} features"
            )
        else:
...
def verify_model_compatibility():
    ...
    try:
        ...
        logger.info(
            f"✅ FeatureOrderManager initialized: {len(default_features)} default features"
        )
