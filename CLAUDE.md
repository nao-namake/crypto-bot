...
| **🟢 今セッションの作業（Phase 90π Reconciliation Loop・2026-06-17）** | TP/SL管理を **Reconciliation Loop** に全面再設計（plan: `~/.claude/plans/tpsl-bitbank-lexical-quail.md`）。契機＝6/16〜17の**裸ポジ事故**（0.02 BTC long が SL失効=CANCELED_UNFILLEDで裸→成行決済・**実損-2,000円**）。背景＝状態源4分散（VP揮発/sl_state_persistence/Firestore/実ポジ）＋対症療法の積層（TP/SL関連115コミット・孤児SL修正6回・SL消失修正5回・stop⇄stop_limit往復）。新規 `src/trading/reconciliation/`（actions/state/desired/diff/invariants/executor/reconciler の7モジュール・**純粋関数中心**）で、**実建玉(fetch_margin_positions)を唯一の真実源**とし毎サイクル「あるべき状態(desired)」と「実際(actual)」の差分を**冪等(idempotent)**に埋める（Kubernetes Operator / Freqtrade 同型）。「**実建玉>0なら必ず実効SLが存在、さもなくば成行決済**」を invariant で構造的に保証し**裸ポジを根治**。bitbank仕様＝OCO/Bracket注文なし・CANCELED_UNFILLEDは「約定進行中」と「真の失効」の両義でステータス判別不能→**実ポジ残量で判定**・価格がSL価格到達後のstop再配置は即CANCELED_UNFILLEDになる罠→**成行決済で回避**。テスト53件（`tests/unit/trading/reconciliation/`・回帰=裸ポジ価格割れ→成行決済、冪等性=裸→PLACE_SL→次サイクルNOOP）・checks.sh全PASS。**段階デプロイ**: **R0**(commit`ba21a359`+生存ログ修正`c3d2ae5a`・2026-06-17デプロイ済・CI/CD成功)=`reconciliation.enabled=true`/`shadow_mode=true`（**発注せずログのみ**・shadow時は毎サイクル`🔧 Phase 90π reconcile[SHADOW]`で生存可視化・既存TP/SL経路(tp_sl_manager等)が並行稼働で挙動不変）。**1週間観察し誤MARKET_CLOSE提案0を確認** → **R1**=`shadow_mode=false`+Phase 87 C1`dry_run`解除（実発注で裸ポジ是正） → **R2**=エントリーTP/SL統合 → **R3**=旧コード削除。**派生課題(未対応)**: ログPnL(Phase 61.9 `-425円`)が実口座損益(`-2,000円`)と乖離（成行スリッページ+往復手数料約420円+金利約85円を建値ベース概算が未計上）→ライブ分析の損益をbitbank実口座基準にすべき。 |
