...
| 項目 | 値 |
|------|-----|
| **現在Phase** | 82（ゴーストポジションTP/SL異常値バグ修正・SL目標値調整・ML再学習）完了 |
| **直前の作業** | Phase 82: ダスト検出・SL 1000→800円・ML再学習(TP0.43%/SL0.37%)・閾値調整 |
| **次の予定** | デプロイ → 1週間運用 → 統計データで戦略判断 |
| **最新成果** | ダスト残渣による TP+66%/SL-88% 配置事故を完全防止。ML運用整合性向上 |
| **最終更新** | 2026年4月16日 |

> 開発履歴: `docs/開発履歴/SUMMARY.md`（Phase 1-77）、`docs/開発履歴/Phase_71-81.md`、`docs/開発履歴/Phase_82.md`（最新）
...
### 固定金額TP/SL（Phase 82: SL目標調整）

| 信頼度 | 閾値 | TP金額 | SL金額 | Net RR比 |
|--------|------|--------|--------|----------|
| 低 | <0.40 | 600円 | 600円 | 1.00:1 |
| 高 | >=0.40 | 750円 | 800円 | 0.94:1 |
| フォールバック | 信頼度不明 | 750円 | 800円 | 0.94:1 |

Phase 82変更: SL目標1000→800円（実績平均-1,192円→目標1,000円収束を狙う）。
...
| 固定金額SL | 600-800円（Phase 82 信頼度別、旧800-1000円） |
| 日次損失上限 | 5,000円（1%） |
| 週次損失上限 | 20,000円（4%） |
...
### ML品質フィルタ（Phase 82閾値調整）

メタラベリング（Triple Barrier Method）方式。MLは方向予測ではなく、戦略の出した取引が成功するかを判定。
Phase 82: 運用TP 0.43%/SL 0.37% に合わせ再学習（旧 TP 0.45%/SL 0.30%）。

| 閾値 | 値 | 動作 |
|------|-----|------|
| accept_threshold | **0.58**（Phase 82: 0.65→0.58） | p(1)≥0.58 → 取引承認 |
| reject_threshold | **0.42**（Phase 82: 0.45→0.42） | p(1)<0.42 → 拒否 |
| uncertain_penalty | **0.5** | 中間帯(0.42-0.58)は信頼度を50%縮小 |

**再学習後モデル信頼度分布**（2026/4/16学習）:
- Class 1確率 mean=0.50, std=0.10, range=[0.25-0.78]
- CV F1: 0.533±0.032（LGB 40% / XGB 40% / RF 20%）
- 予測クラスバランス: Class 0 43% / Class 1 57%（偏りなし）
- accept >= 0.58 で約 22% 通過（適切なフィルタ）
