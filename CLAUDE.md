...
| 項目 | 値 |
|------|-----|
| **現在Phase** | 83A（CMFバグ修正・SL距離下限0.7%強制・ML再学習）完了 |
| **直前の作業** | Phase 83A: CMF ImportError修正・SL floor 0.7%・ML再学習(TP0.65%/SL0.7%) |
| **次の予定** | デプロイ後24時間観測 → 勝率/SL率改善確認 |
| **最新成果** | 1週間勝率36.4%/-8,478円の原因3つ（CMF停止・SL幅狭・ML乖離）を全て修正 |
| **最終更新** | 2026年4月24日 |

> 開発履歴: `docs/開発履歴/SUMMARY.md`（Phase 1-77）、`docs/開発履歴/Phase_71-81.md`、`docs/開発履歴/Phase_82.md`、`docs/開発履歴/Phase_83.md`（最新）
...
### 固定金額TP/SL（Phase 83A-2: SL下限0.7%強制 + 目標金額大幅増）

| 信頼度 | 閾値 | TP金額 | SL金額 | 備考 |
|--------|------|--------|--------|------|
| 低 | <0.40 | 600円 | 1,200円 | floor 0.7%強制のため実損失は手数料込で約1,000円 |
| 高 | >=0.40 | 750円 | 1,500円 | 同上 |

**Phase 83A-2 変更理由**: 1週間運用（4/16-4/23）で SL幅が実効0.23-0.44%となりBTCノイズ幅未満でSL連発。勝率36.4%、-8,478円の主因と特定。signal_simulation検証でSL 0.75%時勝率82%を実証し下限強制を採用。

**SL下限強制（Phase 83A-2 新設）**:
- `min_distance.ratio: 0.007` (0.7%) を固定金額SL計算後に floor として強制適用
- `sl_offset < avg_price * 0.007` の場合、`sl_offset = avg_price * 0.007` に置換
- ログ出力: `🛡️ Phase 83A-2: SL距離floor適用 - XXX円(0.XX%) → YYY円(0.70%)`
...
### ML品質フィルタ（Phase 83A-3 再学習）

メタラベリング（Triple Barrier Method）方式。MLは方向予測ではなく、戦略の出した取引が成功するかを判定。
Phase 83A-3: 新SL floor 0.7%運用に合わせ再学習（meta_tp_ratio 0.0065 / meta_sl_ratio 0.007）。
...
**再学習後モデル信頼度分布**（2026/4/24学習、Phase 83A-3）:
- Class 1確率 mean=0.531, std=0.067, range=[0.305-0.650]
- CV F1: 0.612（LGB 40% / XGB 40% / RF 20%、Phase 82 0.533から+0.08改善）
- 学習サンプル: 34,970件（180日分）
- class_distribution: success 33% / failure 67%
- 実データ4,596件推論: accept 24.1% / middle 66.7% / reject 9.3%
- 時系列安定性: 隣接足diff mean 0.020（急変なし）
- 極端予測(>0.75/<0.25): 0件（過学習なし）

**Phase 82モデルバックアップ**: `models/production/ensemble_{full,basic}.phase82.pkl.bak` 保持中。
