...
| 項目 | 値 |
|------|-----|
| **現在Phase** | 70（期待値改善: 日次損失上限・RR比改善・シグナル一貫性・ML閾値引き上げ） |
| **直前の作業** | Phase 70.1: 日次損失上限適正化(5%→0.5%) / ML閾値再調整(モデル性能に合わせ0.38/0.35/0.38) / 分析スクリプト更新 |
| **次の予定** | ライブ検証（24時間） |
| **最新成果** | TP 750円/SL 500円（RR 1.5:1）/ 日次損失上限2,500円 / ML閾値モデル性能適合 |
| **最終更新** | 2026年3月22日 |

> 開発履歴: `docs/開発履歴/SUMMARY.md`（Phase 1-69）、`docs/開発履歴/Phase_70.md`（最新）
...
### 固定金額TP/SL（Phase 70: RR比1.5:1改善）

| 信頼度 | 閾値 | TP金額 | SL金額 | RR比 |
|--------|------|--------|--------|------|
| 低 | <0.40 | 600円 | 400円 | 1.5:1 |
| 高 | >=0.40 | 750円 | 500円 | 1.5:1 |
| フォールバック | 信頼度不明 | 750円 | 500円 | 1.5:1 |

TP計算式: `TP価格 = エントリー価格 ± (必要含み益 / 数量)`
必要含み益: `目標純利益 + エントリー手数料(0.1%) + 利息 + 決済手数料(0%)`

SL計算式: `SL価格 = エントリー価格 ∓ (SL距離 / 数量)`
SL距離: `(目標最大損失 - 決済手数料) / ポジションサイズ`（Phase 69: entry_feeはサンクコスト→除外）
