...
### ML品質フィルタ（Phase 79閾値調整）

メタラベリング（Triple Barrier Method）方式。MLは方向予測ではなく、戦略の出した取引が成功するかを判定。

| 閾値 | 値 | 動作 |
|------|-----|------|
| accept_threshold | **0.65**（Phase 79: 0.55→0.65） | ml_pred=1かつ信頼度≥0.65 → 取引承認 |
| reject_threshold | **0.45**（Phase 79: 0.35→0.45） | 信頼度<0.45 or ml_pred=0+信頼度≥0.55 → 拒否 |
| uncertain_penalty | **0.5**（Phase 79: 0.7→0.5） | 中間帯は信頼度を50%縮小 |

モデル信頼度分布: mean=0.5928, std=0.0871, range=[0.50-0.92]
→ accept=0.65で約30%通過（適切なフィルタ機能）
