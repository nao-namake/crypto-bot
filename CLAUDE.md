# CLAUDE.md - Phase 48完了・Claude Code最適化ガイド

**🎯 即座に理解すべき重要事項：本システムはPhase 48完了済み・確定申告対応システム実装（95%時間削減）・Discord週間レポート実装（通知99%削減・コスト35%削減）・TP/SL設定最適化・状態永続化実装・統合TP/SL+トレーリングストップ実装・注文数91.7%削減・ML Agreement Logic修正・証拠金チェックリトライ機能実装・Strategy-Aware ML実装・55特徴量学習・ML統合率100%達成・テストカバレッジ68.32%達成・1,117テスト100%成功・CI/CD品質保証完了の企業級AI自動取引システム**
...
## ✅ **開発時チェックリスト**（Phase 48完了時点）

### **🔍 必須確認事項**
1. **🧪 品質チェック**: `bash scripts/testing/checks.sh`で1,117テスト・68.32%カバレッジ確認
2. **☁️ 本番稼働**: Cloud Run・週間レポート・取引ログ確認
3. **📊 Phase 48機能**: Discord週間レポート・損益グラフ・通知99%削減・コスト35%削減確認
4. **📋 Phase 47機能**: 確定申告システム・SQLite取引記録・CSV出力・移動平均法損益計算確認
5. **🎯 Phase 42.4機能**: TP/SL設定最適化・ハードコード削除・状態永続化・22注文問題解決確認
6. **🎯 Phase 42.3機能**: ML Agreement Logic修正・Feature Warning抑制・証拠金チェックリトライ確認
7. **🎯 Phase 42.2機能**: トレーリングストップ実装・2%発動・3%距離・最小0.5%利益ロック確認
8. **🎯 Phase 42.1機能**: 統合TP/SL実装・注文数91.7%削減・加重平均価格・8ステップフロー確認
9. **🎯 Phase 41.8.5機能**: ML統合率100%達成・閾値0.45/0.60最適化・3段階統合ロジック確認
10. **🧠 Phase 41.8機能**: Strategy-Aware ML・55特徴量学習・訓練/推論一貫性・F1スコア0.56-0.61確認
11. **🎯 Phase 40機能**: 79パラメータ最適化・統合最適化スクリプト・期待効果+50-70%確認
12. **💰 Phase 38.7.2機能**: 完全指値オンリー実装・年間¥150,000削減・約定率90-95%確認
13. **🏗️ Phase 38機能**: trading層5層アーキテクチャ・後方互換性・テスト追加確認
14. **🎯 Phase 37.4機能**: SL配置成功率100%・trigger_price修正・エラー30101解消確認
15. **💰 Phase 37.3機能**: コスト削減35-45%・5分間隔実行・月額700-900円確認（Phase 48でさらに最適化）
16. **🛡️ Phase 37.2機能**: bitbank API GET認証・エラー20003解消確認
17. **🎯 Phase 37機能**: SL注文stop対応・エラー50062解消確認
18. **🛡️ Phase 36機能**: Graceful Degradation・Container exit(1)解消確認
19. **📊 Phase 35機能**: バックテスト10倍高速化・45分実行時間確認

### **🚀 開発開始前チェック**
1. **📊 最新状況把握**: システム状況・エラー状況・Phase 48ステータス確認
2. **🧪 品質基準**: 1,117テスト・68.32%カバレッジ・コード品質確認
3. **⚙️ 設定整合性**: config整合性・環境変数・features.yaml・unified.yaml・thresholds.yaml確認

---

**🎯 Phase 48完了・企業級AI自動取引システム**:
