...
| 項目 | 値 |
|------|-----|
| **現在Phase** | 69（SL超過修正 + 逆張りショート対策） |
| **直前の作業** | Phase 69: SL計算entry_fee除去 + normal_rangeトレンド型重み付与 + EMAトレンドフィルタ |
| **次の予定** | ライブデプロイ → GCPログ検証 |
| **最新成果** | Phase 69: SL距離2倍改善(0.09%→0.19%) + normal_rangeトレンド型25% + 逆トレンドペナルティ50% |
| **最終更新** | 2026年3月14日 |
...
### 固定金額TP/SL（Phase 68.8: 信頼度別2段階）

| 信頼度 | 閾値 | TP金額 | SL金額 |
|--------|------|--------|--------|
| 低 | <0.40 | 400円 | 400円 |
| 高 | >=0.40 | 500円 | 500円 |
| フォールバック | 信頼度不明 | 500円 | 500円 |
