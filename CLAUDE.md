...
#### γ-③.4 (コミット `1dbaf0ec`): Maker 観察可能化 + timeout 拡張
| 修正 | 内容 |
|---|---|
| ログレベル格上げ | order_strategy.py の 5 箇所（Maker戦略有効・Maker注文試行・Maker約定成功・Maker買い価格・Maker売り価格）+ executor.py の 1 箇所（Phase 90γ-③.3 Taker許可）を info→warning |
| timeout 拡張 | thresholds.yaml: timeout_seconds 60→120 秒 / retry_interval_ms 2000→1500 ms / max_retries 3→5 |
| 期待効果 | (1) 観察可能性回復で真の Maker 阻害要因が判明 (2) 120 秒以内に spread 拡大すれば Maker 約定 → Maker 化率 0% → 40-70% 期待 |
| 副作用 | コードロジック変更ゼロ（取引件数影響なし）|
...
- 計画書: `~/.claude/plans/gcp-humming-bear.md`（Phase 90γ-② 計画含む全体記録）/ `~/.claude/plans/gcp-silly-frog.md`（γ-③.1 → γ-③.2 → γ-③.3 → γ-③.4 と上書き使用）
- 詳細記録: `docs/開発履歴/Phase_90.md` 末尾「Phase 90γ-③.1 / γ-③.2 / γ-③.3 / γ-③.4」セクション
...
- 修正済本体:
  ...
  - γ-③.4: `src/trading/execution/order_strategy.py`（5 ログ info→warning）`src/trading/execution/executor.py:314`（1 ログ info→warning）`config/core/thresholds.yaml`（timeout/retry/max_retries 拡張）
...
eeaad1d1 docs: Phase 90γ-③.2/③.3 完了に伴うドキュメント更新
