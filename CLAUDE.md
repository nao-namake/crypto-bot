# CLAUDE.md - Phase 63開発ガイド

**最終更新**: 2026年2月8日

---

## システム現状（Phase 62.21完了）

### 概要

| 項目 | 値 |
|------|-----|
| **Phase** | 62.21完了（SL検証10分化・タイムアウトバグ修正）|
...
| 区分 | 内容 | 効果 |
|------|------|------|
| **Maker戦略** | エントリー/TP決済でMaker約定 | 往復手数料0%（Phase 62.19: Maker無料化対応） |
| **SL改善** | 逆指値指値化 + 幅0.4% | スリッページ抑制・勝率改善 |
| **手数料最適化** | バックテスト/ライブ統一 | 正確な損益計算 |
| **分析機能** | スリッページ記録・レポート | 実態把握可能化 |
| **SLパターン分析** | MFE/MAE活用・ライブ/バックテスト両対応 | 改善示唆自動生成 |
| **手数料改定対応** | 2026年2月2日改定対応 | Maker 0%・Taker 0.1%に更新 |
| **TP/SL欠損自動復旧** | 10分後検証・自動再構築 | APIエラー時の保護強化（Phase 62.21: 5分→10分） |
| **タイムアウトバグ修正** | 決済失敗時success=False | 正しいエラーハンドリング（Phase 62.21） |
...
#### 固定金額TP（Phase 61.7追加・Phase 62.19手数料改定対応）
...
（Phase 62.19: Maker 0%のため手数料は実質0円）
