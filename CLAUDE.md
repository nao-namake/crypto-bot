...
| 項目 | 値 |
|------|-----|
| **現在Phase** | **Phase 90θ SLMonitor誤発火 真因特定＆根本修正 実装完了（2026-06-02）→ コミット・デプロイ待ち**。Phase 90η のポジ残量ガードはデプロイ済（6/1 07:03 JST・イメージ`659b5f82`で確認）だが、ライブ分析＋GCPログ精査で**デプロイ後も誤発火が継続**と判明（6/1 16:25/16:30 に2件・実ポジ0なのにDRY_RUN緊急決済発火）。真因は2層: **(1)幽霊VPクリーンアップ漏れ**=`stop_manager._check_tp_sl_execution`がSL約定を`status=="closed"`のみで判定し`CANCELED_UNFILLED`を約定検知できず実0VPが残存→毎5分C5対象、**(2)C5ガードの設計限界**=`CANCELED_UNFILLED`はstop約定の正常な中間状態で建玉が一時残存し1回スナップショットの残量確認(`fetch_margin_positions`キャッシュ無)をすり抜ける。修正: **P0-A** `is_canceled_unfilled`をSL約定として検知し幽霊VP即除去 / **P0-B** canceled_unfilled連続検出カウンタ(3回連続+残存でのみ緊急決済昇格・`_fetch_failure_counts`と同型) / **P1** REJECTEDをガードから除外(注文却下=SL未配置・実0スキップの逆リスク回避) / **P3** ライブ分析にSLMonitor誤発火監視追加(従来DRY_RUN発火を検出できない盲点を解消)。dry_run継続(実SL決済はPhase 64.12担保)。テスト追加・checks.sh全PASS。Phase 90η/90ζ本番稼働中 |
...
| **最終更新** | 2026年6月2日 - Phase 90θ SLMonitor誤発火 真因特定＆根本修正（Phase 90ηガードのデプロイ後誤発火継続を発見→幽霊VPクリーンアップ漏れ＋canceled_unfilled中間状態のタイミング競合の2層を修正・REJECTED逆リスク除外・ライブ分析に誤発火監視追加）実装完了 |
