...
| 項目 | 値 |
|------|-----|
| **現在Phase** | **Phase 90γ-③.1 (Drift exclude_features オシレーター漏れ修正 + min_instances 整合) 完了・本番デプロイ済（2026-05-24 02:53 JST）** |
| **直前の作業** | 2026-05-24 ライブ分析 24h で Phase 90γ-③ デプロイ後も Drift 検出が **545 件/24h（直近 1h で 73 件）** 継続発生していたのを発見。検出 TOP15 が**正規化済みオシレーター系（adx_14 26, cmf_20 16, rsi_14 13, plus_di_14 13, cci_20 13, bb_position 11, channel_position 11）**だったため exclude_features 漏れと判定。43→59 個に拡張（オシレーター 9 + volume_lag 3 + returns 4 個）+ min_instances 設定（thresholds.yaml/gcp_config.yaml）を本番デプロイ実態 (=0) に整合 |
| **次の予定** | 24h 観察で drift 件数 545 → 数十件以下に抑制されたか検証 → Phase 90γ-④ (ML 品質改善・Calibration / Focal Loss / CatBoost / Optuna 試行数増) 着手判断 |
...
| **本番効果（5/24 02:13 ライブ分析 24h 時点）** | Phase 88 I3 EMERGENCY_STOP **0 件** / bitbank 50062 **0 件** / Phase 50.4 維持率拒否 **0 件** / Auto Retraining HTTP 401 **デプロイ後 0 件** / Drift 検出 545 件/24h（Phase 90γ-③.1 で抑制中）/ 取引拒否は Phase 85 trending 仕様（24h 平均 ADX 63.0 = 一般指標でも明確な trending と確認済） |
| **追加課金** | **ゼロ**（GPU 不採用 / LLM 不採用 / 全て無料 API） |
| **GCP 月額** | 現状 ¥3,000 → Stage 1+3 後 **¥1,400-1,700 見込み**（実測待ち） |
| **最終更新** | 2026年5月24日 - Phase 90γ-③.1 (Drift オシレーター漏れ修正 + min_instances 整合) 実装完了 |
...
## 🚀 セッション再開時の最優先タスク（2026-05-24 時点・Phase 90γ-③.1 完了 → drift 抑制効果検証フェーズ）

**Phase 90γ シリーズ (γ-①・γ-① レビュー・γ-②・γ-③・γ-③.1) 全工程完了・本番デプロイ済**。24h 観察で drift 件数 545 → 数十件以下に抑制されたか検証 → Phase 90γ-④ (ML 品質改善) 着手判断。
...
# Phase 89-β Drift 検出件数（Phase 90γ-③.1 後の期待: 545 → 数十件以下）
gcloud logging read 'textPayload=~"Phase 89-β: Drift 検出"' --freshness=24h | wc -l

# Drift 検出特徴量 TOP15（オシレーター系が消えているか確認）
...
24h 観察結果に基づき以下を判定:
- Drift 件数 545 → 数十件以下に抑制達成 → Phase 90γ-③.1 完全成功 → Phase 90γ-④ (ML 改善) 着手
- Drift 件数まだ多い → exclude_features の追加漏れ調査（残った検出特徴量を追加除外）
...
6. **ADX 遅行性対策（議論余地）**: 価格は収束していても ADX が高止まりして trending 継続判定する問題。ADX + 直近 4h 値幅の AND 条件 or ADX 変化率を加味（ただし Phase 85 全停止根拠の損失データあるため慎重に）
...
- 計画書: `~/.claude/plans/gcp-humming-bear.md`（Phase 90γ-② 計画含む全体記録）/ `~/.claude/plans/gcp-silly-frog.md`（Phase 90γ-③.1）
- 詳細記録: `docs/開発履歴/Phase_90.md` 末尾「Phase 90γ-③ / γ-③.1」セクション
