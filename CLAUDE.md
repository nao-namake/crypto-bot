# CLAUDE.md - Phase 40完了・Claude Code最適化ガイド

**🎯 即座に理解すべき重要事項：本システムはPhase 40完了済み・79パラメータOptuna最適化実装・統合最適化スクリプト完成・テストカバレッジ70.56%達成・1,097テスト100%成功・CI/CD品質保証完了の企業級AI自動取引システム**
...
# 期待結果: ✅ 1,097テスト100%成功・70.56%カバレッジ通過・約80秒で完了
```
...
📁 重要ファイル:
scripts/testing/checks.sh       # 品質チェック（開発必須・1,094テスト）
...
### **🧪 品質保証（必須遵守）**
- **🔬 テスト実行**: 開発前後に`checks.sh`必須実行・1,094テスト100%維持・70.58%カバレッジ
...
## ✅ **開発時チェックリスト**（Phase 38.7.2完了時点）

### **🔍 必須確認事項**
1. **🧪 品質チェック**: `bash scripts/testing/checks.sh`で1,097テスト・70.56%カバレッジ確認
2. **☁️ 本番稼働**: Cloud Run・Discord通知・取引ログ確認
3. **🎯 Phase 40機能**: 79パラメータ最適化・統合最適化スクリプト・期待効果+50-70%確認
4. **💰 Phase 38.7.2機能**: 完全指値オンリー実装・年間¥150,000削減・約定率90-95%確認
5. **🏗️ Phase 38機能**: trading層5層アーキテクチャ・後方互換性・テスト追加確認
6. **🎯 Phase 37.4機能**: SL配置成功率100%・trigger_price修正・エラー30101解消確認
7. **💰 Phase 37.3機能**: コスト削減35-45%・5分間隔実行・月額1,100-1,300円確認
8. **🛡️ Phase 37.2機能**: bitbank API GET認証・エラー20003解消確認
9. **🎯 Phase 37機能**: SL注文stop対応・エラー50062解消確認
10. **🛡️ Phase 36機能**: Graceful Degradation・Container exit(1)解消確認
11. **📊 Phase 35機能**: バックテスト10倍高速化・45分実行時間確認

### **🚀 開発開始前チェック**
1. **📊 最新状況把握**: システム状況・エラー状況・Phase 40ステータス確認
2. **🧪 品質基準**: 1,097テスト・70.56%カバレッジ・コード品質確認
3. **⚙️ 設定整合性**: config整合性・環境変数・features.yaml・unified.yaml・thresholds.yaml確認

---

**🎯 Phase 40完了・企業級AI自動取引システム**:
