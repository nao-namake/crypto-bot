...
| **次の予定** | **明朝（6/2）に Phase 90η Day 1 検証**（デプロイ `crypto-bot-service-prod-phase89a-cost-opt-0531-2146` 6/1 06:46 JST 後の一晩分で確認）：①稼働率が ~100% 表示に直ったか（旧 46.8% 誤表示の解消）、②WebSocket 警告が消え `✅ ... trigger モード運用のため非常駐（正常）` に変わったか、③**SLMonitor 新ログ `✅ Phase 90η: SL ... ポジション既消滅 → 既決済とみなしスキップ`** が出ているか（誤発火が already_closed で抑止される＝根本修正の動作確認）、④デプロイ後 `sl_monitor_validator.py --days 3` で誤発火率が改善したか。**並行して既存 Day 1 項目も継続**：(Phase 90ζ) 土日 SL縮小 WARNING / (Phase 90ε) 土日 TP500・TP約定率/保有時間 / (Phase 90δ) post_only 実 Maker 化率・50062 件数減・TP約定率。Phase 90γ-⑥ 累積効果（PF・月利）は Day 7（6/4-5 頃）に最終判定。数日後に実 MFE 分布でレジーム別TP/SL の要否を再評価 |
