# CLAUDE.md - Phase 29.5完了・Claude Code最適化ガイド

**🎯 即座に理解すべき重要事項：本システムはPhase 29.5完了済み・ML予測統合実装・真のハイブリッドMLbot実現・625テスト100%成功・64.74%カバレッジ・CI/CD品質保証完了の企業級AI自動取引システム**
...
### **✅ Phase 29.5完了ステータス**
- **📅 2025/09/30完了**: ML予測統合実装・真のハイブリッドMLbot実現・品質保証完了
- **🧪 テスト品質**: 625テスト100%成功・64.74%カバレッジ・CI/CD品質ゲート通過
...
### **🧪 品質チェック（開発前後必須）**
```bash
# Phase 29.5品質チェック - 開発前後に必ず実行
bash scripts/testing/checks.sh

# 期待結果: ✅ 625テスト100%成功・64.74%カバレッジ通過・約30秒で完了
```
...
📁 重要ファイル:
scripts/testing/checks.sh       # 品質チェック（開発必須・625テスト）
config/core/unified.yaml        # 統合設定ファイル（Phase 29.5最適化完了）
config/core/thresholds.yaml     # ML統合設定（Phase 29.5新規追加）
...
## ✅ **Phase 29.5作業時チェックリスト**（次回作業用）

### **🔍 必須確認事項**
1. **🧪 品質チェック**: `bash scripts/testing/checks.sh`で625テスト・64.74%カバレッジ確認
2. **☁️ 本番稼働**: Cloud Run・Discord通知・取引ログ確認
3. **🤖 Phase 29.5機能**: ML予測統合・戦略70%+ML30%加重平均・一致/不一致判定確認
4. **🎯 Phase 29機能**: 統一設定管理体系・設定不整合解消確認
5. **💰 Phase 28機能**: テイクプロフィット/ストップロス・完全トレーディングサイクル確認
6. **🧠 Phase 27機能**: ML信頼度連動制限・最小ロット優先・少額運用対応確認

### **🚀 開発開始前チェック**
1. **📊 最新状況把握**: システム状況・エラー状況・Phase 29.5ステータス確認
2. **🧪 品質基準**: 625テスト・64.74%カバレッジ・コード品質確認
3. **⚙️ 設定整合性**: config整合性・環境変数・unified.yaml・thresholds.yaml確認
