# tests/manual/ - 手動テスト・開発検証（Phase 49完了時点）

## 🎯 役割・責任

手動テスト・開発検証スクリプトを管理し、開発時の動作確認、コンポーネント検証、API接続テストを支援します。自動テストでカバーできない実際のAPI接続や統合動作を手動で確認し、開発品質の向上と問題の早期発見を支援します（Phase 49: 1,117テスト・68.32%カバレッジ達成）。
...
- `scripts/testing/checks.sh`: 品質チェック（Phase 49完了版）・テスト統合・CI/CD品質ゲート
