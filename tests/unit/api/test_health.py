...
class TestHealthAPI:
    ...
    def temp_status_file(self):
        ...
        # 元のファイルパスを保存
        original_file = health_checker.state_manager.local_state_file if health_checker.state_manager else "status.json"
        ...
        with patch('crypto_bot.api.health.health_checker') as mock_checker:
            mock_checker.state_manager = Mock() if health_checker.state_manager else None
            mock_checker.state_manager.local_state_file = temp_file if mock_checker.state_manager else None
    ...
    def test_error_handling_in_health_checks(self, client):
        ...
        with patch('crypto_bot.api.health.health_checker.check_basic_health') as mock_check:
