...
class TestTradeTrackerBasic:
    ...
    def test_record_exit_long_profit(self):
        ...
        assert trade["pnl"] == 100  # (15100000 - 15000000) * 0.001
        assert trade["exit_reason"] == "TP"
        assert len(tracker.completed_trades) == 1
        assert tracker.total_pnl == 100
    ...
    def test_record_exit_long_loss(self):
        ...
        assert trade is not None
        assert trade["pnl"] == -100  # (14900000 - 15000000) * 0.001
        assert tracker.total_pnl == -100
    ...
    def test_record_exit_short_profit(self):
        ...
        assert trade is not None
        assert trade["pnl"] == 100  # (15000000 - 14900000) * 0.001
    ...
    def test_record_exit_short_loss(self):
        ...
        assert trade is not None
        assert trade["pnl"] == -100  # (15000000 - 15100000) * 0.001
...
class TestTradeTrackerMFEMAE:
    ...
    def test_mfe_mae_statistics(self):
        ...
        assert metrics["trades_with_missed_profit"] == 1  # 取引1のみ
        assert metrics["missed_profit_total"] == 150  # 200 - 50
...
class TestTradeTrackerPerformanceMetrics:
    ...
    def test_profit_factor_calculation(self):
        ...
        # 利益300円、損失100円 → PF = 3.0
        tracker.record_entry("win_1", "buy", 0.001, 15000000, datetime.now(), "Test")
        tracker.record_exit("win_1", 15300000, datetime.now(), "TP")  # +300

        tracker.record_entry("loss_1", "buy", 0.001, 15000000, datetime.now(), "Test")
        tracker.record_exit("loss_1", 14900000, datetime.now(), "SL")  # -100

        metrics = tracker.get_performance_metrics()
        assert metrics["total_profit"] == 300
        assert metrics["total_loss"] == -100
