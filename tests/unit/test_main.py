...
class TestPrepareDataEnhanced:
    ...
    @patch("crypto_bot.main.MarketDataFetcher")
    @patch("crypto_bot.main.DataPreprocessor")
    def test_prepare_data_non_datetime_index(
        self, mock_preprocessor, mock_fetcher_class
    ):
...
class TestIntegrationScenarios:
    ...
    def test_full_prepare_data_workflow(
        ...
        cfg = {
            ...
            "ml": {"feat_period": 20, "target_type": "regression", "test_size": 0.3},
        }
...
class TestCLICommands:
    ...
    def test_backtest_command_single_strategy(
        self,
        mock_split,
        mock_engine_class,
        mock_factory,
        ...
        metrics_df = pd.DataFrame(
            {"net_profit": [100.5, 200.3], "sharpe_ratio": [1.2, 1.5]}
        )
        trades_df = pd.DataFrame(
            {"entry_time": ["2023-01-01", "2023-01-02"], "profit": [50.0, 75.0]}
        )
    ...
    def test_train_command_with_model_type(
        self, mock_save, mock_train_best, mock_prepare, mock_load_config
    ):
    ...
    def test_optimize_backtest_command(self, mock_optimize, mock_load_config):
        ...
        with runner.isolated_filesystem():
            ...
            mock_optimize.assert_called_once_with(
                test_config, output_csv="optimization_results.csv"
            )
    ...
    def test_optimize_and_train_command(
        ...
        mock_study.trials_dataframe.return_value = pd.DataFrame(
            {"value": [0.8, 0.85], "params_n_estimators": [50, 100]}
        )
        mock_optuna.return_value = mock_study
