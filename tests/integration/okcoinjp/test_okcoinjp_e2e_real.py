...
def okcoinjp_client(version_manager):
    ...
    if not api_key or not api_secret:
        pytest.skip(
            "OKCoinJP API keys not set. Set OKCOINJP_API_KEY, OKCOINJP_API_SECRET, and OKCOINJP_PASSPHRASE environment variables for production testing."
        )
...
def test_api_version_compatibility(version_manager):
    ...
    if report["overall_status"] != "PASS":
        ...
        # 取引所が存在しない場合もスキップ
        required_methods_check = report["checks"]["required_methods"]
        if (
            not required_methods_check["passed"]
            and "has no attribute" in required_methods_check["message"]
        ):
            pytest.skip(
                f"Exchange not available in CCXT: {required_methods_check['message']}"
            )
