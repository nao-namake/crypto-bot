    ...
    def setUpClass(cls):
        ...
        cls.test_config = {
            ...
            "performance_monitoring": {
                "metrics": ["accuracy", "profitability", "sharpe_ratio", "max_drawdown"],
                "alert_thresholds": {
                    "accuracy_drop": 0.1,
    ...
    def test_01_market_environment_analyzer_basic(self):
        ...
        try:
            ...
            required_keys = [
                ...
                "liquidity_score", "overall_stress"
            ]
    ...
    def test_05_ab_testing_system_basic(self):
        ...
        try:
            ...
            for group, performance_boost in [("control", 0), ("treatment", 0.1)]:
                for i in range(25):  # 十分なサンプルサイズ
                    result = {
                        ...
                        "profitability": 0.5 + performance_boost + np.random.normal(0, 0.08),
                        "sharpe_ratio": 1.0 + performance_boost + np.random.normal(0, 0.2)
                    }
            ...
            if status is not None:
                ...
                logger.info(f"   ✅ Experiment status: {status.get('status', 'unknown')}")
            else:
                # ステータスが取得できない場合も許容
                logger.info("   ⚠️  Experiment status not available - this is acceptable")
    ...
    def test_06_integrated_workflow(self):
        ...
        try:
            ...
            for cycle in range(5):  # 5サイクル実行
                # 1. 市場環境解析
                market_analysis = market_analyzer.analyze_comprehensive_market_environment(
                    self.sample_price_data,
                    external_data=self.sample_market_context
                )
    ...
    def test_07_phase_b_integration(self):
        ...
        try:
            ...
            try:
                ...
                logger.info("   ⚠️  Phase B modules not available, testing with fallback")
    ...
    def test_08_phase_c1_integration(self):
        ...
        try:
            ...
            try:
                ...
                from crypto_bot.ml.cross_timeframe_ensemble import CrossTimeframeIntegrator
                
                logger.info("   ✅ Phase C1 modules available for integration")
                phase_c1_available = True
                
            except ImportError:
                logger.info("   ⚠️  Phase C1 modules not available, testing with simulation")
            ...
            ensemble_results = {
                "stage1_results": {  # タイムフレーム内アンサンブル
                    "15m": {"prediction": 1, "confidence": 0.75, "models_agreement": 0.8},
                    "1h": {"prediction": 1, "confidence": 0.82, "models_agreement": 0.9}, 
                    "4h": {"prediction": 0, "confidence": 0.68, "models_agreement": 0.7}
                },
            ...
            weight_diff = sum([abs(adjusted_weights[tf] - base_weights[tf]) 
                             for tf in ["15m", "1h", "4h"]]) / 3
    ...
    if result.wasSuccessful():
        ...
        if failures:
            ...
            for i, (test, traceback) in enumerate(result.failures, 1):
                print(f"   {i}. {test}: {traceback.split('AssertionError: ')[-1].split('\\n')[0]}")
