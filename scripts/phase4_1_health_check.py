...
class ProductionHealthChecker:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("margin_mode") == True:
                    self.log_check("信用取引モード", "success", "信用取引モードが有効です")
                else:
                    self.log_check("信用取引モード", "warning", "信用取引モードが無効です")
                    
                if data.get("exchange") == "bitbank":
                    self.log_check("取引所設定", "success", "Bitbank取引所が設定されています")
                else:
                    self.log_check("取引所設定", "warning", f"取引所: {data.get('exchange')}")
                    
                if data.get("api_credentials") == "healthy":
                    self.log_check("API認証", "success", "API認証が正常です")
                else:
                    self.log_check("API認証", "warning", f"API認証: {data.get('api_credentials')}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if kelly_ratio > 0.1:
                    ...
                    self.log_check("Kelly比率", "warning", f"Kelly比率が低い: {kelly_ratio}")
                ...
                if max_drawdown < 0.1:
                    self.log_check("最大ドローダウン", "success", f"最大ドローダウン: {max_drawdown}")
                else:
                    self.log_check("最大ドローダウン", "warning", f"最大ドローダウンが高い: {max_drawdown}")
            ...
            if response.status_code == 200:
                ...
                for metric in important_metrics:
                    if metric in metrics_text:
                        self.log_check(f"メトリクス: {metric}", "success", "メトリクスが利用可能です")
                    else:
                        self.log_check(f"メトリクス: {metric}", "warning", "メトリクスが見つかりません")
    ...
    def generate_report(self) -> Dict:
        ...
        successful_checks = len([r for r in self.check_results if r["status"] == "success"])
