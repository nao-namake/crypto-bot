...
class Phase8StatisticsVerification:
    ...
    def _test_statistics_initialization(self) -> bool:
        ...
        try:
            ...
            if status["active_trades"] != 0:
                self.logger.error(
                    f"初期アクティブ取引数が異常: {status['active_trades']}"
                )
                return False
    ...
    def _test_signal_recording(self) -> bool:
        ...
        try:
            ...
            if len(recorded_signals) == len(signals):
                ...
                self.logger.error(
                    f"シグナル記録数不一致: {len(recorded_signals)}/{len(signals)}"
                )
                return False
    ...
    def _test_trade_execution(self) -> bool:
        ...
        try:
            ...
            if len(executed_trades) == len(trade_scenarios):
                ...
                self.logger.error(
                    f"取引実行数不一致: {len(executed_trades)}/{len(trade_scenarios)}"
                )
                return False
    ...
    def _test_realtime_updates(self) -> bool:
        ...
        try:
            ...
            self.integration_service.record_trade_signal(
                signal="realtime_test", confidence=0.8, strategy_type="RealtimeTest"
            )
    ...
    def generate_verification_report(self):
        ...
        try:
            ...
            self.logger.info(
                f"   検証実行時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            self.logger.info(
                f"   アクティブ取引数: {final_status.get('active_trades', 0)}"
            )
