...
class ProductionIntegrationSystem:
    ...
    def __init__(self, config_path: str = None):
        ...
        config_path : str
            設定ファイルパス
        """
        self.config_path = config_path or str(project_root / "config" / "production_integration.yml")
        ...
        logger.info(f"Production Integration System initialized - Phase: {self.current_phase.value}")
    ...
    def _load_deployment_config(self) -> DeploymentConfig:
        ...
        try:
            ...
            return DeploymentConfig(
                ...
                phase_advance_conditions=config_data.get('phase_advance_conditions', {
                    'min_improvement': 0.02,  # 2%以上の改善
                    'min_confidence': 0.8,   # 80%以上の信頼度
                    'max_drawdown': -0.03,   # 3%以下のドローダウン
                    'min_trades': 10         # 最低10取引
                }),
                phase_rollback_conditions=config_data.get('phase_rollback_conditions', {
        ...
        try:
            ...
            if current_index < len(phase_sequence) - 1:
                ...
                if self._check_advance_conditions():
                    ...
                    logger.warning("Advance conditions not met, staying in current phase")
            else:
    ...
    def rollback_to_previous_phase(self, reason: str = "Performance degradation"):
        ...
        try:
            ...
            if current_index > 0:
                ...
                logger.warning(f"Rolled back to phase: {previous_phase.value} - Reason: {reason}")
            else:
    ...
    def _get_current_metrics(self) -> Optional[PerformanceMetrics]:
        ...
        if np.random.random() < 0.1:  # 10%の確率でメトリクス更新
            strategy_type = "ensemble" if self.deployment_config.ensemble_enabled else "traditional"
    ...
    def _check_performance_anomalies(self, metrics: PerformanceMetrics):
        ...
        # 緊急停止条件チェック
        if (metrics.max_drawdown < rollback_conditions['max_drawdown'] or
            metrics.win_rate < rollback_conditions['min_win_rate'] or
            metrics.error_count / max(metrics.signal_count, 1) > rollback_conditions['max_error_rate']):
    ...
    def _check_advance_conditions(self) -> bool:
        ...
        return (improvement >= advance_conditions['min_improvement'] and
                len(recent_ensemble) >= advance_conditions['min_trades'])
    
    def _check_auto_transitions(self):
        """自動移行チェック"""
        # ロールバック条件チェック
        if self._should_rollback():
            self.rollback_to_previous_phase("Automatic rollback due to performance degradation")
        
        # 進歩条件チェック（自動進歩は慎重に）
        elif (self.current_phase in [DeploymentPhase.SHADOW_TESTING] and
              self._check_advance_conditions()):
    ...
    def _should_rollback(self) -> bool:
        ...
        for metrics in recent_metrics:
            if (metrics.win_rate < self.deployment_config.min_win_rate or
                metrics.max_drawdown < self.deployment_config.max_drawdown_limit):
    ...
    def _update_production_config(self, **kwargs):
        ...
        try:
            production_config_path = project_root / "config" / "bitbank_101features_production.yml"
    ...
    def _get_performance_summary(self) -> Dict[str, Any]:
        ...
        if self.traditional_metrics:
            recent_traditional = self.traditional_metrics[-10:]
            summary['traditional_avg_win_rate'] = np.mean([m.win_rate for m in recent_traditional])
            summary['traditional_avg_return'] = np.mean([m.total_return for m in recent_traditional])
        
        if self.ensemble_metrics:
            recent_ensemble = self.ensemble_metrics[-10:]
            summary['ensemble_avg_win_rate'] = np.mean([m.win_rate for m in recent_ensemble])
            summary['ensemble_avg_return'] = np.mean([m.total_return for m in recent_ensemble])
    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                integration_system.rollback_to_previous_phase(reason or "Manual rollback")
                print("✅ 前フェーズにロールバックしました")
                
            elif choice == '4':
                reason = input("緊急停止理由を入力してください: ").strip()
                integration_system.emergency_stop_system(reason or "Manual emergency stop")
                print("🚨 緊急停止を実行しました")
                ...
                print(f"  アンサンブル有効: {status['deployment_config']['ensemble_enabled']}")
                print(f"  トラフィック分割: {status['deployment_config']['traffic_split']:.1%}")
                
                if 'performance_summary' in status:
                    perf = status['performance_summary']
                    print(f"  従来メトリクス: {perf['traditional_metrics_count']}件")
                    print(f"  アンサンブルメトリクス: {perf['ensemble_metrics_count']}件")
