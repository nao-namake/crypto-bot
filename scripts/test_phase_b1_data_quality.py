...
def test_macro_fetcher_multisource(config: dict):
    ...
    try:
        ...
        print(
            f"🔗 統合Macroデータ: {len(unified_data) if unified_data is not None else 0} records"
        )
        ...
        if macro_data_dict and any(not df.empty for df in macro_data_dict.values()):
            total_records = sum(
                len(df) for df in macro_data_dict.values() if not df.empty
            )
    ...
    try:
        ...
        if active_sources:
            ...
            if default_ratio <= target_default_ratio:
                print(
                    f"✅ 品質改善目標達成! ({default_ratio:.3f} ≤ {target_default_ratio})"
                )
                return True
            else:
                print(
                    f"⚠️ 品質改善継続必要 ({default_ratio:.3f} > {target_default_ratio})"
                )
                return False
        else:
            print("❌ 全データソース失敗")
