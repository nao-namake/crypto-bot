    ...
    def run_command(self, command: str, description: str, timeout: int = 300) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self.log_step(
                    description, "success", f"Output: {result.stdout[:200]}..."
                )
                return True
            else:
...
def phase_3_1_local_quality_checks(manager: DeploymentManager) -> bool:
    ...
    if not manager.run_command(
        "python3 -m py_compile crypto_bot/main.py", "Python構文チェック (main.py)"
    ):
        return False
    ...
    manager.log_step(
        "local_quality_checks", "completed", "全てのローカル品質チェックが完了"
    )
    return True
...
def phase_3_1_basic_tests(manager: DeploymentManager) -> bool:
    ...
    if not manager.run_command(
        "python3 -m pytest tests/unit/main/ -v --tb=short", "メインモジュールテスト"
    ):
...
def phase_3_1_build_image(manager: DeploymentManager) -> bool:
    ...
    # Docker buildx設定確認
    if not manager.run_command("docker buildx ls", "Docker buildx設定確認"):
        return False
    ...
    manager.log_step(
        "build_image", "completed", f"AMD64イメージビルド完了: {build_tag}"
    )
    return True
