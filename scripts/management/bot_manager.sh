...
perform_status_check() {
    ...
    if [ "$has_processes" = false ] && [ "$has_locks" = false ]; then
        ...
        echo ""
        echo -e "${BLUE}💡 Claude Codeでバックグラウンドプロセス表示が出る場合:${NC}"
        echo -e "${YELLOW}   これは表示上の残存記録で、実際のプロセスではありません${NC}"
        echo -e "${YELLOW}   上記確認で「システム完全停止状態」なら安全です${NC}"
        echo ""
        echo -e "${BLUE}📈 推奨コマンド:${NC}"
        echo "bash scripts/management/run_safe.sh local paper   # ペーパートレード"
        echo "bash scripts/management/run_safe.sh status        # 状況確認"
        return 0
    else
