...
#   - Secret Manager認証情報の統合管理
#   - CI/CD用IAM権限の自動付与
#   - プロジェクト設定の最適化・検証
#
# 使用方法:
#   bash scripts/deployment/setup_gcp_environment.sh --full         # 完全セットアップ（推奨）
#   bash scripts/deployment/setup_gcp_environment.sh --apis-only    # API・サービス設定のみ
#   bash scripts/deployment/setup_gcp_environment.sh --secrets-only # Secret Manager設定のみ
#   bash scripts/deployment/setup_gcp_environment.sh --verify       # 設定検証のみ
#
# 前提条件:
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/gcp_environment_setup_$(date +%Y%m%d_%H%M%S).log"
...
create_or_update_secret() {
    ...
    if gcloud secrets describe "$secret_id" --project="$PROJECT_ID" &>/dev/null; then
        ...
        if gcloud secrets create "$secret_id" \
            ...
            --labels="environment=production,managed-by=crypto-bot,phase=22" \
            --project="$PROJECT_ID"; then
            step_result "シークレット作成成功: $secret_id" "true"
        else
            step_result "シークレット作成失敗: $secret_id" "false"
            return 1
        fi
    fi
    ...
    if [ -z "$secret_value" ]; then
        log "WARN" "空の値が入力されました。スキップします: $secret_id"
        return 1
    fi
    
    # シークレット値設定
    if echo "$secret_value" | gcloud secrets versions add "$secret_id" \
        --data-file=- \
        --project="$PROJECT_ID"; then
        step_result "シークレット値設定成功: $secret_id" "true"
    else
        step_result "シークレット値設定失敗: $secret_id" "false"
        return 1
    fi
}
...
setup_secret_manager() {
    ...
    echo "  - Bitbank APIキー (bitbank-api-key)"
    echo "  - Bitbank APIシークレット (bitbank-api-secret)"
    echo "  - Discord WebhookURL (discord-webhook-url)"
    echo ""
...
print_setup_summary() {
    ...
    if [ "$FAILED_STEPS" -eq 0 ]; then
        echo -e "${GREEN}🎉 GCP環境セットアップが完了しました！${NC}"
...
print_usage() {
    ...
    echo ""
    echo "オプション:"
    echo "  --full         完全セットアップ（推奨）"
    echo "  --apis-only    API・サービス設定のみ"
    echo "  --secrets-only Secret Manager設定のみ"
    echo "  --verify       設定検証のみ"
    echo "  --help         このヘルプを表示"
    echo ""
    echo "例:"
    echo "  $0 --full         # 初回セットアップ（推奨）"
    echo "  $0 --apis-only    # API有効化のみ"
    echo "  $0 --secrets-only # 認証情報設定のみ"
    echo "  $0 --verify       # 設定確認のみ"
}
...
main() {
    ...
    log "INFO" "GCP環境セットアップ開始 (モード: $mode)"
    ...
    case "$mode" in
        ...
        "--verify")
            ...
            ;;
        "--apis-only")
            log "INFO" "API・サービス設定モード実行中"
            check_prerequisites
            enable_required_apis
            setup_artifact_registry
            ;;
        "--secrets-only")
            log "INFO" "Secret Manager設定モード実行中"
            check_prerequisites
            setup_secret_manager
            ;;
        "--full"|*)
            log "INFO" "完全セットアップモード実行中"
            check_prerequisites
            enable_required_apis
            setup_artifact_registry
            setup_github_service_account
