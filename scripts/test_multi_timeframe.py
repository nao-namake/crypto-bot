...
class MultiTimeframeTester:
    ...
    def test_timeframe_conversion(self, hourly_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            conversion_results["15m"] = {
                ...
                "actual_ratio": len(data_15m) / len(hourly_data) if data_15m is not None and len(hourly_data) > 0 else 0,
            ...
            conversion_results["4h"] = {
                ...
                "actual_ratio": len(data_4h) / len(hourly_data) if data_4h is not None and len(hourly_data) > 0 else 0,
            ...
            for timeframe, result in conversion_results.items():
                if result["success"]:
                    logger.info(f"    ✅ {timeframe}: {result['output_size']} samples (ratio: {result['actual_ratio']:.2f})")
                else:
    ...
    def test_data_synchronization(self, hourly_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            synchronized_data = synchronizer.synchronize_multi_timeframe_data(timeframe_data)
            ...
            if synchronized_data:
                ...
                for timeframe, data in synchronized_data.items():
                    ...
                    logger.info(f"    ✅ {timeframe}: {len(data)} synchronized timestamps")
                
                # データ品質チェック（簡易版）
                total_records = sum(len(data) for data in synchronized_data.values())
                avg_records = total_records / len(synchronized_data) if synchronized_data else 0
                sync_results["quality_score"] = min(avg_records / 100, 1.0)  # 簡易スコア
                logger.info(f"    📊 Avg synchronized records per timeframe: {avg_records:.1f}")
            ...
            for timeframe in timeframes:
                ...
                if data is not None and not data.empty:
                    quality_metrics = {
                        ...
                        "nan_percentage": (data.isna().sum().sum() / (len(data) * len(data.columns))) * 100,
                    ...
                    logger.info(f"    ✅ {timeframe}: {quality_metrics['total_records']} records, "
        ...
        try:
            ...
            perf_results = {
                ...
                "memory_usage": {
                    ...
                    "increase_mb": final_memory - initial_memory if memory_available else 0,
                    "monitoring_available": memory_available
                },
                "throughput": {
                    "records_per_second": len(hourly_data) / total_time if total_time > 0 else 0
                }
            }
            ...
            logger.info(f"  📊 Throughput: {perf_results['throughput']['records_per_second']:.1f} records/s")
    ...
    def run_comprehensive_test(self) -> Dict:
        ...
        try:
            ...
            # Step 2: タイムフレーム変換テスト
            self.test_results["timeframe_tests"] = self.test_timeframe_conversion(test_data)
            
            # Step 3: データ同期テスト
            self.test_results["synchronization_tests"] = self.test_data_synchronization(test_data)
            ...
            failed_tests = sum(1 for test_result in [
            ...
            if errors == 0 and failed_tests == 0:
                ...
                logger.error(f"❌ Multi-timeframe tests FAILED! Errors: {errors}, Failed tests: {failed_tests}")
