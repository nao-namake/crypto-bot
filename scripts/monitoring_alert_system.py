...
class NotificationService:
    ...
    def _format_alert_email(self, alert: Alert) -> str:
        """アラートメール フォーマット"""
        lines = []
        lines.append("アンサンブル学習システム アラート通知")
        lines.append("=" * 50)
...
class PerformanceMonitor:
    ...
    def analyze_metrics(self, metrics: MonitoringMetrics) -> List[Alert]:
        ...
        if (
            ...
            and metrics.model_agreement < self.config.min_model_agreement
        ):
            alert = self.alert_manager.create_alert(
...
class MonitoringDashboard:
    ...
    def __init__(
        self, alert_manager: AlertManager, performance_monitor: PerformanceMonitor
    ):
        self.alert_manager = alert_manager
    ...
    def _calculate_trend_data(
        self, recent_metrics: List[MonitoringMetrics]
    ) -> Dict[str, Any]:
        """トレンドデータ計算"""
        ...
        return {
            ...
            "trend_summary": {
                ...
                "win_rate_trend": (
                    np.polyfit(range(len(win_rates)), win_rates, 1)[0]
                    if len(win_rates) > 1
                    else 0
                ),
            },
...
class MonitoringAlertSystem:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.performance_monitor = PerformanceMonitor(
            self.monitoring_config, self.alert_manager
        )
