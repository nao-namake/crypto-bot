...
class FeatureIntegrityTester:
    ...
    def _load_full_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"📋 Full production config loaded: {len(config.get('ml', {}).get('extra_features', []))} features"
            )
            return config
    ...
    def _prepare_comprehensive_test_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                logger.info(
                    f"✅ Comprehensive test data prepared: {len(test_df)} rows × {len(test_df.columns)} columns"
                )
                return test_df
            else:
                # モックデータ生成（大規模）
    ...
    def _generate_comprehensive_mock_data(self, n_rows: int = 500) -> pd.DataFrame:
        ...
        logger.info(
            f"📊 Comprehensive mock data generated: {len(df)} rows with realistic price dynamics"
        )
        return df
    ...
    def test_151_feature_complete_generation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result_df is not None:
                generated_features = set(result_df.columns) - set(
                ...
                feature_test_results = {
                    ...
                    "feature_coverage_percent": (
                        len(generated_features & self.expected_features)
                    ...
                    * 100,
                    ...
                    "category_breakdown": {
                        ...
                        "other_features": total_features
                        - len(technical_features)
                        - len(external_features)
                        - len(basic_features),
                    },
                    "success": total_features >= 151
                    and len(missing_features) <= 5,  # 許容誤差5特徴量
                }

                self.results["feature_integrity"][
                    "complete_generation"
                ] = feature_test_results
                ...
                logger.info(
                    f"  • Success: {'✅' if feature_test_results['success'] else '❌'}"
                )

                return feature_test_results
            else:
                error_result = {
                    "error": "Feature generation returned None",
                    "success": False,
                }
    ...
    def test_batch_vs_legacy_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if batch_result is not None and legacy_result is not None:
                ...
                legacy_features = set(legacy_result.columns) - set(
                    self.test_data.columns
                )
                ...
                for feature in common_features:
                    if (
                        feature in batch_result.columns
                        and feature in legacy_result.columns
                    ):
                        ...
                        if len(batch_series) > 0 and len(legacy_series) > 0:
                            ...
                            if correlation < 0.99:
                                mean_diff = abs(
                                    batch_series.mean() - legacy_series.mean()
                                )
                ...
                avg_correlation = (
                    np.mean(list(feature_correlations.values()))
                ...
                consistency_results = {
                    ...
                    "high_consistency_features": len(
                        [c for c in feature_correlations.values() if c >= 0.99]
                    ),
                    "success": len(consistency_issues) <= 5
                    and avg_correlation >= 0.95,  # 許容基準
                }
                ...
                logger.info(
                    f"  • High Consistency: {consistency_results['high_consistency_features']}"
                )
                logger.info(
                    f"  • Success: {'✅' if consistency_results['success'] else '❌'}"
                )

                return consistency_results
            else:
                error_result = {
                    "error": "One or both systems failed to generate features",
                    "success": False,
                }
    ...
    def test_error_handling_robustness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                error_scenarios.append(
                    {"scenario": "empty_dataframe", "success": False, "error": str(e)}
                )
                ...
                error_scenarios.append(
                    {
                        ...
                        "success": result is not None
                        and not result.isnull().all().all(),
                        "error": None,
                    }
                )
            except Exception as e:
                error_scenarios.append(
                    {"scenario": "heavy_nan_data", "success": False, "error": str(e)}
                )
            ...
            error_handling_results = {
                ...
                "success_rate": (
                    len(successful_scenarios) / len(error_scenarios)
                    if error_scenarios
                    else 0
                ),
                ...
                "success": len(successful_scenarios)
                >= len(error_scenarios) * 0.75,  # 75%成功率期待
            }
            ...
            logger.info(
                f"  • Success Rate: {error_handling_results['success_rate']:.2%}"
            )
            logger.info(
                f"  • Success: {'✅' if error_handling_results['success'] else '❌'}"
            )
    ...
    def test_external_data_quality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 外部データ統合テスト
            external_batches = external_integrator.create_external_data_batches(
                ...
                if len(batch) > 0:
                    ...
                    for feature_name, feature_series in batch.data.items():
                        if isinstance(feature_series, pd.Series):
                            nan_rate = feature_series.isnull().sum() / len(
                                feature_series
                            )
                    ...
                    source_quality[source_name] = {
                        ...
                        "high_quality_features": len(
                            [r for r in nan_rates.values() if r <= 0.1]
                        ),
                        "poor_quality_features": len(
                            [r for r in nan_rates.values() if r > 0.5]
                        ),
            ...
            external_data_results = {
                ...
                "successful_sources": len(
                    [s for s in source_quality.values() if s["success"]]
                ),
                "source_quality_detail": source_quality,
                "integration_stats": integration_stats,
                "overall_quality_score": (
                    np.mean([s["quality_score"] for s in source_quality.values()])
                    if source_quality
                    else 0
                ),
                "success": len([s for s in source_quality.values() if s["success"]])
                >= len(source_quality) * 0.5,  # 50%のソース成功期待
            }

            self.results["external_data_quality"][
                "integration_test"
            ] = external_data_results
            ...
            logger.info(
                f"  • Successful Sources: {external_data_results['successful_sources']}"
            )
            logger.info(
                f"  • Overall Quality Score: {external_data_results['overall_quality_score']:.3f}"
            )
            logger.info(
                f"  • Success: {'✅' if external_data_results['success'] else '❌'}"
            )
        ...
        try:
                ...
                edge_cases.append(
                    {
                        ...
                        "features_generated": (
                            len(result.columns) - len(tiny_df.columns)
            ...
            try:
                ...
                extreme_returns = np.random.normal(
                    0, 5, len(extreme_vol_df)
                )  # 500%年率ボラティリティ
                extreme_vol_df["close"] = extreme_vol_df["close"].iloc[0] * np.exp(
                    extreme_returns.cumsum()
                )
                ...
                # 無限大・NaN値チェック
                has_inf = (
                    np.isinf(result.select_dtypes(include=[np.number])).any().any()
                    if result is not None
                    else True
                )
                has_extreme_nan = (
                    result.isnull().sum().sum() > len(result) * 0.8
                    if result is not None
                    else True
                )
                ...
                edge_cases.append(
                    {
                        ...
                        "success": result is not None
                        and not has_inf
                        and not has_extreme_nan,
                        "features_generated": (
                            len(result.columns) - len(extreme_vol_df.columns)
                            if result is not None
                            else 0
                        ),
                        "has_infinity": has_inf,
                        "extreme_nan_rate": (
                            result.isnull().sum().sum() / result.size
                            if result is not None
                            else 1
                        ),
                        "error": None,
                ...
                edge_cases.append(
                    {
                        ...
                        "success": result is not None
                        and len(result.columns) > len(flat_df.columns),
                        "features_generated": (
                            len(result.columns) - len(flat_df.columns)
                            if result is not None
                            else 0
                        ),
                        "error": None,
                ...
                edge_cases.append(
                    {
                        ...
                        "success": result is not None,
                        "features_generated": (
                            len(result.columns) - len(self.test_data.columns)
                            if result is not None
                            else 0
                        ),
                        "error": None,
            ...
            edge_case_results = {
                ...
                "success_rate": (
                    len(successful_cases) / len(edge_cases) if edge_cases else 0
                ),
                "cases_detail": edge_cases,
                "avg_features_generated": (
                    np.mean([c.get("features_generated", 0) for c in successful_cases])
                    if successful_cases
                    else 0
                ),
                "robustness_score": (
                ...
                "success": len(successful_cases)
                >= len(edge_cases) * 0.75,  # 75%成功率期待
            }
            ...
            logger.info(
                f"  • Avg Features Generated: {edge_case_results['avg_features_generated']:.1f}"
            )
            logger.info(
                f"  • Success: {'✅' if edge_case_results['success'] else '❌'}"
            )
    ...
    def calculate_overall_assessment(self):
        ...
        overall_assessment = {
            ...
            "total_tests_run": sum(
                len(tests) for tests in assessments.values() if tests
            ),
            "total_successful_tests": sum(
                ...
                for tests in assessments.values()
                if tests
            ),
        ...
        logger.info(
            f"  • Production Ready: {'✅' if overall_assessment['ready_for_production'] else '❌'}"
        )
    ...
    def save_results(
        self,
        output_path: str = "/Users/nao/Desktop/bot/test_results/feature_integrity_quality_test.json",
    ):
        """結果保存"""
