...
def clean_btc_historical_data():
    ...
    backup_path = Path(f"data/btc_usd_2024_hourly_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
    ...
    extreme_anomaly_mask = (
        (df['open'] > 1e80) | (df['high'] > 1e80) | 
        (df['low'] > 1e80) | (df['close'] > 1e80)
    )
    ...
    if volume_anomaly.sum() > 0:
        ...
        df['volume'] = df['volume'].fillna(df['volume'].median())  # 残りは中央値で埋める
    ...
    return {
        ...
        "price_range": {col: {"min": float(df[col].min()), "max": float(df[col].max())} for col in price_columns},
        "volume_range": {"min": float(df['volume'].min()), "max": float(df['volume'].max())}
    }
