...
class PerformanceComparisonSystem:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.output_dir = Path(
            self.config.get(
                "output_dir", project_root / "results" / "performance_comparison"
            )
        )
    ...
    def load_trading_results(self, traditional_file: str, ensemble_file: str):
        ...
        try:
            ...
            if os.path.exists(traditional_file):
                self.traditional_data = self._load_results_file(
                    traditional_file, "traditional"
                )
                logger.info(
                    f"Traditional results loaded: {len(self.traditional_data)} records"
                )
            ...
            if os.path.exists(ensemble_file):
                ...
                logger.info(
                    f"Ensemble results loaded: {len(self.ensemble_data)} records"
                )
    ...
    def _load_results_file(
        self, file_path: str, strategy_type: str
    ...
    def _generate_sample_metrics(
        self, strategy_type: str, count: int = 100
    ) -> List[PerformanceMetrics]:
        """サンプルメトリクス生成"""
        ...
        for i in range(count):
            ...
            metrics = PerformanceMetrics(
                ...
                confidence_accuracy_correlation=(
                    np.random.uniform(0.3, 0.7) if strategy_type == "ensemble" else None
                ),
                ensemble_diversity=(
                    np.random.uniform(0.4, 0.8) if strategy_type == "ensemble" else None
                ),
                sample_size=1,
            )
    ...
    def _run_basic_comparison(self) -> Dict[str, ComparisonResult]:
        ...
        for metric in primary_metrics:
            ...
            improvement_pct = (
                (improvement / abs(trad_mean)) * 100 if trad_mean != 0 else 0
            )
            ...
            pooled_std = np.sqrt(
                (
                    (len(traditional_values) - 1) * np.var(traditional_values, ddof=1)
            ...
            se_diff = pooled_std * np.sqrt(
                1 / len(traditional_values) + 1 / len(ensemble_values)
            )
            ...
            # 解釈
            interpretation = self._interpret_improvement(
                improvement_pct, effect_size, p_value
            )
    ...
    def _run_statistical_tests(self) -> Dict[str, StatisticalTest]:
        ...
        for metric in primary_metrics:
            ...
            t_stat, p_value = stats.ttest_ind(
                ensemble_values, traditional_values, equal_var=False
            )

            # Mann-Whitney U test (ノンパラメトリック)
            u_stat, u_p_value = stats.mannwhitneyu(
                ensemble_values, traditional_values, alternative="two-sided"
            )
            ...
            # 検定結果解釈
            is_significant = (
                p_value < self.config["comparison"]["significance_threshold"]
            )
            ...
            tests[f"{metric}_mannwhitney"] = StatisticalTest(
                ...
                is_significant=u_p_value
                < self.config["comparison"]["significance_threshold"],
            )
    ...
    def _run_time_series_analysis(self) -> Dict[str, Any]:
        ...
        # リターン推移比較
        analysis["return_trend"] = self._analyze_return_trend(
            traditional_ts, ensemble_ts
        )

        # ドローダウン分析
        analysis["drawdown_analysis"] = self._analyze_drawdown_patterns(
            traditional_ts, ensemble_ts
        )

        # 勝率の安定性
        analysis["winrate_stability"] = self._analyze_winrate_stability(
            traditional_ts, ensemble_ts
        )
    ...
    def _run_ml_specific_analysis(self) -> Dict[str, Any]:
        ...
        if any(m.avg_confidence for m in self.ensemble_data if m.avg_confidence):
            analysis["confidence_accuracy_correlation"] = (
                self._analyze_confidence_accuracy()
            )

        # アンサンブル多様性効果
        if any(
            m.ensemble_diversity for m in self.ensemble_data if m.ensemble_diversity
        ):
    ...
    def _calculate_cohens_d(self, group1: List[float], group2: List[float]) -> float:
        ...
        return (
            (np.mean(group1) - np.mean(group2)) / pooled_std if pooled_std != 0 else 0
        )
    ...
    def _interpret_improvement(
        self, improvement_pct: float, effect_size: float, p_value: float
    ) -> str:
        """改善効果解釈"""
    ...
    def _interpret_statistical_test(
        ...
        if is_significant:
            if effect_size > 0.8:
                ...
                return (
                    f"Moderate evidence of improvement (p={p_value:.4f}, medium effect)"
                )
    ...
    def _analyze_return_trend(
        self, traditional_ts: pd.DataFrame, ensemble_ts: pd.DataFrame
    ) -> Dict:
        """リターン推移分析"""
        return {
            "traditional_trend": np.polyfit(
                range(len(traditional_ts)), traditional_ts["total_return"], 1
            )[0],
            "ensemble_trend": np.polyfit(
                range(len(ensemble_ts)), ensemble_ts["total_return"], 1
            )[0],
            "trend_improvement": np.polyfit(
                range(len(ensemble_ts)), ensemble_ts["total_return"], 1
            )[0]
            - np.polyfit(range(len(traditional_ts)), traditional_ts["total_return"], 1)[
                0
            ],
        }
    ...
    def _analyze_drawdown_patterns(
        ...
        return {
            ...
            "drawdown_improvement": ensemble_ts["max_drawdown"].mean()
            - traditional_ts["max_drawdown"].mean(),
            "drawdown_volatility_reduction": traditional_ts["max_drawdown"].std()
            - ensemble_ts["max_drawdown"].std(),
        }

    def _analyze_winrate_stability(
        self, traditional_ts: pd.DataFrame, ensemble_ts: pd.DataFrame
    ) -> Dict:
        """勝率安定性分析"""
        return {
            ...
            "stability_improvement": traditional_ts["win_rate"].std()
            - ensemble_ts["win_rate"].std(),
        }

    def _analyze_performance_persistence(
        self, traditional_ts: pd.DataFrame, ensemble_ts: pd.DataFrame
    ) -> Dict:
        """パフォーマンス持続性分析"""
        # 移動平均ベースの分析
    ...
    def _analyze_prediction_accuracy(self) -> Dict:
        ...
        return {
            ...
            "accuracy_consistency_improvement": np.std(trad_accuracy)
            - np.std(ens_accuracy),
        }
    ...
    def _analyze_confidence_accuracy(self) -> Dict:
        """信頼度と精度の相関分析"""
        ensemble_confidence = [
            m.avg_confidence for m in self.ensemble_data if m.avg_confidence
        ]
        ensemble_accuracy = [
            m.prediction_accuracy for m in self.ensemble_data if m.avg_confidence
        ]
    ...
    def _analyze_ensemble_diversity(self) -> Dict:
        ...
        accuracy_scores = [
            m.prediction_accuracy for m in self.ensemble_data if m.ensemble_diversity
        ]
        ...
        if len(diversity_scores) > 10:
            ...
            return {
                ...
                "optimal_diversity_range": (
                    np.percentile(diversity_scores, 25),
                    np.percentile(diversity_scores, 75),
                ),
            }
    ...
    def _detect_overfitting_signs(self) -> Dict:
        ...
        return {
            ...
            "overfitting_risk": (
                "High"
                if accuracy_return_correlation < 0.3
                else "Medium" if accuracy_return_correlation < 0.6 else "Low"
            ),
            "recommendation": (
                "Monitor for overfitting"
                if accuracy_return_correlation < 0.5
                else "Healthy correlation"
            ),
        }
    ...
    def _calculate_overall_score(self) -> Dict:
        ...
        for metric, weight in weights.items():
            if metric in self.comparison_results.get("basic_comparison", {}):
                improvement = self.comparison_results["basic_comparison"][
                    metric
                ].improvement_pct
        ...
        return {
            ...
            "score_interpretation": (
                ...
                if total_score > 10
                    ...
                    "Good"
                    if total_score > 5
                    else "Moderate" if total_score > 0 else "Poor"
                )
            ),
        }
    ...
    def _calculate_risk_adjusted_metrics(self) -> Dict:
        ...
        return {
            ...
            "risk_adjusted_superiority": (np.mean(ens_sharpe) - np.mean(trad_sharpe))
            > 0.1,
        }
    ...
    def _evaluate_practical_significance(self) -> Dict:
        ...
        return {
            ...
            "deployment_recommendation": (
                "Recommended"
                if sum(practical_improvements.values()) >= 3
                else (
                    "Consider"
                    if sum(practical_improvements.values()) >= 2
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if (
            hasattr(self, "comparison_results")
            and "basic_comparison" in self.comparison_results
        ):
            basic_results = self.comparison_results["basic_comparison"]

            # リターン改善
            if (
                "total_return" in basic_results
                and basic_results["total_return"].improvement_pct > 5
            ):
                recommendations.append(
                    "Strong return improvement observed - proceed with deployment"
                )

            # リスク削減
            if (
                "max_drawdown" in basic_results
                and basic_results["max_drawdown"].improvement < 0
            ):
                recommendations.append(
                    "Reduced drawdown risk - improves portfolio stability"
                )

            # 勝率向上
            if (
                "win_rate" in basic_results
                and basic_results["win_rate"].improvement_pct > 3
            ):
                recommendations.append("Improved win rate supports trading psychology")

            # シャープレシオ
            if (
                "sharpe_ratio" in basic_results
                and basic_results["sharpe_ratio"].improvement_pct > 10
            ):
                recommendations.append("Excellent risk-adjusted return improvement")

        if not recommendations:
            recommendations.append(
                "Monitor performance over longer period for conclusive results"
            )
    ...
    def _get_comparison_period(self) -> Dict:
        ...
        if self.traditional_data and self.ensemble_data:
            return {
                "start_date": min(
                    m.period_start for m in self.traditional_data + self.ensemble_data
                ).isoformat(),
                "end_date": max(
                    m.period_end for m in self.traditional_data + self.ensemble_data
                ).isoformat(),
                "total_days": (
                    max(
                        m.period_end for m in self.traditional_data + self.ensemble_data
                    ...
                    - min(
                        m.period_start
                        for m in self.traditional_data + self.ensemble_data
                    )
                ).days,
            }
        return {}
    ...
    def generate_detailed_report(self) -> str:
        ...
        report_lines = []
        report_lines.append(
            "🔍 アンサンブル学習 vs 従来手法 - 詳細パフォーマンス比較レポート"
        )
        ...
        report_lines.append(
            f"  アンサンブルサンプル数: {metadata.get('ensemble_sample_size', 0)}"
        )
        ...
        if "overall_assessment" in self.comparison_results:
            ...
            if "overall_score" in assessment:
                ...
                report_lines.append(
                    f"  総合改善スコア: {score_info.get('weighted_improvement_score', 0):.2f}"
                )
                report_lines.append(
                    f"  評価: {score_info.get('score_interpretation', 'N/A')}"
                )

            if "practical_significance" in assessment:
                practical = assessment["practical_significance"]
                report_lines.append(
                    f"  実用性評価: {practical.get('deployment_recommendation', 'N/A')}"
                )
        ...
        if "ml_analysis" in self.comparison_results:
            ...
            if "prediction_accuracy" in ml_analysis:
                ...
                report_lines.append(
                    f"  予測精度改善: {acc.get('accuracy_improvement', 0):.3f}"
                )
                report_lines.append(
