...
class SimpleABTest(BaseAnalyzer):
    ...
    def collect_variant_data(
        ...
        try:
            ...
            success, logs = self.fetch_trading_logs(hours=hours, service_suffix=service_suffix, limit=300)
    ...
    def generate_recommendation(
        ...
        freq_improvement = ((variant_b.signal_frequency - variant_a.signal_frequency) / 
                           variant_a.signal_frequency * 100) if variant_a.signal_frequency > 0 else 0
        ...
        if is_statistically_significant and is_practically_significant:
            if freq_improvement > 0:
                ...
                winner = "A"
                recommendation = f"バリアントAを維持推奨（シグナル頻度{abs(freq_improvement):.1f}%良好）"
    ...
    def run_ab_test(
        ...
        # 改善率計算
        improvement_percentage = ((variant_b.signal_frequency - variant_a.signal_frequency) / 
                                 variant_a.signal_frequency * 100) if variant_a.signal_frequency > 0 else 0
    ...
    def generate_report(self, result: ABTestResult) -> str:
        ...
        report.append(f"  バリアントA: {result.variant_a.total_signals}シグナル ({result.variant_a.signal_frequency:.2f}/時間)")
        report.append(f"  バリアントB: {result.variant_b.total_signals}シグナル ({result.variant_b.signal_frequency:.2f}/時間)")
...
def main():
    ...
    parser.add_argument("--hours", type=int, default=24,
                       help="テスト期間（時間）")
    parser.add_argument("--variant-a", default="",
                       help="バリアントAサービスサフィックス")
    parser.add_argument("--variant-b", default="-stage10",
                       help="バリアントBサービスサフィックス")
    parser.add_argument("--project", default="my-crypto-bot-project",
                       help="GCPプロジェクトID")
    parser.add_argument("--service", default="crypto-bot-service",
                       help="ベースサービス名")
    parser.add_argument("--output", default="logs/ab_testing",
                       help="出力ディレクトリ")
