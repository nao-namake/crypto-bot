...
class MLIntegrationSystemTester:
    ...
    def _prepare_integration_test_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                logger.info(
                    f"✅ Integration test data prepared: {len(test_df)} rows × {len(test_df.columns)} columns"
                )
                return test_df
            else:
                # モックデータ生成
    ...
    def test_ml_pipeline_complete_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(non_numeric_columns) > 0:
                logger.info(
                    f"🧹 Excluding non-numeric columns: {list(non_numeric_columns)}"
                )
            ...
            logger.info(
                f"✅ Train: {len(X_train)} rows, Test: {len(X_test)} rows, Features: {len(numeric_columns)}"
            )
            ...
            base_estimator = create_model(
                "lgbm", n_estimators=10, max_depth=3, random_state=42
            )
            ...
            training_success = (
                True  # fit()はMLModelオブジェクトを返すため、成功とみなす
            )
            ...
            precision = (
                ((predictions == 1) & (y_test == 1)).sum() / (predictions == 1).sum()
                if (predictions == 1).sum() > 0
                else 0
            )
            recall = (
                ((predictions == 1) & (y_test == 1)).sum() / (y_test == 1).sum()
                if (y_test == 1).sum() > 0
                else 0
            )
            ...
            ml_pipeline_results = {
                ...
                "success": (
                    feature_count >= 50
                    and accuracy > 0.4  # 最低限の予測性能
                    and total_time < 120  # 2分以内完了
                ),
            }

            self.results["ml_pipeline_integration"][
                "complete_pipeline"
            ] = ml_pipeline_results
            ...
            logger.info(
                f"  • Success: {'✅' if ml_pipeline_results['success'] else '❌'}"
            )
    ...
    def test_backtest_system_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_config_path = os.path.join(
                self.results_dir, "integration_backtest_config.yml"
            )
            backtest_config = self.config.copy()
            backtest_config.update(
                {
                    "mode": "backtest",
                    "data": {"source": "csv", "csv_path": test_csv_path},
                    "backtest": {
            ...
            with open(test_config_path, "w", encoding="utf-8") as f:
                yaml.dump(
                    backtest_config, f, default_flow_style=False, allow_unicode=True
                )
            ...
            data_loading_success = (
                os.path.exists(test_csv_path) and os.path.getsize(test_csv_path) > 0
            )
            ...
            feature_pipeline_success = (
                features_result is not None and not features_result.empty
            )
            ...
            backtest_integration_results = {
                ...
                "features_generated": (
                    len(features_result.columns) if features_result is not None else 0
                ),
                "backtest_components": {
                ...
                "performance_metrics": {
                    "total_time_seconds": total_time,
                    "data_rows_per_second": (
                        len(test_data_for_backtest) / total_time
                        if total_time > 0
                        else 0
                    ),
                },
                ...
                "success": (
                    data_loading_success
                    and config_loading_success
                    and config_valid
                    and feature_pipeline_success
                ),
            }

            self.results["backtest_system_integration"][
                "components_test"
            ] = backtest_integration_results
            ...
            logger.info(
                f"  • Config Loading: {'✅' if (config_loading_success and config_valid) else '❌'}"
            )
            logger.info(
                f"  • Feature Pipeline: {'✅' if feature_pipeline_success else '❌'}"
            )
            logger.info(f"  • Total Time: {total_time:.1f}s")
            logger.info(
                f"  • Success: {'✅' if backtest_integration_results['success'] else '❌'}"
            )
    ...
    def test_strategy_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if MLStrategy is not None:
                ...
                sample_features = sample_data[["close", "volume"]].fillna(
                    0
                )  # 最小限の特徴量
                sample_target = (
                    (sample_data["close"].pct_change() > 0).astype(int).fillna(0)
                )
                ...
                test_config["strategy"]["params"] = (
                    test_config["strategy"].get("params", {}).copy()
                )
                ...
                try:
                    ...
                    if hasattr(ml_strategy, "generate_signal") or hasattr(
                        ml_strategy, "get_signal"
                    ):
                        signal_method = getattr(
                            ml_strategy, "generate_signal", None
                        ) or getattr(ml_strategy, "get_signal", None)
                        signal = signal_method(test_sample)
                        ml_strategy_success = signal is not None
                        signal_type = type(signal).__name__
                    else:
                        # フォールバック: 基本属性チェック
                        ml_strategy_success = hasattr(ml_strategy, "model") or hasattr(
                            ml_strategy, "feature_engineer"
                        )
            ...
            try:
                ...
                strategy_results["feature_engineer_strategy"] = {
                    "initialization_success": True,
                    "feature_generation_success": features_result is not None
                    and not features_result.empty,
                    "features_count": (
                        len(features_result.columns)
                        if features_result is not None
                        else 0
                    ),
                    "batch_engines_enabled": getattr(
                        fe_strategy, "batch_engines_enabled", False
                    ),
            ...
            # 総合評価
            overall_success = any(
                result.get("success", False) for result in strategy_results.values()
            )
            ...
            self.results["strategy_integration"][
                "comprehensive_test"
            ] = strategy_integration_results
            ...
            for strategy_name, result in strategy_results.items():
                logger.info(
                    f"  • {strategy_name}: {'✅' if result.get('success', False) else '❌'}"
                )
        ...
        try:
            ...
            performance_metrics = {
                "large_dataset_processing": {
                    ...
                    "rows_per_second": (
                        len(large_dataset) / feature_time if feature_time > 0 else 0
                    ),
                    "features_per_second": (
                        (
                            len(features_result.columns)
                ...
                "performance_benchmarks": {
                    ...
                    "rows_per_second_achieved": (
                        len(large_dataset) / feature_time >= 100
                        if feature_time > 0
                        else False
                    ),
            ...
            speed_improvement = (
                ((legacy_time - batch_time) / legacy_time * 100)
                if legacy_time > 0
                else 0
            )
            ...
            performance_metrics["batch_vs_legacy_performance"] = {
                ...
                "batch_features": (
                    len(batch_result.columns) if batch_result is not None else 0
                ),
                "legacy_features": (
                    len(legacy_result.columns) if legacy_result is not None else 0
                ),
            ...
            # Overall Success
            overall_success = (
                performance_metrics["large_dataset_processing"]["success"]
                and performance_metrics["performance_benchmarks"][
                    "rows_per_second_achieved"
                ]
                and performance_metrics["performance_benchmarks"][
            ...
            performance_pipeline_results = {
                ...
                "overall_success": overall_success,
                "summary": {
                    "large_dataset_success": performance_metrics[
                        "large_dataset_processing"
                    ]["success"],
                    "performance_benchmarks_met": all(
                        [
                            performance_metrics["performance_benchmarks"][
                                "rows_per_second_achieved"
                            ],
                            performance_metrics["performance_benchmarks"][
                                "memory_usage_acceptable"
                    ...
                    "batch_improvement_achieved": performance_metrics[
                        "batch_vs_legacy_performance"
                    ]["performance_target_achieved"],
                },
            }

            self.results["performance_pipeline"][
                "comprehensive_test"
            ] = performance_pipeline_results

            logger.info(f"🚀 Performance Pipeline Results:")
            logger.info(
                f"  • Large Dataset: {len(large_dataset)} rows in {feature_time:.1f}s"
            )
            logger.info(
                f"  • Rows/sec: {performance_metrics['large_dataset_processing']['rows_per_second']:.1f}"
            )
    ...
    def test_memory_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_management_results = {
                ...
                "success": (
                    not memory_efficiency["memory_leak_detected"]
                    and memory_efficiency["memory_usage_acceptable"]
                    and memory_efficiency["memory_increase_acceptable"]
                ),
            }
            ...
            logger.info(
                f"  • Memory Leak: {'❌ Detected' if memory_efficiency['memory_leak_detected'] else '✅ None'}"
            )
            logger.info(
                f"  • Success: {'✅' if memory_management_results['success'] else '❌'}"
            )
    ...
    def calculate_overall_assessment(self):
        ...
        assessments = {
            ...
            "backtest_system_integration": self.results.get(
                "backtest_system_integration", {}
            ),
        ...
        overall_assessment = {
            ...
            "critical_systems": {
                ...
                "backtest_system": category_scores.get("backtest_system_integration", 0)
                >= 0.8,
            ...
            "production_readiness": overall_score >= 0.75
            and all(
                [
        ...
        logger.info(
            f"  • Production Ready: {'✅' if overall_assessment['production_readiness'] else '❌'}"
        )
    ...
    def save_results(
        self,
        output_path: str = "/Users/nao/Desktop/bot/test_results/ml_integration_system_test.json",
    ):
        """結果保存"""
        ...
        try:
            ...
            logger.info(
                f"💾 ML integration system test results saved to: {output_path}"
            )
        except Exception as e:
...
def main():
    ...
    try:
        ...
        if overall:
            ...
            print(
                f"📊 Integration Level: {overall.get('integration_level', 'Unknown')}"
            )
            print(
                f"🚀 Production Ready: {'✅ YES' if overall.get('production_readiness', False) else '❌ NO'}"
            )
            print(
                f"💡 Recommendation: {overall.get('recommendation', 'No recommendation')}"
            )
            ...
            for system, status in critical.items():
                print(
                    f"  • {system.replace('_', ' ').title()}: {'✅' if status else '❌'}"
                )
        else:
            print(
                "❌ INTEGRATION TEST FAILED: Could not complete comprehensive assessment"
            )
