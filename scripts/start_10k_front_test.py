...
class FrontTestManager:
    ...
    def _setup_logging(self):
        ...
        log_file = log_dir / f"10k_front_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def _verify_external_apis(self) -> bool:
        ...
        try:
            ...
            if not macro_data:
                self.logger.warning("マクロ経済データ取得に失敗（フォールバック値使用）")
            else:
    ...
    def _generate_final_report(self):
        ...
        try:
            ...
            # ファイル保存
            report_file = project_root / f"front_test_final_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def check_prerequisites():
    ...
    # 設定ファイルチェック
    config_path = Path(__file__).parent.parent / 'config' / 'production' / 'bitbank_10k_front_test.yml'
    if not config_path.exists():
