...
class PerformanceTestRunner:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_features = [
                "rsi_14", "rsi_7", "rsi_21", "sma_20", "sma_50", "ema_12", "ema_26",
                "macd", "atr_14", "stoch", "adx"  # 高速なテクニカル指標のみ
            ]
            
            config["ml"]["extra_features"] = test_features
            
            logger.info(f"📋 Configuration loaded for testing: {len(test_features)} features")
    ...
    def _prepare_test_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                logger.info(f"✅ Test data prepared: {len(test_df)} rows × {len(test_df.columns)} columns")
                return test_df
            else:
                # モックデータ生成
    ...
    def test_legacy_system_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            legacy_results = {
                ...
                "output_columns": len(result_df.columns) if result_df is not None else 0,
                "features_generated": len(result_df.columns) - len(self.test_data.columns) if result_df is not None else 0,
                "memory_before_mb": memory_before,
                "memory_peak_mb": max(memory_during),
                "memory_after_mb": memory_after,
                "tracemalloc_current_mb": current / 1024 / 1024,
                "tracemalloc_peak_mb": peak / 1024 / 1024,
                "features_per_second": (len(result_df.columns) - len(self.test_data.columns)) / processing_time if processing_time > 0 and result_df is not None else 0
            }
            ...
            logger.info(f"  • Features Generated: {legacy_results['features_generated']}")
            logger.info(f"  • Memory Peak: {legacy_results['memory_peak_mb']:.1f} MB")
            logger.info(f"  • Features/Second: {legacy_results['features_per_second']:.1f}")
    ...
    def test_batch_system_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"  • Processing Time: {processing_time:.3f}s")
            logger.info(f"  • Features Generated: {batch_results['features_generated']}")
            logger.info(f"  • Memory Peak: {batch_results['memory_peak_mb']:.1f} MB")
            logger.info(f"  • Features/Second: {batch_results['features_per_second']:.1f}")
    ...
    def calculate_performance_comparison(self):
        """性能比較計算"""
        if "error" in self.results["legacy_system"] or "error" in self.results["batch_system"]:
        ...
        logger.info(f"  • Speed Target Achieved: {'✅' if comparison_results['speed_target_achieved'] else '❌'}")
        logger.info(f"  • Memory Target Achieved: {'✅' if comparison_results['memory_target_achieved'] else '❌'}")
        logger.info(f"  • Overall Success: {'✅' if comparison_results['overall_success'] else '❌'}")
    
    def save_results(self, output_path: str = "/Users/nao/Desktop/bot/test_results/performance_comparison.json"):
        """結果保存"""
    ...
    try:
        ...
        if "comparison" in results and "overall_success" in results["comparison"]:
            if results["comparison"]["overall_success"]:
                ...
                print(f"   • Speed improvement: {results['comparison']['speed_improvement_percent']:.1f}%")
                print(f"   • Memory improvement: {results['comparison']['memory_improvement_percent']:.1f}%")
            else:
                print("⚠️  PARTIAL SUCCESS: Some targets not achieved")
                print(f"   • Speed improvement: {results['comparison']['speed_improvement_percent']:.1f}% (Target: 75%)")
                print(f"   • Memory improvement: {results['comparison']['memory_improvement_percent']:.1f}% (Target: 50%)")
        else:
