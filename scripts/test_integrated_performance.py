...
class IntegratedPerformanceTester:
    ...
    def generate_scaled_test_data(self, scale_factor: int = 1) -> pd.DataFrame:
        ...
        logger.info(f"📊 Generating {n_samples} test samples (scale factor: {scale_factor})...")
        ...
        logger.info(f"✅ Generated test data: {len(df)} rows, {df.memory_usage(deep=True).sum() / 1024**2:.1f} MB")
        return df
        ...
        try:
            ...
            perf_metrics.update({
                ...
                "throughput": {
                    ...
                    "features_per_second": (len(features_df.columns) * len(features_df)) / processing_time if processing_time > 0 and features_df is not None else 0
                }
            })
            ...
            logger.info(f"  🔄 Throughput: {perf_metrics['throughput']['samples_per_second']:.1f} samples/s")
        ...
        try:
            ...
            for tf in timeframes:
                ...
                if result is not None:
                    ...
                    logger.info(f"  🔄 {tf} conversion: {conversion_time:.2f}s → {len(result)} samples")
                else:
            ...
            if len(converted_data) > 1:
                ...
                synchronized_data = synchronizer.synchronize_multi_timeframe_data(converted_data)
        ...
        for scale in scale_factors:
            ...
            try:
                ...
                scalability_results[f"scale_{scale}x"] = {
                    ...
                    "total_time": total_time,
                    "efficiency_ratio": len(scaled_data) / total_time if total_time > 0 else 0
                }
                
                logger.info(f"    ✅ Scale {scale}x: {total_time:.2f}s, Efficiency: {scalability_results[f'scale_{scale}x']['efficiency_ratio']:.1f} samples/s")
            ...
            if "feature_performance" in performance_data:
                ...
                if "preprocessing_time" in fp and fp["preprocessing_time"] > 10:
                    bottlenecks["slowest_operations"].append({
                        ...
                        "severity": "high" if fp["preprocessing_time"] > 30 else "medium"
                    })
                ...
                if isinstance(test_data, dict) and "memory_usage" in test_data:
                    ...
                    if memory_increase > 100:  # 100MB以上の増加
                        bottlenecks["memory_intensive_operations"].append({
                            ...
                            "severity": "high" if memory_increase > 500 else "medium"
                        })
            
            # 推奨事項の生成
            if bottlenecks["slowest_operations"]:
                bottlenecks["recommendations"].append("Consider optimizing feature generation algorithms")
            if bottlenecks["memory_intensive_operations"]:
                bottlenecks["recommendations"].append("Implement memory-efficient data processing strategies")
            ...
            if "scalability_tests" in performance_data:
                ...
                if len(scales) >= 2:
                    ...
                    if "efficiency_ratio" in scale_1 and "efficiency_ratio" in scale_2:
                        efficiency_degradation = (scale_1["efficiency_ratio"] - scale_2["efficiency_ratio"]) / scale_1["efficiency_ratio"]
                        
                        if efficiency_degradation > 0.3:  # 30%以上の効率低下
                            bottlenecks["efficiency_concerns"].append({
                                "issue": "Poor scalability",
                                "degradation_percentage": efficiency_degradation * 100,
                                "recommendation": "Review algorithms for O(n²) complexity"
                            })
            
            logger.info(f"  🔍 Found {len(bottlenecks['slowest_operations'])} slow operations")
            logger.info(f"  💾 Found {len(bottlenecks['memory_intensive_operations'])} memory-intensive operations")
    ...
    def run_comprehensive_performance_test(self) -> Dict:
        ...
        try:
            ...
            self.test_results["bottleneck_analysis"] = self.analyze_bottlenecks(all_performance_data)
            
            # 総合判定
            errors = sum(1 for test_result in [
                self.test_results["performance_metrics"].get("feature_performance", {}),
                self.test_results["performance_metrics"].get("multiframe_performance", {}),
...
def main():
    ...
    if "performance_metrics" in results:
        ...
        if "feature_performance" in perf:
            ...
            logger.info(f"Feature Generation: {fp.get('preprocessing_time', 0):.2f}s, {fp.get('feature_count', 0)} features")
