...
class TradingDataCollector(BaseAnalyzer):
    ...
    def calculate_performance_metrics(self, hours: int = 24) -> PerformanceMetrics:
        ...
        self.performance_metrics.signals_per_hour = self.performance_metrics.total_signals / hours if hours > 0 else 0
        ...
        if confidences:
            ...
            self.performance_metrics.high_confidence_signals = len([c for c in confidences if c > 0.7])
    ...
    def export_performance_json(self) -> str:
        ...
        report_data = {
            ...
            "daily_stats_summary": {
                ...
                "avg_signals_per_day": sum(stats.total_signals for stats in self.daily_stats.values()) / len(self.daily_stats) if self.daily_stats else 0
            },
            "data_files": {
    ...
    def generate_summary_report(self) -> str:
        ...
        report.append(f"  高信頼シグナル: {self.performance_metrics.high_confidence_signals}件 (>0.7)")
        report.append("")
...
def main():
    ...
    parser.add_argument("--region", default="asia-northeast1",
                       help="GCPリージョン")
    parser.add_argument("--output", default="logs/data_collection",
                       help="出力ディレクトリ")
