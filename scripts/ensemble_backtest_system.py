...
class EnsembleBacktestSystem:
    ...
    def _load_data(self):
        ...
        try:
            ...
            logger.info(
                f"Data loaded: {len(self.data)} records from "
                f"{self.data.index[0]} to {self.data.index[-1]}"
            )
    ...
    def run_comparative_backtest(self) -> Dict[str, BacktestResult]:
        ...
        for strategy_name, strategy in strategies.items():
            ...
            try:
                ...
                logger.info(
                    f"Completed backtest for {strategy_name}: "
    ...
    def _run_single_backtest(
        self, strategy_name: str, strategy, data: pd.DataFrame
    ) -> BacktestResult:
        """単一戦略バックテスト実行"""
        # バックテスト設定
        initial_balance = self.config.get("backtest", {}).get(
            "initial_balance", 1000000
        )
        ...
        for i in range(50, len(data)):  # 特徴量計算のため50行後から開始
            ...
            try:
                ...
                if len(equity_curve) > 1:
                    daily_return = (equity_curve[-1] - equity_curve[-2]) / equity_curve[
                        -2
                    ]
    ...
    def _calculate_backtest_result(
        ...
        volatility = (
            returns_series.std() * np.sqrt(365 * 24) if len(returns_series) > 1 else 0
        )

        sharpe_ratio = (
            (returns_series.mean() * 365 * 24) / volatility if volatility > 0 else 0
        )
        ...
        # Calmar ratio
        calmar_ratio = (
            (total_return * 100) / abs(max_drawdown * 100) if max_drawdown != 0 else 0
        )

        # Sortino ratio
        negative_returns = returns_series[returns_series < 0]
        downside_deviation = (
            negative_returns.std() * np.sqrt(365 * 24)
            if len(negative_returns) > 1
            else 0
        )
        sortino_ratio = (
            (returns_series.mean() * 365 * 24) / downside_deviation
        ...
        if traditional_result and ensemble_results:
            for ensemble_name, ensemble_result in ensemble_results.items():
                improvement_analysis[ensemble_name] = {
                    "return_improvement": ensemble_result.total_return
                    - traditional_result.total_return,
                    "sharpe_improvement": ensemble_result.sharpe_ratio
                    - traditional_result.sharpe_ratio,
                    "winrate_improvement": ensemble_result.win_rate
                    - traditional_result.win_rate,
                    "drawdown_improvement": ensemble_result.max_drawdown
                    - traditional_result.max_drawdown,
                    "trades_change": ensemble_result.total_trades
                    - traditional_result.total_trades,
                }
    ...
    def save_results(self, output_dir: str = None):
        ...
        if self.comparison_metrics and "comparison_table" in self.comparison_metrics:
            ...
            comparison_df.to_csv(
                output_path / f"strategy_comparison_{timestamp}.csv", index=False
            )
        ...
        for strategy_name, result in self.results.items():
            ...
            result.equity_curve.to_csv(
                output_path / f"equity_curve_{strategy_name}_{timestamp}.csv"
            )
            ...
            if result.trade_details:
                ...
                trades_df.to_csv(
                    output_path / f"trades_{strategy_name}_{timestamp}.csv", index=False
                )

        # レポート保存
        with open(
            output_path / f"comparison_report_{timestamp}.yaml", "w", encoding="utf-8"
        ) as f:
            yaml.dump(
                self.comparison_metrics, f, default_flow_style=False, allow_unicode=True
            )
...
class TraditionalMLStrategyMock:
    ...
    def generate_signal(self, data: pd.DataFrame, position: Position) -> Signal:
        ...
        # エグジット条件
        if position.exist and (
            short_ma.iloc[-1] < long_ma.iloc[-1] or returns.iloc[-1] < -0.02
        ):
...
class EnsembleStrategyMock:
    ...
    def __init__(self, config, method="trading_stacking"):
        ...
        self.confidence_threshold = (
            config.get("ml", {}).get("ensemble", {}).get("confidence_threshold", 0.65)
        )
        self.ensemble_info = {
            "method": method,
...
def main():
    ...
    try:
        ...
        for strategy_name, result in results.items():
            print(
                f"{strategy_name:25} | Return: {result.total_return:8.2%} | "
