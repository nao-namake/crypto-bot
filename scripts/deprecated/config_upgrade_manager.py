...
class ConfigUpgradeManager:
    ...
    def create_backup(self) -> str:
        ...
        try:
            ...
            if self.production_config.exists():
                ...
                logger.warning(
                    f"⚠️ Production config not found: {self.production_config}"
                )
                return ""
    ...
    def run_validation_tests(self) -> Dict:
        ...
        try:
            ...
            result_151 = self._run_test_script(
                "scripts/test_151_features.py", f"--config {self.validation_config}"
            )
            validation_results["feature_test"] = result_151

            # Test 2: マルチタイムフレームテスト
            logger.info("  Testing multi-timeframe processing...")
            result_mtf = self._run_test_script(
                "scripts/test_multi_timeframe.py", f"--config {self.validation_config}"
            )
            validation_results["integration_test"] = result_mtf

            # Test 3: パフォーマンステスト
            logger.info("  Testing performance...")
            result_perf = self._run_test_script(
                "scripts/test_integrated_performance.py",
                f"--config {self.validation_config}",
            )
            validation_results["performance_test"] = result_perf

            # 総合判定
            validation_results["test_status"] = self._evaluate_test_results(
                validation_results
            )
    ...
    def upgrade_to_production(self) -> bool:
        ...
        try:
            if (
                self.upgrade_results["validation_results"].get("test_status")
                != "passed"
            ):
            ...
            with open(self.production_config, "w") as f:
                yaml.dump(
                    adjusted_config, f, default_flow_style=False, allow_unicode=True
                )
        ...
        try:
            ...
            if not backup_filename:
                ...
                if not backup_files:
                    ...
                    return False
                backup_filename = max(
                    backup_files, key=lambda f: f.stat().st_mtime
                ).name
