...
class Feature151Tester:
    ...
    def analyze_features(self, features_df: pd.DataFrame) -> Dict:
        ...
        for feature in feature_names:
            if feature in ["open", "high", "low", "close", "volume"]:
                ...
                x in feature for x in ["autocorrelation", "seasonal", "regime", "cycle"]
            ):
                categories["時系列トレンド"].append(feature)
            elif any(
                x in feature
                for x in ["cross_correlation", "relative_strength", "spread"]
            ):
                categories["クロスアセット"].append(feature)
            elif any(
                x in feature
                for x in ["vix", "dxy", "fear_greed", "fg_", "funding", "oi_"]
            ):
                categories["外部データ"].append(feature)
            elif any(
                x in feature
                for x in ["volatility_regime", "momentum_signals", "liquidity"]
            ):
    ...
    def validate_feature_quality(self, features_df: pd.DataFrame) -> Dict:
        ...
        for col in features_df.columns:
            ...
            # 無限大の割合
            inf_ratio = np.isinf(
                features_df[col].replace([np.inf, -np.inf], np.nan)
            ).sum() / len(features_df)
            if inf_ratio > 0:
    ...
    def run_test(self) -> Dict:
        ...
        try:
            ...
            if actual_features == 151:
                ...
                logger.error(
                    f"❌ Feature count test FAILED! Expected 151, got {actual_features}"
                )
            ...
            if quality_metrics["constant_features"]:
                logger.warning(
                    f"⚠️ Constant features detected: {len(quality_metrics['constant_features'])}"
                )
