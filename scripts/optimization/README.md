# Phase 40: Optuna包括最適化 - 使い方ガイド（Phase 52.4更新版）

**最終更新**: 2025年11月16日 - Phase 52.4コード整理・実験的ステータス明示

---

## ⚠️ **実験的機能 - 成績向上保証なし**

**この最適化システム（Phase 40）は現在、以下の理由により実運用での成績向上は期待できません**:

### 🔴 **重大な制限事項**

1. **❌ シミュレーションのみ（実バックテストなし）**
   - 現在の実装は実際のバックテストを実行していません
   - 「理想的なパラメータに近い値」を選ぶシミュレーションのみ
   - 実際の市場データでの検証なし

2. **❌ 戦略パラメータが古い**
   - `optimize_strategy_parameters.py` は5戦略用（MochipoyAlert, MultiTimeframe等）
   - Phase 52.4システムは6戦略（ATRBased, DonchianChannel, ADXTrendStrength, BBReversal, MACDEMACrossover, StochasticReversal）
   - 実行するとエラーになります

3. **⚠️ Phase 49（2025年10月25日）以降未テスト**
   - 最新システムとの互換性未確認
   - Phase 52.4の6戦略システムに未対応

### 📊 **代替案: 既存バックテストデータ活用**

成績向上を目指す場合、既存のバックテスト結果（`docs/バックテスト記録/`）の統計分析を推奨します:
- 実際の市場データでの検証済み結果
- 戦略別・TP/SL設定別パフォーマンス比較
- Phase 51.6-51.11の戦略選択に直接活用可能

### 🔧 **将来の改善予定**

- Phase 53以降: `backtest_integration.py` の実バックテスト統合完成
- Phase 53以降: 6戦略対応版 `optimize_strategy_parameters.py` 実装
- 実運用3ヶ月データ収集後: 大規模パラメータ最適化再検討

**最終検証**: Phase 49（2025年10月25日）
**現在のシステム**: Phase 52.4（2025年11月16日）

---

## 📋 このディレクトリの役割

**Phase 40最適化スクリプト格納ディレクトリ**（実験的）

Optunaを使用したベイズ最適化により、システム全体のパラメータを包括的に最適化する**設計**です。
（注: 現在は実バックテスト未統合のため、実用性は限定的）

**⚠️ Phase 52.4時点の重要事項**:
- **Phase 42.4対応**: TP/SL距離パラメータはOptuna最適化対象外（FIXED_TP_SL_PARAMS）に固定
- **Phase 46対応**: スイングトレード機能削除（統合TP/SL・トレーリングストップ）により一部パラメータ無効化
- **Phase 49対応**: デイトレード特化・個別TP/SL管理回帰
- **Phase 52.4対応**: 6戦略システム（optimize_strategy_parameters.py は5戦略用で非対応）

**対象パラメータ**: 79パラメータ（設計値）
- Phase 40.1: リスク管理（12パラメータ）
- Phase 40.2: 戦略パラメータ（30パラメータ）← Phase 52.4非対応
- Phase 40.3: ML統合（7パラメータ）
- Phase 40.4: MLハイパーパラメータ（30パラメータ）

**期待効果**（※Phase 40設計時の期待値・実運用での保証なし）:
- シャープレシオ: +50-70%向上（理論値）
- 年間リターン: +50-100%向上（理論値）
- ML信頼度: +15-25%向上（理論値）

**注**: これらは過去の期待値であり、実バックテスト未統合・戦略非対応のため実現性は不明
...
**最終更新**: 2025年11月16日 - Phase 52.4コード整理完了・実験的ステータス明示・成績向上保証なし警告追加
