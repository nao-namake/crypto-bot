...
class OperationalStatusChecker:
    ...
    def __init__(self, config_path: str = None):
        self.config_path = config_path or str(PROJECT_ROOT / "scripts/status_config.json") 
    ...
    def load_config(self) -> Dict:
        ...
        try:
            ...
            return {"check_phases": {}, "thresholds": {}, "hidden_error_patterns": {"patterns": []}}
    ...
    def check_gcp_cloud_run_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                rev_result = subprocess.run(cmd_revisions, capture_output=True, text=True, timeout=20)
                ...
                if rev_result.returncode == 0:
                    ...
                    if revisions:
                        ...
                        creation_time = latest_rev.get("metadata", {}).get("creationTimestamp")
                        if creation_time:
                            created_at = datetime.fromisoformat(creation_time.replace('Z', '+00:00'))
                            revision_age_hours = (datetime.now().astimezone() - created_at).total_seconds() / 3600
    ...
    def check_api_connectivity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "checks": checks,
            "details": f"Response time: {checks['response_time']:.2f}s" if checks['response_time'] and checks['response_time'] > 0 else "Response time unavailable"
        }
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if json_output:
                    ...
                    status = "healthy" if success_rate >= 80 and critical_issues == 0 else "degraded"
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for log in logs[:3]:
                    ...
                    # メモリ使用量パターン検索  
                    memory_match = re.search(r'memory.*?(\d+).*?MB', text, re.IGNORECASE)
                    if memory_match:
    ...
    def run_phase1_infrastructure_checks(self) -> Dict[str, Any]:
        ...
        for check_name, check_result in checks.items():
            ...
            if score < 50:
                phase1_results["issues"].append({
                    ...
                    "message": check_result.get("error", check_result.get("details", "Unknown issue")),
                    "score": score
                })
        ...
        logger.info(f"✅ Phase 1 完了: スコア {phase1_results['score']:.1f}/100 ({phase1_results['status']})")
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for log in logs:
                    ...
                    if progress_match:
                        ...
                        latest_progress = {"current": current, "total": total, "ratio": current/total if total > 0 else 0}
                ...
                # 評価
                if fetch_issues:
                    critical_issues = [i for i in fetch_issues if i["severity"] == "CRITICAL"]
                    if critical_issues:
                        return {
                            "status": "critical",
                            "issues": fetch_issues,
    ...
    def check_signal_generation_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = ["python", str(signal_script), "--hours", "4", "--threshold-alert", "60"]
            ...
            if result.returncode == 0:
                ...
                # アラート閾値以下
                alert_match = re.search(r'Health score (\d+) below threshold', result.stdout)
                if alert_match:
    ...
    def check_main_loop_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            main_result = subprocess.run(main_loop_cmd, capture_output=True, text=True, timeout=15)
            main_loop_reached = main_result.returncode == 0 and main_result.stdout.strip()
    ...
    def run_phase2_application_checks(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Phase 2 完了: スコア {phase2_results['score']:.1f}/100 ({phase2_results['status']})")
                    ...
                    if logs:
                        ...
                        if log_timestamp:
                            try:
                                ...
                                hours_ago = (datetime.now().astimezone() - log_time).total_seconds() / 3600
    ...
    def _check_main_loop_not_reached_pattern(self) -> Optional[Dict]:
        ...
        try:
            ...
            init_result = subprocess.run(init_cmd, capture_output=True, text=True, timeout=15)
            main_result = subprocess.run(main_cmd, capture_output=True, text=True, timeout=15)
                ...
                if len(revisions) >= 2:
                    ...
                    if active and latest != active:
                        return {
                            ...
                            "latest_revision": latest.get("metadata", {}).get("name", "unknown"),
                            "active_revision": active.get("metadata", {}).get("name", "unknown")
                        }
                ...
                if len(memory_values) >= 5:
                    ...
                    if avg_recent > avg_early * 1.2:  # 20%以上の増加
                        return {
                            ...
                            "growth_percent": (avg_recent - avg_early) / avg_early * 100
                        }
    ...
    def run_phase3_hidden_issues_detection(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Phase 3 完了: スコア {phase3_results['score']:.1f}/100 ({phase3_results['status']})")
        ...
        try:
            ...
            for phase_name, phase_data in self.results["phases"].items():
                ...
                for issue in phase_issues:
                    ...
                    actions = self._generate_actions_for_issue(check, severity, issue.get("message", ""))
...
"""
    ...
    def run_phase4_overall_assessment(self) -> Dict[str, Any]:
        ...
        for check_name, check_result in checks.items():
            ...
            if score < 80:
                phase4_results["issues"].append({
                    "check": check_name,
                    "severity": "MEDIUM",
                    "message": check_result.get("error", check_result.get("details", "Analysis issue")),
                    "score": score
                })
        ...
        logger.info(f"✅ Phase 4 完了: スコア {phase4_results['score']:.1f}/100 ({phase4_results['status']})")
    ...
    def _generate_final_report(self):
        ...
        if "phase1" in self.results["phases"]:
            ...
            for check_name, check_result in phase1["checks"].items():
                status_icon = "✅" if check_result.get("status") == "healthy" else "⚠️" if check_result.get("status") in ["warning", "degraded"] else "❌"
        ...
        if "phase3" in self.results["phases"]:
            ...
            for check_name, check_result in phase3["checks"].items():
                status_icon = "✅" if check_result.get("status") == "healthy" else "⚠️" if check_result.get("status") in ["warning", "info"] else "❌"
                print(f"  {status_icon} {check_name}: {check_result.get('details', 'No details')}")
...
def main():
    ...
    parser.add_argument("--phase", choices=["phase1", "phase2", "phase3", "phase4"], help="特定フェーズのみ実行")
