...
def test_phase_c1_imports():
    ...
    try:
        ...
        from crypto_bot.utils.ensemble_confidence import EnsembleConfidenceCalculator, calculate_ensemble_confidence
...
def test_timeframe_ensemble_processor():
    ...
    try:
        ...
        logger.info(f"   プロセッサー情報: {processor_info['timeframe']}, enabled={processor_info['ensemble_enabled']}")
...
def test_cross_timeframe_integrator():
    ...
    try:
        ...
        # 統合予測実行テスト
        integrated_signal, integration_info = integrator.integrate_timeframe_predictions(
...
def test_production_config_compatibility():
    ...
    try:
        ...
        logger.info(f"   信頼度閾値: {ensemble_config.get('confidence_threshold', 0.5)}")
