...
class ABTestingSystem:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.multiple_testing_correction = stats_config.get(
            "multiple_testing_correction", "bonferroni"
        )
        self.effect_size_methods = stats_config.get(
            "effect_size_methods", ["cohen_d", "hedges_g"]
        )

        # データ管理
        self.active_experiments: Dict[str, ExperimentConfig] = {}
        self.experiment_results: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=10000)
        )
        self.experiment_analytics: Dict[str, Dict] = {}

        # 統計結果キャッシュ
        self.statistical_results: Dict[str, List[StatisticalTestResult]] = defaultdict(
            list
        )
        ...
        self.persistence_path = ab_config.get("persistence", {}).get(
            "path", "data/ab_testing"
        )

        # 可視化設定
        self.enable_visualization = ab_config.get("visualization", {}).get(
            "enabled", True
        )
    ...
    def create_experiment(
        ...
        try:
            ...
            experiment = ExperimentConfig(
                ...
                or self.default_significance_level,
    ...
    def record_result(
        ...
        try:
            if experiment_id not in self.active_experiments:
                logger.warning(
                    f"Recording result for unknown experiment: {experiment_id}"
                )
                return
    ...
    def analyze_experiment(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            treatment_results = [
                r for r in results if r.group == TreatmentGroup.TREATMENT
            ]
    ...
    def _analyze_metric(
        ...
        try:
            ...
            try:
                ...
                pooled_std = np.sqrt(
                    (
                        (len(control_array) - 1) * np.var(control_array)
                        + (len(treatment_array) - 1) * np.var(treatment_array)
                    )
                    / (len(control_array) + len(treatment_array) - 2)
                )
                cohens_d = (
                    (np.mean(treatment_array) - np.mean(control_array)) / pooled_std
                    if pooled_std > 0
                    else 0
                )
                ...
                se_diff = np.sqrt(
                    np.var(treatment_array) / len(treatment_array)
                    + np.var(control_array) / len(control_array)
                )
                t_critical = stats.t.ppf(
                    1 - alpha / 2, len(control_array) + len(treatment_array) - 2
                )
            ...
            try:
                ...
                metric_analysis["statistical_tests"]["mann_whitney"] = (
                    StatisticalTestResult(
                        ...
                        interpretation=self._interpret_mann_whitney(
                            p_value_mw, r_effect_size, alpha
                        ),
                    )
                )
                ...
                logger.warning(
                    f"Mann-Whitney test failed for {metric_name}: {mw_error}"
                )
    ...
    def _interpret_t_test(
        self, p_value: float, effect_size: float, alpha: float
    ) -> str:
        """t検定結果解釈"""
        if p_value < alpha:
            ...
            return (
                f"No significant effect detected (p={p_value:.4f}, d={effect_size:.3f})"
            )
    ...
    def _assess_practical_significance(
        ...
        try:
            ...
            threshold = practical_thresholds.get(
                metric_name, practical_thresholds["default"]
            )
    ...
    def _generate_analysis_summary(
        ...
        try:
            ...
            # 各指標の有意性チェック
            for metric_name, metric_analysis in analysis_results[
                "statistical_tests"
            ].items():
                if (
                    isinstance(metric_analysis, dict)
                    and "statistical_tests" in metric_analysis
                ):
                    # t検定結果優先
            ...
            if summary["overall_significance"]:
                summary["recommendations"].append(
                    "Continue with treatment configuration"
                )
                if any(abs(es) > 0.5 for es in summary["effect_sizes"].values()):
                    summary["recommendations"].append(
                        "Strong evidence for treatment superiority"
                    )
            else:
                summary["recommendations"].append(
                    "No strong evidence for treatment superiority"
                )
                summary["recommendations"].append(
    ...
    def _evaluate_early_stopping(
        self, experiment_id: str, analysis_results: Dict[str, Any]
    ):
        """早期停止判定"""
        try:
            experiment = self.active_experiments[experiment_id]
            ...
            if summary.get("overall_significance", False):
                ...
                primary_significant = any(
                    m in significant_metrics for m in experiment.primary_metrics
                )
                ...
                if primary_significant and large_effects:
                    should_stop = True
                    stop_reason = (
                        "Strong statistical significance with large effect sizes"
                    )
    ...
    def stop_experiment(self, experiment_id: str, reason: str = "Manual stop"):
        ...
        try:
            ...
            if final_analysis and final_analysis["summary"].get(
                "overall_significance", False
            ):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.monitor_thread = threading.Thread(
                target=self._monitoring_loop, daemon=True, name="ABTestMonitor"
            )
    ...
    def get_experiment_status(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            treatment_count = sum(
                1 for r in results if r.group == TreatmentGroup.TREATMENT
            )
            ...
            status = {
                ...
                "progress": {
                    ...
                    "progress_percentage": min(
                        100, (len(results) / experiment.min_sample_size) * 100
                    ),
                },
                "duration": {},
            ...
            if experiment_id in self.statistical_results:
                ...
                status["latest_analysis"] = {
                    ...
                    "significant_metrics": latest["summary"].get(
                        "significant_metrics", []
                    ),
                    "overall_significant": latest["summary"].get(
                        "overall_significance", False
                    ),
                }
    ...
    def generate_experiment_report(self, experiment_id: str) -> Optional[str]:
        ...
        try:
            ...
            report = []
            report.append("# A/B Test Experiment Report")
            ...
            for metric_name, metric_analysis in latest_analysis[
                "statistical_tests"
            ].items():
                if "statistical_tests" in metric_analysis:
                    ...
                    report.append(
                        f"**Control:** Mean={desc_stats['control']['mean']:.4f}, "
                        f"SD={desc_stats['control']['std']:.4f}"
                    )
                    report.append(
                        f"**Treatment:** Mean={desc_stats['treatment']['mean']:.4f}, "
            ...
            report.append(
                f"**Overall Significant:** {summary.get('overall_significance', False)}"
            )
            report.append(
                f"**Significant Metrics:** {', '.join(summary.get('significant_metrics', []))}"
            )
            report.append("")
    ...
    def create_dynamic_vs_static_experiment(
        ...
        try:
            hypothesis = (
                "Dynamic weight adjustment will outperform static weights "
            ...
            logger.info(
                f"🧪 Created dynamic vs static weights experiment: {experiment_id}"
            )
            return experiment_id
