...
def enhanced_init_5_fetch_price_data(fetcher, dd: dict, max_retries: int = 5, timeout: int = 30) -> Optional[pd.DataFrame]:
    """
    ...
    logger.info(f"🔧 [INIT-5] Configuration: max_retries={max_retries}, timeout={timeout}s")
    ...
    try:
        ...
        try:
            ...
            fear_greed_fetcher = FearGreedDataFetcher()
        ...
        try:
            ...
            try:
                ...
                if initial_df is not None and not initial_df.empty:
                    ...
                    missing_columns = [col for col in required_columns if col not in initial_df.columns]
                    
                    if missing_columns:
                        logger.warning(f"⚠️ [INIT-5] Missing required columns: {missing_columns}")
                    else:
                        logger.info("✅ [INIT-5] All required columns present")
                        
                    # データ範囲確認
                    logger.info(f"📊 [INIT-5] Data range: {initial_df.index.min()} to {initial_df.index.max()}")
                    logger.info(f"📊 [INIT-5] Price range: {initial_df['close'].min():.2f} to {initial_df['close'].max():.2f}")
            ...
            fetch_time = time.time() - start_time
            logger.error(f"❌ [INIT-5] Attempt {attempt + 1} failed after {fetch_time:.2f}s: {e}")
...
def enhanced_init_6_calculate_atr(initial_df: Optional[pd.DataFrame], period: int = 14) -> Optional[pd.Series]:
    """
    INIT-6段階: ATR計算（強化版）
    ...
    if initial_df is not None and not initial_df.empty:
        try:
            ...
            if missing_columns:
                logger.error(f"❌ [INIT-6] Missing required columns for ATR: {missing_columns}")
                return None
                
            # ATR計算に必要な最小レコード数確認
            min_records = period + 1
            if len(initial_df) < min_records:
                logger.error(f"❌ [INIT-6] Insufficient data for ATR calculation: {len(initial_df)} < {min_records}")
                return None
                
            logger.info(f"📊 [INIT-6] Data validation passed: {len(initial_df)} records available")
            ...
            if atr_series is not None and not atr_series.empty:
                ...
                logger.info(f"📊 [INIT-6] ATR statistics: latest={latest_atr:.6f}, mean={mean_atr:.6f}")
                
                # 異常値チェック
                if latest_atr <= 0 or latest_atr > 1.0:
                    logger.warning(f"⚠️ [INIT-6] ATR value may be unusual: {latest_atr}")
...
def enhanced_init_6_fallback_atr(period: int = 14) -> pd.Series:
    ...
    logger.info(f"✅ [INIT-6] Enhanced fallback ATR generated: {len(atr_series)} values")
    logger.info(f"📊 [INIT-6] Fallback ATR statistics: latest={latest_atr:.6f}, mean={atr_series.mean():.6f}")
    
    return atr_series


def enhanced_init_7_initialize_entry_exit(strategy, risk_manager, atr_series: pd.Series) -> Any:
    """
...
def enhanced_init_8_clear_cache() -> None:
    ...
    try:
        from crypto_bot.ml.external_data_cache import clear_global_cache, get_global_cache
...
def enhanced_init_sequence(fetcher, dd: dict, strategy, risk_manager, balance: float):
    ...
    entry_exit = enhanced_init_7_initialize_entry_exit(strategy, risk_manager, atr_series)
    ...
    logger.info("✅ [INIT-ENHANCED] Enhanced initialization sequence completed successfully")
