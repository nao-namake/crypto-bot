...
from .trading_statistics_manager import TradeRecord
...
class TradingIntegrationService:
    ...
    def integrate_with_execution_client(self, execution_client):
        ...
        original_cancel_order = getattr(execution_client, 'cancel_order', None)
    ...
    def record_trade_signal(self, signal: str, confidence: float, strategy_type: str = "ML",
                           expected_profit: float = 0.0, risk_level: str = "medium",
        ...
        self.logger.info(f"取引シグナル記録: {signal_id} - {signal} (信頼度: {confidence})")
        
        return signal_id
    
    def record_market_update(self, symbol: str, price: float, volume: float = None,
                           bid: float = None, ask: float = None, **kwargs) -> None:
        """市場データ更新記録"""
        # ビッドアスクスプレッド計算
        bid_ask_spread = None
        if bid and ask:
    ...
    def execute_integrated_trade(self, symbol: str, side: str, amount: float,
                               price: float = None, strategy_type: str = "ML",
                               confidence: float = 0.0, **kwargs) -> Dict[str, Any]:
        """統合取引実行"""
        try:
            # 取引エントリー記録
    ...
    def close_integrated_trade(self, trade_id: str, exit_price: float,
                             exit_fee: float = 0.0, reason: str = "manual") -> Dict[str, Any]:
        """統合取引決済"""
        try:
            # 取引決済記録
            success = self.status_manager.record_trade_exit(trade_id, exit_price, exit_fee)
    ...
    def get_trading_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_summary": self.status_manager.stats_manager.get_performance_summary(),
        }
    ...
    def get_performance_report(self) -> str:
        ...
        integrated_report.append("=" * 80)
        integrated_report.append(f"📅 生成日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if self.active_trades:
            for trade_id, trade_info in self.active_trades.items():
                integrated_report.append(f"   - {trade_id}: {trade_info['side']} {trade_info['amount']} {trade_info['symbol']}")
        ...
        try:
            if order_result and isinstance(order_result, dict):
                if order_result.get('status') == 'filled':
                    ...
                    self.status_manager.record_trade_exit(trade_id, exit_price, exit_fee)
    ...
    def shutdown(self):
        ...
        try:
            ...
            # アクティブ取引の強制決済（オプション）
            if self.active_trades:
                self.logger.warning(f"アクティブ取引が残っています: {len(self.active_trades)}")
...
def main():
    ...
    # テストシグナル記録
    signal_id = integration_service.record_trade_signal(
        signal="buy",
        confidence=0.8,
