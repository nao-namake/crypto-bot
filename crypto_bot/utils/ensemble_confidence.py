...
class EnsembleConfidenceCalculator:
    ...
    def calculate_prediction_confidence(
        ...
        try:
            ...
            # 2. 確率極端度ベース信頼度  
            probability_confidence = self._calculate_probability_confidence(probabilities)
            
            # 3. モデル合意度（個別予測が提供されている場合）
            if individual_predictions is not None:
                model_agreement = self._calculate_model_agreement_score(individual_predictions)
    ...
    def _calculate_probability_confidence(self, probabilities: np.ndarray) -> np.ndarray:
        """確率極端度ベース信頼度計算"""
    ...
    def calculate_consensus_score(
        ...
        if weights is not None and len(weights) == len(signal_values):
            ...
            weighted_var = np.average(
                [(x - weighted_mean)**2 for x in signal_values], 
                weights=weights
            )
    ...
    def assess_risk_level(
        self, confidence_scores: Union[np.ndarray, float]
    ) -> str:
        """リスクレベル評価"""
    ...
    def calculate_exit_threshold(
        ...
        # 信頼度ベース調整
        if confidence_score is not None:
            confidence_adj = (1.0 - confidence_score) * 0.1  # 低信頼度ほど早めにエグジット
        else:
            confidence_adj = 0.0
