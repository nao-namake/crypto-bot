...
class ADWINDetector(DriftDetectorBase):
    ...
    def update(self, sample: float) -> bool:
        ...
        if self.total_samples > 1:
            ...
            self.variance = (
                (self.total_samples - 2) * self.variance
                + (sample - old_mean) * (sample - new_mean)
            ) / (self.total_samples - 1)
    ...
    def _compress_buckets(self):
        ...
        while len(self.buckets) > self.max_buckets:
            ...
            min_idx = min(
                range(len(self.buckets)), key=lambda i: self.buckets[i]["count"]
            )
        ...
        for cut_point in range(1, len(self.buckets)):
            ...
            if left_count > 0 and right_count > 0:
                ...
                if self._is_change_detected(
                    left_mean, right_mean, left_count, right_count
                ):
                    return True
...
class StatisticalDriftDetector(DriftDetectorBase):
    ...
    def _detect_drift(self) -> bool:
        ...
        if len(reference_data.shape) > 1:
            ...
            # 1D data
            statistic, min_p_value = stats.ks_2samp(
        ...
        if error == 1:  # Error occurred
            ...
            if len(self.error_positions) >= 2:
                ...
                for i in range(1, len(self.error_positions)):
                    distances.append(
                        self.error_positions[i] - self.error_positions[i - 1]
                    )
                ...
                if distances:
                    ...
                    # Update minimum values
                    if (
                        self.mean_distance + self.std_distance
                    ...
                    if min_level > 0 and current_level / min_level < self.alpha_drift:
                        ...
                        self.last_drift_time = datetime.now()
                        logger.info(
                            f"EDDM drift detected at sample {self.sample_count}"
                        )
                        self._reset_after_drift()
                        return True
                    elif (
                        min_level > 0 and current_level / min_level < self.alpha_warning
                    ):
                        self.warning_level = True
                    else:
                        self.warning_level = False
