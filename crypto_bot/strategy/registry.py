...
class StrategyRegistry:
    ...
    def register(self, name: str, strategy_class: Type[StrategyBase]) -> None:
        ...
        if not issubclass(strategy_class, StrategyBase):
            raise ValueError(f"Strategy class {strategy_class} must inherit from StrategyBase")
    ...
    def load_external_strategy(self, file_path: str, strategy_name: str = None) -> None:
        ...
        """
        try:
            spec = importlib.util.spec_from_file_location("external_strategy", file_path)
            ...
            if not strategies_found:
                ...
                logger.info(f"Loaded external strategies from {file_path}: {strategies_found}")
