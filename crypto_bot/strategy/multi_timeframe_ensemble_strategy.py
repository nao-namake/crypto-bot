...
from crypto_bot.ml.timeframe_ensemble import TimeframeEnsembleProcessor, create_timeframe_ensemble_processor
from crypto_bot.ml.cross_timeframe_ensemble import CrossTimeframeIntegrator, create_cross_timeframe_integrator
from crypto_bot.utils.ensemble_confidence import EnsembleConfidenceCalculator
from crypto_bot.ml.preprocessor import FeatureEngineer

# Phase B基盤統合
try:
    from crypto_bot.feature_engineering.batch_feature_calculator import BatchFeatureCalculator
    from crypto_bot.feature_engineering.technical_feature_engine import TechnicalFeatureEngine
    from crypto_bot.feature_engineering.external_data_integrator import ExternalDataIntegrator
...
class MultiTimeframeEnsembleStrategy(StrategyBase):
    ...
    def _get_short_term_features(self) -> List[str]:
        """短期取引用特徴量セット（15分足特化）"""
        return [
            "rsi_14", "rsi_7", "macd", "bb_percent", "volume_zscore",
            "vix", "momentum_14", "day_of_week", "hour_of_day",
            "price_change_1h", "volatility_1h", "breakout_signal",
            "stoch", "willr", "adx", "cmf", "atr_7"
        ]

    def _get_long_term_features(self) -> List[str]:
        """長期トレンド用特徴量セット（4時間足特化）"""
        return [
            "sma_200", "sma_50", "ema_50", "adx", "dxy", "fear_greed",
            "trend_strength", "bb_width", "cmf", "support_resistance",
            "fibonacci_level", "central_bank_policy", "macro_regime",
            "vix", "funding", "cross_correlation", "regime_detection"
        ]
    ...
    def _get_full_feature_set(self) -> List[str]:
        ...
        return [
            ...
            # DXY・金利（10特徴量）  
            "dxy", "dxy_change", "us_10y", "yield_curve", "real_rates", "dollar_strength",
            "fed_funds_rate", "treasury_volatility", "currency_momentum", "rate_expectations",
            ...
            "bb_percent", "bb_width", "sma_20", "sma_50", "sma_200", "ema_12", "ema_26",
            "ema_50", "adx", "cci", "williams_r", "stoch_k", "stoch_d", "momentum_14",
            
            # Phase 3.2A-D高度特徴量（65特徴量）
            "price_position", "candle_patterns", "support_resistance", "breakout_signals",
            "autocorrelation", "seasonal_patterns", "regime_detection", "cycle_analysis",
            "cross_correlation", "relative_strength", "spread_analysis",
            ...
            "price_change_1h", "price_change_4h", "price_change_24h", "volume_change_1h",
            "volume_change_24h", "volume_zscore", "volatility_1h", "volatility_24h",
    ...
    def _apply_phase_b_processing(self, data: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        """Phase B基盤バッチ処理適用"""
        try:
    ...
    def _generate_market_context(self, price_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(price_df) >= 50:
                ...
                context["trend_strength"] = min(abs(current_price - sma_50) / sma_50, 1.0)
            else:
            ...
            # 市場レジーム判定（Phase C1統合）
            context["market_regime"] = self.confidence_calculator.assess_market_regime(context)
        ...
        try:
            ...
            if self.multi_timeframe_fetcher is not None:
                ...
                if timeframe in multi_data:
                    ...
                    logger.debug(f"✅ {timeframe} data from fetcher: {len(tf_data)} records")
                else:
            ...
            # データ品質チェック
            if len(tf_data) < 50:
                logger.warning(f"Insufficient {timeframe} data: {len(tf_data)} rows")
    ...
    def _convert_timeframe_data(self, price_df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        """従来方式タイムフレーム変換"""
        try:
            if timeframe == "15m":
    ...
    def _track_ensemble_performance(
        self, timeframe_predictions: Dict, integrated_signal: float, 
        integration_info: Dict, final_signal: Signal
    ):
        """アンサンブルパフォーマンス追跡"""
        record = {
            ...
            "consensus_score": integration_info.get("consensus_score", 0.0),
            "integration_quality": integration_info.get("integration_quality", "unknown"),
        ...
        for tf, pred_data in timeframe_predictions.items():
            ...
            record["timeframe_confidences"][tf] = confidence[0] if hasattr(confidence, '__len__') else confidence
    ...
    def get_ensemble_strategy_info(self) -> Dict:
        ...
        info = {
            ...
            "phase_b_integrated": PHASE_B_AVAILABLE and hasattr(self, 'phase_b_integrated'),
            "timeframes": self.timeframes,
        ...
        try:
            info["cross_timeframe_integrator"] = self.cross_timeframe_integrator.get_integrator_info()
        except Exception as e:
        ...
        if self.prediction_history:
            ...
            info["recent_performance"] = self._analyze_recent_ensemble_performance(recent_predictions)
            
        return info

    def _analyze_recent_ensemble_performance(self, recent_predictions: List[Dict]) -> Dict:
        """最近のアンサンブルパフォーマンス分析"""
        try:
            ...
            return {
                ...
                "consensus_stability": np.std(consensus_scores) if consensus_scores else 0.0,
                "avg_integrated_signal": np.mean(integrated_signals) if integrated_signals else 0.5,
                "signal_diversity": np.std(integrated_signals) if integrated_signals else 0.0,
