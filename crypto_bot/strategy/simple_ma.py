...
class SimpleMAStrategy(StrategyBase):
    ...
    def __init__(self, short_period: int = 20, long_period: int = 50, config: dict = None):
        """
        Args:
        ...
        logger.info(f"SimpleMAStrategy initialized: short={short_period}, long={long_period}")
    ...
    def logic_signal(self, price_df: pd.DataFrame, position: Position) -> Optional[Signal]:
        ...
        logger.debug(f"MA values - Short: {current_short:.2f}, Long: {current_long:.2f}")
...
class BollingerBandsStrategy(StrategyBase):
    ...
    def logic_signal(self, price_df: pd.DataFrame, position: Position) -> Optional[Signal]:
        ...
        logger.debug(f"Bollinger Bands - Upper: {current_upper:.2f}, SMA: {current_sma:.2f}, Lower: {current_lower:.2f}")
        logger.debug(f"Current price: {current_price:.2f}")
        
        if position_exists:
            # ポジション保有中は中央線への回帰でクローズ
            if abs(current_price - current_sma) < abs(current_sma * 0.005):  # 0.5%以内で中央線
