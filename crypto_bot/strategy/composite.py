...
class CompositeStrategy(StrategyBase):
    ...
    def __init__(self, 
        ...
        logger.info(f"Initialized CompositeStrategy with {len(strategies)} strategies, mode: {combination_mode}")
        for i, (strategy, weight) in enumerate(self.normalized_strategies):
            logger.info(f"  Strategy {i+1}: {strategy.__class__.__name__} (weight: {weight:.3f})")
    
    def logic_signal(self, price_df: pd.DataFrame, position: Position) -> Optional[Signal]:
        """
    ...
    def _combine_signals(self, signals: List[Tuple[Optional[Signal], float]], current_price: float) -> Optional[Signal]:
        """
    ...
    def _weighted_average_combine(self, signals: List[Tuple[Signal, float]], current_price: float) -> Optional[Signal]:
        """重み付き平均によるシグナル統合"""
        buy_weight = 0.0
        sell_weight = 0.0
    ...
    def _majority_vote_combine(self, signals: List[Tuple[Signal, float]], current_price: float) -> Optional[Signal]:
        """多数決によるシグナル統合"""
    ...
    def _unanimous_combine(self, signals: List[Tuple[Signal, float]], current_price: float) -> Optional[Signal]:
        """全戦略一致によるシグナル統合"""
        if not signals:
            return None
    ...
    def _first_match_combine(self, signals: List[Tuple[Signal, float]]) -> Optional[Signal]:
        """最初にマッチしたシグナルを返す"""
        if signals:
            return signals[0][0]
        return None
