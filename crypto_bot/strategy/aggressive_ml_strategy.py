...
class AggressiveMLStrategy(MLStrategy):
    ...
    def __init__(self, model_path: str, threshold: float = None, config: dict = None):
        ...
        self.profit_target_multiplier = strategy_params.get(
        ...
        self.aggressive_risk_manager = AggressiveRiskManager(
            ...
            kelly_max_fraction=risk_config.get("kelly_criterion", {}).get(
                "max_fraction", 0.4
            ),
    ...
    def generate_signal(self, data: pd.DataFrame, position: Position) -> Signal:
        ...
        try:
            ...
            # 積極的強化判定
            enhanced_signal = self._enhance_signal_aggressively(
                data, base_signal, position
            )
    ...
    def _enhance_signal_aggressively(
        self, data: pd.DataFrame, base_signal: Signal, position: Position
    ) -> Signal:
        """
        シグナルの積極的強化
        ...
        if signal.exist:
            logger.info(
                f"Aggressive signal generated: {signal.side}, "
                f"confidence={confidence:.3f}, prediction={prediction:.3f}"
            )

        return signal

    def _prepare_features_aggressively(
    ...
    def _extract_multi_timeframe_features(
        ...
        try:
            ...
            if len(data) >= 4:
                ...
                momentum_15m = (
                    recent_4h["close"].iloc[-1] / recent_4h["close"].iloc[0] - 1
                ) * 100
            ...
            if len(data) >= 24:
                ...
                # トレンド一致スコア
                trend_alignment = (
                    np.sign(short_trend) + np.sign(medium_trend) + np.sign(long_trend)
                )
    ...
    def _generate_aggressive_entry_signal(
        ...
        # 積極的エントリー条件
        aggressive_long = (
            prediction > 0.5 + dynamic_threshold
        ...
        if confidence > 0.75:
            ...
            aggressive_long = aggressive_long or (
                prediction > 0.5 + high_confidence_threshold
            )
            aggressive_short = aggressive_short or (
                prediction < 0.5 - high_confidence_threshold
            )
    ...
    def _generate_aggressive_exit_signal(
        ...
        # 積極的利確判定
        profit_exit = self._check_aggressive_profit_taking(
            position, current_price, data
        )
        if profit_exit:
            return Signal(
                side="SELL" if position.side == "BUY" else "BUY",
    ...
    def _calculate_dynamic_threshold(
        self, data: pd.DataFrame, confidence: float
    ) -> float:
        """
        動的閾値計算（積極版）
        ...
        # 最終閾値
        dynamic_threshold = (
            base_threshold * confidence_factor * volatility_factor * vix_factor
        )
    ...
    def _check_basic_exit_conditions(
        self, position: Position, prediction: float, confidence: float
    ) -> bool:
        """
        基本エグジット条件チェック
    ...
    def _check_aggressive_profit_taking(
        self, position: Position, current_price: float, data: pd.DataFrame
    ) -> bool:
        """
        積極的利確判定
        ...
        if position.hold_bars <= 3 and profit_rate > 0.02:  # 3時間以内で2%利益
            logger.info(
                f"Quick profit taking: {profit_rate:.2%} in {position.hold_bars} bars"
            )
            return True
        ...
        if len(data) >= 14:
            atr = (
                data["close"]
                .rolling(14)
                .apply(lambda x: pd.Series(x).pct_change().abs().mean())
                .iloc[-1]
            )
    ...
    def get_strategy_metrics(self) -> Dict:
        ...
        if self.signal_confidence_history:
            recent_confidences = [
                s["confidence"] for s in self.signal_confidence_history[-20:]
            ]
