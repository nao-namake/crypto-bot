...
class GlobalCacheManager:
    ...
    def get(self, key: str, default: Any = None) -> Optional[Any]:
        ...
        with self._cache_lock:
            ...
            if key in self._cache:
                ...
                logger.debug(f"🎯 Cache hit: {key} (quality: {entry.quality_score:.2f})")
                return entry.data
    ...
    def put(self, key: str, data: Any, source: str = "unknown", 
            ttl: timedelta = None, quality_score: float = 1.0) -> bool:
        """キャッシュデータ保存"""
        try:
            with self._cache_lock:
                # TTL設定
                ...
                logger.debug(f"💾 Cached: {key} (TTL: {ttl}, Quality: {quality_score:.2f}, Size: {size_bytes}B)")
                return True
    ...
    def get_or_fetch(self, key: str, fetch_func: callable, source: str = "unknown",
                     ttl: timedelta = None, quality_score: float = 1.0, 
    ...
    def _check_memory_limit(self, additional_bytes: int = 0) -> bool:
        """メモリ制限チェック"""
        total_size_mb = (self._stats["total_size_bytes"] + additional_bytes) / (1024 * 1024)
    ...
    def get_stats(self) -> Dict[str, Any]:
        """キャッシュ統計取得"""
        with self._cache_lock:
            hit_rate = (self._stats["hits"] / self._stats["requests"]) * 100 if self._stats["requests"] > 0 else 0
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            ...
            return {
                ...
                "ttl_settings": {k: v.total_seconds() / 3600 for k, v in self.default_ttls.items()}
            }
...
# 外部データフェッチャー統合用デコレータ
def cached_external_data(source: str, ttl_hours: float = None, quality_threshold: float = 0.5):
