...
class CrossTimeframeIntegrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.consensus_threshold = multi_config.get("timeframe_consensus_threshold", 0.6)
        self.missing_data_tolerance = multi_config.get("missing_data_tolerance", 0.1)
        
        # 統合方法設定
        self.integration_method = multi_config.get("integration_method", "quality_weighted_ensemble")
        ...
        self.ensemble_confidence_threshold = ensemble_config.get("confidence_threshold", 0.65)
    ...
    def integrate_timeframe_predictions(
        ...
        try:
            ...
            if self.dynamic_weighting_enabled:
                ...
                dynamic_weights = self._get_base_weights_for_predictions(processed_predictions)
            ...
            integration_info = {
                ...
                "meets_consensus_threshold": consensus_score >= self.consensus_threshold,
            }
            
            # 7. 履歴記録・パフォーマンス追跡
            self._record_integration(
                integrated_signal, integration_info, processed_predictions, market_context
            )
        ...
        for timeframe, pred_data in timeframe_predictions.items():
            try:
                ...
                processed_data = {
                    ...
                    "confidence": pred_data.get("unified_confidence", pred_data.get("confidence", 0.5)),
                    "model_agreement": pred_data.get("model_agreement", 1.0),
                    "prediction_quality": pred_data.get("prediction_quality", "unknown"),
    ...
    def _calculate_dynamic_weights(
        ...
        if self.market_adaptation_enabled and market_context:
            market_regime = self.confidence_calculator.assess_market_regime(market_context)
        ...
        performance_adjustments = self._calculate_performance_adjustments(timeframes_order)
        ...
        # 6. 履歴記録
        self._record_weight_update(timeframes_order, dynamic_weights, {
    ...
    def _calculate_performance_adjustments(self, timeframes_order: List[str]) -> List[float]:
        """履歴ベースパフォーマンス調整計算"""
    ...
    def _calculate_cross_timeframe_consensus(
        ...
        """タイムフレーム間合意度計算"""
        probabilities = [pred_data["probability"] for pred_data in processed_predictions.values()]
    ...
    def _assess_integration_quality(
        ...
        """統合品質評価"""
        # 基本品質要因
        confidence_scores = [pred_data["confidence"] for pred_data in processed_predictions.values()]
    ...
    def _record_integration(
        ...
        record = {
            ...
            "market_regime": market_context.get("market_regime", "unknown") if market_context else "unknown",
        }
    ...
    def create_final_signal(
        ...
        if position_exists:
            ...
            if integrated_signal > entry_threshold and integration_quality in ["good", "excellent"]:
                logger.info(
            ...
            elif integrated_signal < (1.0 - entry_threshold) and integration_quality in ["good", "excellent"]:
                logger.info(
...
def create_cross_timeframe_integrator(config: Dict[str, Any]) -> CrossTimeframeIntegrator:
    """
    タイムフレーム間統合システム作成
