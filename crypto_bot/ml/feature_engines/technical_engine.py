...
class TechnicalFeatureEngine:
    ...
    def __init__(self, config: Dict[str, Any], batch_calculator: BatchFeatureCalculator):
        ...
        try:
            ...
            logger.warning("⚠️ IndicatorCalculator not available, using built-in methods")
            self.ind_calc = None
    ...
    def _parse_technical_features(self) -> Dict[str, Dict]:
        ...
        for indicator in ["rsi", "sma", "ema", "atr", "volume_zscore"]:
            if configs[indicator]["periods"]:
                configs[indicator]["periods"] = sorted(list(set(configs[indicator]["periods"])))
    ...
    def calculate_rsi_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            return self.batch_calc.create_feature_batch("rsi_batch", rsi_features, df.index)
    ...
    def calculate_sma_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            return self.batch_calc.create_feature_batch("sma_batch", sma_features, df.index)
    ...
    def calculate_ema_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            return self.batch_calc.create_feature_batch("ema_batch", ema_features, df.index)
    ...
    def calculate_atr_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            logger.debug(f"✅ ATR batch: {len(atr_features)} indicators ({all_periods})")
            return self.batch_calc.create_feature_batch("atr_batch", atr_features, df.index)
    ...
    def calculate_volume_zscore_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            logger.debug(f"✅ Volume Z-Score batch: {len(zscore_features)} indicators ({periods})")
            return self.batch_calc.create_feature_batch("volume_zscore_batch", zscore_features, df.index)
        ...
        try:
            ...
            if self.technical_configs["macd"]["enabled"]:
                if self.indicator_available and self.ind_calc:
                    ...
                    # 内蔵MACD計算
                    macd_line, macd_signal, macd_hist = self._calculate_macd_builtin(df["close"])
            ...
            if self.technical_configs["stoch"]["enabled"]:
                if self.indicator_available and self.ind_calc:
                    ...
                    try:
                        ...
                        elif hasattr(stoch_result, '__iter__') and len(list(stoch_result)) == 2:
                        ...
                        logger.warning(f"⚠️ IndicatorCalculator stochastic failed: {e}, using builtin")
            ...
            if self.technical_configs["adx"]["enabled"]:
                if self.indicator_available and self.ind_calc:
                    try:
                        ...
                        logger.warning(f"⚠️ IndicatorCalculator ADX failed: {e}, using builtin")
                        adx = self._calculate_adx_builtin(df)
                        complex_features["adx"] = adx
                else:
                    adx = self._calculate_adx_builtin(df)
            ...
            logger.debug(f"✅ Complex indicators batch: {len(complex_features)} indicators")
            return self.batch_calc.create_feature_batch("complex_batch", complex_features, df.index)
    ...
    def calculate_lag_rolling_batch(self, df: pd.DataFrame) -> FeatureBatch:
        ...
        try:
            ...
            lag_roll_features[f"close_mean_{rolling_window}"] = close_series.rolling(rolling_window).mean()
            lag_roll_features[f"close_std_{rolling_window}"] = close_series.rolling(rolling_window).std()
            
            logger.debug(f"✅ Lag/Rolling batch: {len(lag_roll_features)} features")
            return self.batch_calc.create_feature_batch("lag_roll_batch", lag_roll_features, df.index)
    ...
    def calculate_all_technical_batches(self, df: pd.DataFrame) -> List[FeatureBatch]:
        ...
        logger.info(f"🔄 Technical batches completed: {len(non_empty_batches)} batches, {total_features} features")
    ...
    def _calculate_macd_builtin(self, series: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Series]:
        """内蔵MACD計算"""
    ...
    def _calculate_stochastic_builtin(self, df: pd.DataFrame, k_period: int = 14, d_period: int = 3) -> Tuple[pd.Series, pd.Series]:
        """内蔵Stochastic計算"""
    ...
    def _calculate_adx_builtin(self, df: pd.DataFrame, period: int = 14) -> pd.Series:
        ...
        plus_di = 100 * (pd.Series(plus_dm).rolling(window=period).sum() / tr.rolling(window=period).sum())
        minus_di = 100 * (pd.Series(minus_dm).rolling(window=period).sum() / tr.rolling(window=period).sum())
...
def create_technical_engine(config: Dict[str, Any], batch_calculator: BatchFeatureCalculator) -> TechnicalFeatureEngine:
