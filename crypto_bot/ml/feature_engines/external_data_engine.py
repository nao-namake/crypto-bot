...
class ExternalDataResult:
    ...
    def __init__(self, source_name: str, success: bool, data: Optional[pd.DataFrame] = None, 
                 features: Optional[Dict[str, pd.Series]] = None, error: Optional[str] = None):
    ...
    def is_valid(self) -> bool:
        ...
        return self.success and (
            (self.data is not None and not self.data.empty) or 
            (self.features and any(not series.empty for series in self.features.values()))
        )
...
class ExternalDataIntegrator:
    ...
    def __init__(self, config: Dict[str, Any], batch_calculator: BatchFeatureCalculator):
        self.config = config
    ...
    def _fetch_vix_data_batch(self, df_index: pd.Index) -> ExternalDataResult:
        ...
        if not self.external_features["vix"] or not self.fetchers["vix"]:
            return ExternalDataResult("vix", False, error="VIX not enabled or fetcher not available")
        ...
        try:
            ...
            if vix_data is not None and not vix_data.empty:
                ...
                if not vix_features_df.empty:
                    # インデックス合わせ・リサンプリング
                    aligned_features = self._align_external_data(vix_features_df, df_index)
                    ...
                    logger.debug(f"✅ VIX batch: {len(vix_feature_dict)} features, {fetch_time:.3f}s")
    ...
    def _fetch_macro_data_batch(self, df_index: pd.Index) -> ExternalDataResult:
        """Macroデータバッチ取得"""
        if not (self.external_features["macro"] or self.external_features["dxy"]) or not self.fetchers["macro"]:
            return ExternalDataResult("macro", False, error="Macro not enabled or fetcher not available")
        ...
        try:
            ...
            if macro_data and not all(df.empty for df in macro_data.values()):
                # Macro特徴量計算
                macro_features_df = self.fetchers["macro"].calculate_macro_features(macro_data)
                
                if not macro_features_df.empty:
                    # インデックス合わせ・リサンプリング
                    aligned_features = self._align_external_data(macro_features_df, df_index)
                    ...
                    fetch_time = time.time() - start_time
                    logger.debug(f"✅ Macro batch: {len(macro_feature_dict)} features, {fetch_time:.3f}s")
                    
                    return ExternalDataResult(
                        "macro", True, data=aligned_features, features=macro_feature_dict
                    )
    ...
    def _fetch_fear_greed_data_batch(self, df_index: pd.Index) -> ExternalDataResult:
        ...
        if not self.external_features["fear_greed"] or not self.fetchers["fear_greed"]:
            return ExternalDataResult("fear_greed", False, error="Fear&Greed not enabled or fetcher not available")
        ...
        try:
            ...
            if fg_data is not None and not fg_data.empty:
                # Fear&Greed特徴量計算
                fg_features_df = self.fetchers["fear_greed"].calculate_fear_greed_features(fg_data)
                
                if not fg_features_df.empty:
                    # インデックス合わせ・リサンプリング
                    aligned_features = self._align_external_data(fg_features_df, df_index)
                    ...
                    fetch_time = time.time() - start_time
                    logger.debug(f"✅ Fear&Greed batch: {len(fg_feature_dict)} features, {fetch_time:.3f}s")
                    
                    return ExternalDataResult(
                        "fear_greed", True, data=aligned_features, features=fg_feature_dict
                    )
    ...
    def _fetch_funding_data_batch(self, df_index: pd.Index) -> ExternalDataResult:
        ...
        if not self.external_features["funding"] or not self.fetchers["funding"]:
            return ExternalDataResult("funding", False, error="Funding not enabled or fetcher not available")
        
        try:
            start_time = time.time()
            
            # Funding Rate データ取得
            funding_rate_data = self.fetchers["funding"].get_funding_rate_data(symbol="BTC/USDT", limit=100)
            
            # Open Interest データ取得  
            oi_data = self.fetchers["funding"].get_open_interest_data(symbol="BTC/USDT", limit=100)
            
            combined_features = {}
            
            # Funding Rate 特徴量計算
            if funding_rate_data is not None and not funding_rate_data.empty:
                funding_features_df = self.fetchers["funding"].calculate_funding_features(funding_rate_data)
                if not funding_features_df.empty:
                    # インデックス合わせ・リサンプリング
                    aligned_funding = self._align_external_data(funding_features_df, df_index)
            ...
            if combined_features:
                fetch_time = time.time() - start_time
                logger.debug(f"✅ Funding batch: {len(combined_features)} features, {fetch_time:.3f}s")
                
                return ExternalDataResult(
                    "funding", True, features=combined_features
                )
    ...
    def _align_external_data(self, external_df: pd.DataFrame, target_index: pd.Index) -> pd.DataFrame:
        """外部データをターゲットインデックスに合わせる"""
        try:
            # インデックスがDatetimeIndexの場合の処理
            if isinstance(external_df.index, pd.DatetimeIndex) and isinstance(target_index, pd.DatetimeIndex):
                # タイムゾーン統一
            ...
            logger.warning(f"⚠️ External data alignment failed: {e}, using default reindex")
            # フォールバック: シンプルなreindex
            return external_df.reindex(target_index).fillna(0)
    
    def fetch_all_external_data_concurrent(self, df_index: pd.Index) -> List[ExternalDataResult]:
        """全外部データ並行取得"""
        start_time = time.time()
        results = []
        ...
        with ThreadPoolExecutor(max_workers=min(len(fetch_functions), 4)) as executor:
            # タスク投入
            future_to_name = {
                executor.submit(func, df_index): name 
            ...
            for future in as_completed(future_to_name):
                ...
                try:
                    ...
                    logger.error(f"❌ External data fetch failed for {source_name}: {e}")
        ...
        for result in external_results:
            if result.is_valid() and result.features:
                ...
                if len(batch) > 0:
                    ...
                    logger.debug(f"📦 {result.source_name} batch: {len(batch)} features")
            else:
                logger.warning(f"⚠️ {result.source_name} batch invalid or empty")
        
        logger.info(f"🔄 External data batches created: {len(batches)} batches, {total_features} features")
        return batches
    
    # フォールバック関数群
    
    def _generate_vix_fallback(self, df_index: pd.Index, error: str = "") -> ExternalDataResult:
        """VIXフォールバックデータ生成"""
        try:
            from crypto_bot.data.vix_fetcher import get_available_vix_features
            vix_feature_names = get_available_vix_features()
        except ImportError:
            vix_feature_names = ["vix_level", "vix_change", "vix_zscore", "fear_level", "vix_spike", "vix_regime_numeric"]
        
        fallback_features = {}
        for feature in vix_feature_names:
            if feature == "vix_level":
                fallback_features[feature] = pd.Series([20.0] * len(df_index), index=df_index)
            elif feature == "fear_level":
                fallback_features[feature] = pd.Series([1] * len(df_index), index=df_index)
            elif feature == "vix_regime_numeric":
                fallback_features[feature] = pd.Series([1] * len(df_index), index=df_index)
            else:
                fallback_features[feature] = pd.Series([0] * len(df_index), index=df_index)
        
        logger.debug(f"📈 VIX fallback: {len(fallback_features)} features")
        return ExternalDataResult("vix", True, features=fallback_features)
    
    def _generate_macro_fallback(self, df_index: pd.Index, error: str = "") -> ExternalDataResult:
        """Macroフォールバックデータ生成"""
        fallback_features = {
    ...
    def _generate_fear_greed_fallback(self, df_index: pd.Index, error: str = "") -> ExternalDataResult:
        """Fear&Greedフォールバックデータ生成"""
        try:
            from crypto_bot.data.fear_greed_fetcher import get_available_fear_greed_features
        ...
        for feature in fg_feature_names:
            if feature in ["fg_index", "fg_ma_7", "fg_ma_30"]:
                fallback_features[feature] = pd.Series([50.0] * len(df_index), index=df_index)
            elif feature == "fg_neutral":
                fallback_features[feature] = pd.Series([1] * len(df_index), index=df_index)
            elif feature == "fg_volatility":
                fallback_features[feature] = pd.Series([10.0] * len(df_index), index=df_index)
            else:
                fallback_features[feature] = pd.Series([0] * len(df_index), index=df_index)
    ...
    def _generate_funding_fallback(self, df_index: pd.Index, error: str = "") -> ExternalDataResult:
        """Fundingフォールバックデータ生成"""
        fallback_features = {
    ...
    def get_integration_stats(self) -> Dict[str, Any]:
        ...
        success_rate = self.integration_stats["successful_requests"] / self.integration_stats["total_requests"]
        avg_time = self.integration_stats["total_time"] / self.integration_stats["total_requests"]
...
def create_external_data_integrator(config: Dict[str, Any], batch_calculator: BatchFeatureCalculator) -> ExternalDataIntegrator:
