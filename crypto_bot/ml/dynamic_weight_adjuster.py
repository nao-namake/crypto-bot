...
class DynamicWeightAdjuster:
    ...
    def _calculate_market_adaptation_adjustment(
        ...
        try:
            ...
            stress_level = market_context.get("stress_level", "low")
        ...
        try:
            ...
            for timeframe in self.timeframes:
                ...
                try:
                    ...
                    logger.warning(f"Online model prediction failed for {timeframe}: {model_error}")
                    adjustments[timeframe] = 1.0
    ...
    def _calculate_reinforcement_learning_adjustment(
        ...
        try:
            ...
            if np.random.random() < self.epsilon:
                ...
                # 活用：最良行動
                q_values = [
                    self.q_table[current_state][i] for i in range(len(self.action_space))
                ]
            ...
            if (recent_performance and 
                len(self.weight_history) > 0 and 
                hasattr(self, '_last_rl_state') and 
                hasattr(self, '_last_rl_action')):
    ...
    def _prepare_online_features(
        ...
        try:
            ...
            if len(self.weight_history) > 0:
                recent_weights = [w["final_weights"].get(timeframe, 0.33) 
    ...
    def _encode_state(
        ...
        try:
            ...
            if market_context:
                ...
                vol_level = "high" if market_context.get("volatility", 0.02) > 0.04 else "low"
                vix_level = "high" if market_context.get("vix_level", 20) > 25 else "low"
            ...
            if recent_performance:
                ...
                perf_level = "high" if avg_performance > 0.7 else ("low" if avg_performance < 0.4 else "medium")
    ...
    def _calculate_reward(self, recent_performance: Dict[str, float]) -> float:
        ...
        try:
            ...
            # 多目的報酬設計
            accuracy_reward = recent_performance.get("accuracy", 0.5) - 0.5  # -0.5 to 0.5
            profitability_reward = recent_performance.get("profitability", 0.5) - 0.5
            risk_reward = 0.5 - recent_performance.get("risk", 0.5)  # リスク低減で正報酬
    ...
    def _normalize_and_constrain_weights(self, weights: Dict[str, float]) -> Dict[str, float]:
        """重み正規化・制約適用"""
        try:
    ...
    def _update_weights(self, new_weights: Dict[str, float], adjustment_info: Dict[str, Any]):
        """重み更新・履歴記録"""
        try:
    ...
    def _update_online_models(
        ...
        try:
            for timeframe in self.timeframes:
                ...
                try:
                    features_scaled = scaler.partial_fit([features]).transform([features])
                    ...
                    logger.warning(f"Model update failed for {timeframe}: {model_error}")
    ...
    def reset_statistics(self):
        ...
        for key in self.adjustment_stats:
            if isinstance(self.adjustment_stats[key], (int, float)):
                self.adjustment_stats[key] = 0 if isinstance(
                    self.adjustment_stats[key], int) else 0.0
