...
class FeedbackLoopManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.insight_confidence_threshold = learning_config.get("insight_confidence", 0.7)
        ...
        # データ永続化
        self.enable_persistence = feedback_config.get("persistence", {}).get("enabled", False)
        self.persistence_path = feedback_config.get("persistence", {}).get("path", "data/feedback_history")
    ...
    def record_prediction_feedback(
        ...
        try:
            ...
            if prediction_correct:
                ...
                self.prediction_analysis[timeframe]["incorrect_predictions"].append(event)
                
            self.prediction_analysis[timeframe]["confidence_scores"].append(confidence)
            if market_context:
                self.prediction_analysis[timeframe]["market_contexts"].append(market_context)
    ...
    def _analyze_prediction_performance(self, timeframe: str) -> List[LearningInsight]:
        ...
        try:
            correct_preds = list(self.prediction_analysis[timeframe]["correct_predictions"])
            incorrect_preds = list(self.prediction_analysis[timeframe]["incorrect_predictions"])
            
            if len(correct_preds) + len(incorrect_preds) < self.min_samples_for_learning:
                return insights
                
            # 1. 信頼度vs正確性分析
            correct_confidences = [pred.confidence for pred in correct_preds if pred.confidence is not None]
            incorrect_confidences = [pred.confidence for pred in incorrect_preds if pred.confidence is not None]
            ...
            # 2. 最近のパフォーマンストレンド分析
            recent_correct = len([pred for pred in correct_preds 
                                if pred.timestamp > datetime.now() - timedelta(hours=24)])
            recent_total = len([pred for pred in correct_preds + incorrect_preds 
                              if pred.timestamp > datetime.now() - timedelta(hours=24)])
            ...
            if recent_total >= 20:
                ...
                if abs(recent_accuracy - overall_accuracy) > 0.1:  # 10%以上の差
                    trend_direction = "improving" if recent_accuracy > overall_accuracy else "declining"
                    ...
                    insight = LearningInsight(
                        ...
                        description=f"{timeframe}: Prediction accuracy {trend_direction}",
                        data_evidence={
                        ...
                        recommended_actions=[
                            f"{'Maintain' if trend_direction == 'improving' else 'Review'} current prediction strategy",
    ...
    def _analyze_trade_performance(self, timeframe: str) -> List[LearningInsight]:
        ...
        try:
            successful_trades = list(self.trade_analysis[timeframe]["successful_trades"])
            ...
            if len(pnl_history) >= 30:
                win_rate = len(successful_trades) / (len(successful_trades) + len(failed_trades))
    ...
    def _analyze_market_context_patterns(self, timeframe: str) -> List[LearningInsight]:
        ...
        try:
            market_contexts = list(self.prediction_analysis[timeframe]["market_contexts"])
            correct_preds = list(self.prediction_analysis[timeframe]["correct_predictions"])
            ...
            if len(vix_accuracies) >= 2:
                ...
                if vix_accuracies[best_vix_level] - vix_accuracies[worst_vix_level] > 0.15:  # 15%以上の差
                    insight = LearningInsight(
    ...
    def _analyze_hourly_patterns(self) -> List[LearningInsight]:
        ...
        try:
            ...
            recent_events = [e for e in self.feedback_events 
                           if e.timestamp > datetime.now() - timedelta(days=7)]
            
            for event in recent_events:
                if event.feedback_type == FeedbackType.PREDICTION_OUTCOME and event.prediction_correct is not None:
    ...
    def _analyze_daily_patterns(self) -> List[LearningInsight]:
        ...
        try:
            ...
            for event in recent_events:
                if event.feedback_type == FeedbackType.TRADE_RESULT and event.trade_successful is not None:
    ...
    def _validate_insights(self, insights: List[LearningInsight]) -> List[LearningInsight]:
        """インサイト検証・フィルタリング"""
        validated = []
        ...
        try:
            for insight in insights:
                ...
                if insight.confidence_level >= self.insight_confidence_threshold:
                    ...
                    # 重複チェック
                    existing_ids = [existing.insight_id for existing in self.learning_insights]
    ...
    def _generate_parameter_adjustments(self, insights: List[LearningInsight]) -> List[Dict[str, Any]]:
        """パラメータ調整生成"""
        adjustments = []
    ...
    def _apply_parameter_adjustments(self, adjustments: List[Dict[str, Any]]):
        ...
        try:
            for adjustment in adjustments:
                ...
                limited_factor = max(1 - self.max_parameter_change, 
                ...
                if component == "dynamic_weight_adjuster" and self.dynamic_weight_adjuster:
                    # 動的重み調整器パラメータ更新
                    success = self._update_weight_adjuster_parameters(adjustment, limited_factor)
                    
                elif component == "performance_monitor" and self.performance_monitor:
                    # 性能監視システムパラメータ更新
                    success = self._update_monitor_parameters(adjustment, limited_factor)
                    
                if success:
                    self.feedback_stats["automatic_improvements"] += 1
                    logger.info(f"✅ Applied parameter adjustment: {adjustment['reason']}")
                    
        except Exception as e:
            logger.error(f"Parameter adjustment application failed: {e}")

    def _update_weight_adjuster_parameters(self, adjustment: Dict[str, Any], factor: float) -> bool:
        """重み調整器パラメータ更新"""
        try:
            ...
            if hasattr(self.dynamic_weight_adjuster, 'learning_rate'):
                ...
                logger.info(f"🔧 Updated learning_rate: {old_value:.4f} → {new_value:.4f}")
                return True
    ...
    def _update_monitor_parameters(self, adjustment: Dict[str, Any], factor: float) -> bool:
        """監視システムパラメータ更新"""
        try:
            # パラメータ更新ロジック（実装は具体的なパラメータに依存）
            # ここでは例として閾値を調整
            if hasattr(self.performance_monitor, 'accuracy_threshold'):
                ...
                logger.info(f"🔧 Updated accuracy_threshold: {old_value:.4f} → {new_value:.4f}")
                return True
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            timeframe_stats = {}
            for timeframe in self.timeframes:
                correct_count = len(self.prediction_analysis[timeframe]["correct_predictions"])
                incorrect_count = len(self.prediction_analysis[timeframe]["incorrect_predictions"])
                total_predictions = correct_count + incorrect_count
                
                successful_trades = len(self.trade_analysis[timeframe]["successful_trades"])
                failed_trades = len(self.trade_analysis[timeframe]["failed_trades"])
                total_trades = successful_trades + failed_trades
                
                timeframe_stats[timeframe] = {
                    "prediction_accuracy": correct_count / total_predictions if total_predictions > 0 else 0,
