...
class ApiVersionManager:
    ...
    def check_ccxt_version_compatibility(self, exchange_id: str) -> Tuple[bool, str]:
        ...
        if not last_tested_version:
            ...
            return True, f"First time setup for {exchange_id} with CCXT {current_version}"
        ...
        try:
            ...
            if current_ver > tested_ver:
                ...
                if major_current == major_tested and minor_current <= minor_tested + 1:
                    self._update_exchange_info(exchange_id, current_version)
                    return True, f"Compatible version upgrade: {last_tested_version} -> {current_version}"
                else:
                    return False, f"Major version change detected: {last_tested_version} -> {current_version}. Manual verification required."
    ...
    def get_exchange_client_info(self, exchange_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            client_info = {
                ...
                "has_testnet": getattr(exchange_instance, 'urls', {}).get('test') is not None,
                "supported_methods": [
                    method for method in dir(exchange_instance)
                    if method.startswith('fetch_') or method.startswith('create_') or method.startswith('cancel_')
                ],
    ...
    def validate_api_compatibility(self, exchange_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result["checks"]["required_methods"] = {
                ...
                "message": "All required methods available" if not missing_methods else f"Missing: {missing_methods}"
            }
        ...
        validation_result["checks"]["configuration"] = {
            ...
            "message": "Exchange configuration found" if supported_info else "No configuration for this exchange",
    ...
    def get_outdated_exchanges(self, days_threshold: int = 30) -> list:
        ...
        for exchange_id, info in self.version_config.items():
            ...
            if last_checked:
                try:
                    last_check_date = datetime.fromisoformat(last_checked.replace('Z', '+00:00'))
                    if last_check_date < cutoff_date:
                        outdated.append({
                            "exchange_id": exchange_id,
                            "last_checked": last_checked,
                            "days_since_check": (datetime.now(timezone.utc) - last_check_date).days
                        })
                except ValueError:
                    # 無効な日付形式の場合も古いとみなす
                    outdated.append({
