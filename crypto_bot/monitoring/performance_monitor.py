...
class PerformanceMonitor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 統計検定用データ
        self.baseline_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=200))
        ...
        try:
            for timeframe, metrics in current_metrics.items():
                        ...
                        PerformanceAlert(
                            ...
                            context={
                                ...
                                "total_predictions": len(
                                    self.performance_history[f"{timeframe}_predictions"]
                                ),
                            },
                        )
                    )
                ...
                if metrics.max_drawdown < self.max_drawdown_threshold:
                    alert_level = (
                        AlertLevel.CRITICAL
                        if metrics.max_drawdown < self.max_drawdown_threshold * 1.5
                ...
                if metrics.consecutive_losses >= self.consecutive_loss_threshold:
                    alert_level = (
                        AlertLevel.CRITICAL
                        if metrics.consecutive_losses
                        >= self.consecutive_loss_threshold * 1.5
                        else AlertLevel.WARNING
                    )
                    alerts.append(
                        PerformanceAlert(
        ...
        try:
            ...
            for timeframe, metrics in current_metrics.items():
                ...
                if len(self.baseline_metrics[f"{timeframe}_accuracy"]) < 100:
                    self.baseline_metrics[f"{timeframe}_accuracy"].append(
                        metrics.accuracy
                    )
                    self.baseline_metrics[f"{timeframe}_win_rate"].append(
                        metrics.win_rate
                    )
                    self.baseline_metrics[f"{timeframe}_sharpe"].append(
                ...
                # 十分なデータがある場合のみ統計検定実行
                if (
                    len(self.recent_metrics[f"{timeframe}_accuracy"])
                    < self.min_samples_for_test
                ):
                    continue
                ...
                for metric_name in ["accuracy", "win_rate", "sharpe"]:
                    baseline_data = list(
                        self.baseline_metrics[f"{timeframe}_{metric_name}"]
                    )
                    recent_data = list(
                        self.recent_metrics[f"{timeframe}_{metric_name}"]
                    )[-self.degradation_window :]

                    if (
                        len(baseline_data) >= self.min_samples_for_test
                        and len(recent_data) >= self.min_samples_for_test
                    ):
                        try:
                            # Welchのt検定（分散が等しくない場合に対応）
                            t_stat, p_value = stats.ttest_ind(
                                recent_data, baseline_data, equal_var=False
                            )
                            ...
                            logger.warning(
                                f"Statistical test failed for {timeframe} {metric_name}: {test_error}"
                            )
    ...
    def _update_realtime_profitability(self, trade_record: Dict):
        ...
        try:
            ...
            recent_trades = [
                t for t in list(self.trade_history)[-20:] if t["timeframe"] == timeframe
            ]
    ...
    def _update_monitoring_statistics(
        self, current_metrics: Dict[str, PerformanceMetrics]
    ):
        """監視統計更新"""
        try:
            ...
            if not self.auto_stop_triggered:
                critical_metrics = sum(
                    ...
                    if (
                        metrics.accuracy < self.accuracy_threshold * 0.8
                        or metrics.win_rate < self.win_rate_threshold * 0.8
                    )
                )
        ...
        try:
            ...
            for timeframe in self.timeframes:
                for metric_name in ["accuracy", "win_rate", "sharpe"]:
                    ...
                    if (
                        baseline_key in self.baseline_metrics
                        and recent_key in self.recent_metrics
                        and len(self.baseline_metrics[baseline_key])
                        >= self.min_samples_for_test
                        and len(self.recent_metrics[recent_key])
                        >= self.min_samples_for_test
                    ):

                        baseline_data = list(self.baseline_metrics[baseline_key])
                        recent_data = list(self.recent_metrics[recent_key])[
                            -self.degradation_window :
                        ]
        ...
        try:
            ...
            for key in self.monitor_stats:
                if isinstance(self.monitor_stats[key], (int, float)):
                    self.monitor_stats[key] = (
                        0 if isinstance(self.monitor_stats[key], int) else 0.0
                    )
