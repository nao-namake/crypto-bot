...
class ClassificationMonitor(PerformanceMonitorBase):
    ...
    def get_current_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            if len(unique_classes) == 2:
                metrics.update({
                    "precision_class_0": precision_score(y_true, y_pred, pos_label=unique_classes[0], zero_division=0),
                    "precision_class_1": precision_score(y_true, y_pred, pos_label=unique_classes[1], zero_division=0),
                    "recall_class_0": recall_score(y_true, y_pred, pos_label=unique_classes[0], zero_division=0),
                    "recall_class_1": recall_score(y_true, y_pred, pos_label=unique_classes[1], zero_division=0)
                })
    ...
    def _check_alerts(self, current_metrics: Dict[str, float]):
        ...
        for alert in self.alert_thresholds:
            ...
            if alert.comparison == "below":
                ...
                alert_triggered = self._check_change_alert(alert.metric_name, alert.threshold)
    ...
    def _trigger_alert(self, alert: PerformanceAlert, current_value: float, 
        ...
        logger.warning(f"Performance alert: {alert.metric_name} = {current_value:.4f} "
