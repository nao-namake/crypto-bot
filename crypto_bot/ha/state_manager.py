...
class StateManager:
    ...
    def send_heartbeat(self) -> bool:
        ...
        try:
            doc_ref = self.firestore_client.collection("crypto_bot_instances").document(
    ...
    def elect_leader(self) -> bool:
        ...
        try:
            # 現在のリーダーをチェック
            leader_ref = self.firestore_client.collection(
                "crypto_bot_cluster"
            ).document("leader")
            ...
            def update_leader(transaction, leader_ref):
                ...
                if snapshot.exists:
                    ...
                    if last_hb and isinstance(last_hb, datetime):
                        if (
                            current_time - last_hb
                        ).total_seconds() < self.leader_timeout:
