...
class AggressiveRiskManager(RiskManager):
    ...
    def enhanced_kelly_position_sizing(
        ...
        logger.info(
            f"Enhanced Kelly sizing: win_rate={win_rate:.2f}, "
    ...
    def dynamic_position_sizing(
        ...
        **kwargs,
        ...
        # 安全性チェック
        max_safe_size = (
            balance * self.kelly_max_fraction / abs(entry_price - stop_price)
        ...
        info = {
            ...
            "risk_percentage": (final_size * abs(entry_price - stop_price))
            / balance
            * 100,
        }
    ...
    def add_trade_result(
        ...
        **kwargs,
        ...
        logger.debug(
            f"Added trade result: PnL={pnl:.2f} ({pnl_pct:.2%}), "
            f"confidence={confidence:.3f}"
        )
    ...
    def should_reduce_risk(self) -> bool:
        ...
        conditions = [
            ...
            len([r for r in metrics["recent_returns"] if r < -0.05])
            >= 3,  # 最近5取引で3回以上5%超損失
        ]
    ...
    def get_aggressive_multiplier(
        self, confidence: float, market_conditions: Optional[Dict] = None
    ) -> float:
        """
        積極的乗数を計算
