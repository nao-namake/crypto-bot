...
class MarketEnvironmentAnalyzer:
    ...
    def analyze_comprehensive_market_environment(
        ...
        try:
            ...
            # 6. 予測適応性スコア
            adaptability_score = self._calculate_prediction_adaptability(analysis_result)
            analysis_result["prediction_adaptability"] = adaptability_score
            
            # 7. 取引環境品質評価
            trading_environment = self._assess_trading_environment_quality(analysis_result)
    ...
    def _analyze_volatility_regime(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            if len(price_data) < self.volatility_lookback:
                return {"volatility_regime": "insufficient_data", "volatility_score": 0.5}
            ...
            medium_vol = returns.rolling(self.volatility_lookback).std().iloc[-1] * np.sqrt(365)
    ...
    def _determine_market_regime(
        ...
        try:
            ...
            returns = price_data["close"].pct_change().dropna()
            if len(returns) >= self.volatility_lookback:
                recent_vol = returns.rolling(self.volatility_lookback).std().iloc[-1] * np.sqrt(365)
            ...
            if len(price_data) >= self.trend_lookback:
                ...
                ma_long = price_data["close"].rolling(self.trend_lookback).mean().iloc[-1]
    ...
    def _analyze_trend_characteristics(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        """トレンド特性分析"""
        try:
            if len(price_data) < self.trend_lookback:
    ...
    def _analyze_liquidity_conditions(
        ...
        try:
            ...
            if len(price_data) >= 10:
                high_low_range = (price_data["high"] - price_data["low"]) / price_data["close"]
    ...
    def _calculate_market_stress_indicators(
        ...
        try:
            ...
            if len(price_data) >= 20:
                returns = price_data["close"].pct_change().dropna()
                vol_stress = min(returns.rolling(20).std().iloc[-1] * np.sqrt(365) / 0.05, 1.0)
    ...
    def _calculate_prediction_adaptability(self, analysis_result: Dict[str, Any]) -> float:
        """予測モデル適応性スコア計算"""
        try:
    ...
    def _assess_trading_environment_quality(
        ...
        try:
            # 各要素スコア収集
            vol_score = 1.0 - analysis_result.get("volatility_score", 0.5)  # 低ボラほど高品質
            trend_score = analysis_result.get("trend_strength", 0.5)
            liquidity_score = analysis_result.get("liquidity_score", 0.5)
            stress_score = 1.0 - analysis_result.get("overall_stress", 0.5)  # 低ストレスほど高品質
    ...
    def _update_analysis_state(self, analysis_result: Dict[str, Any]):
        ...
        try:
            ...
            if new_vol_regime != self.current_volatility_regime:
                ...
                logger.info(f"🔄 Volatility regime change: {self.current_volatility_regime} → {new_vol_regime}")
    ...
    def calculate_market_adaptation_weights(
        ...
        try:
            ...
            for timeframe, adjustment in weight_adjustments.items():
                final_weights[timeframe] = adjustment * stress_factor * adaptability_factor
...
def create_market_environment_analyzer(config: Dict[str, Any]) -> MarketEnvironmentAnalyzer:
    """
    市場環境解析システム作成
