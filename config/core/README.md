# config/core/ - システム基本設定 🚀 Phase 46完了（Phase 31.1-46統合）

## 🎯 役割・責任

システム全体で使用する基本設定を管理します。取引所接続、機械学習、戦略、リスク管理などの核となる設定ファイル群を提供し、全システムで一貫した動作を保証します。特に**feature_order.json**は全システムの特徴量定義における単一真実源、**features.yaml**は機能トグル管理、**thresholds.yaml**はPhase 46デイトレード特化設定の中心として機能します。

## 📈 Phase 46完了（2025年10月22日）

**🎯 Phase 46: デイトレード特化設定・スイングトレード設定削除**

**✅ Phase 46.2.2 デイトレード特化設定変更**:
- **TP/SL距離最適化**: thresholds.yaml（デイトレード特化完了）
  - **sl_min_distance_ratio: 0.02**（2.0%維持・デイトレード最適値）
    - 証拠金1万円で最大損失200円
    - BTC 15分足ボラティリティ対応
  - **tp_default_ratio: 2.0**（RR比2.0:1・デイトレード標準値）
    - 勝率33.3%以上で収益化可能
    - 15分足取引に最適化
  - **tp_min_profit_ratio: 0.04**（4.0%・細かく利益確定）
    - 市場ボラティリティ対応
    - デイトレード最適化完了

**✅ Phase 46.2.4 スイングトレード設定削除**:
- **トレーリングストップ設定削除**: thresholds.yaml lines 334-340（-7行）
  ```yaml
  # 削除前（Phase 42.2-43）:
  position_management:
    stop_loss:
  ...
  # Phase 46削除後:
  # トレーリングストップ設定完全削除（デイトレード特化）
  ```

**効果**:
- デイトレード特化設定完了（SL 2%・TP 4%・RR 2.0:1）
- スイングトレード機能完全削除（シンプル性優先）
- 個別TP/SL管理に特化（設定反映確認済み）
- 品質保証: 1,101テスト100%成功・68.93%カバレッジ達成

---

## 📈 Phase 42.4完了（2025年10月20日）

**🎯 Phase 42.4: TP/SL設定最適化・デイトレード対応・2025年市場ベストプラクティス準拠**

**✅ Phase 42.4完了**:
- **TP/SL距離固定値更新**: thresholds.yaml lines 367-387（デイトレード段階的最適化）
  - **sl_min_distance_ratio: 0.02**（2.0%・市場推奨3-8%の下限）
    - 証拠金1万円で最大損失200円
    - 市場データ: BTC日次ボラティリティ2-5%
  - **tp_default_ratio: 1.5**（RR比1.5:1維持・段階的最適化）
    - 勝率40%以上で収益化可能
    - 現行MLモデルF1スコア0.56-0.61に適合
  - **tp_min_profit_ratio: 0.03**（3.0%・細かく利益確定）
    - 市場ボラティリティ中間値対応
    - デイトレード最適化完了
...
- **Optuna最適化統合**: optimize_risk_management.py FIXED_TP_SL_PARAMS同期
  - Phase 40最適化対象外パラメータとして固定
  - デイトレード戦略確立後の最適化に備えた設計

- **ドキュメント完備**: 設定根拠・設計判断を詳細コメント化
  ```yaml
  # 設定根拠:
...
- **品質保証**: 1,164テスト100%成功・69.58%カバレッジ達成
- **Phase 46更新**: RR比1.5:1 → 2.0:1・TP 3.0% → 4.0%・デイトレード完全対応
