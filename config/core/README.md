...
### base.yaml - 基本設定（モード設定一元化対応）
**目的**: 全環境で共通の基本パラメータ、モード設定のデフォルト値

**🎯 モード設定一元化（2025年8月25日実装）**:
- **デフォルトモード**: `mode: paper`（安全な仮想取引）
- **本番運用**: 環境変数`MODE=live`で上書き
- **開発時**: コマンドライン`--mode live`で上書き
- **優先順位**: コマンドライン > 環境変数 > YAMLファイル
...
```yaml
# モード設定（一元化対応・優先順位制御）
mode: paper  # デフォルト：仮想取引（安全）
# 本番環境：環境変数 MODE=live で上書き  
# 開発時：--mode live コマンドライン引数で上書き
