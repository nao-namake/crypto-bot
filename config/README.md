...
**テスト品質**: 1,750テスト・71.59%カバレッジ・37特徴量（SHAP最適化）
...
- `feature_order.json`: 37特徴量順序定義（SHAP最適化）
...
### **設定ファイル変更時の注意**
- **feature_order.json**: 37特徴量順序変更は予測性能に重大影響・慎重に変更
...
- `src/core/config/feature_manager.py`: 37特徴量管理（feature_order.json単一参照）
...
- `src/ml/`: ProductionEnsemble（3モデルアンサンブル・37特徴量）
...
**Phase 65.12時点の設定管理システム**: 1,750テスト100%成功・71.59%カバレッジ・37特徴量（SHAP最適化）Strategy-Aware ML・6戦略（レンジ型4+トレンド型2）・1ファイル体系（thresholds.yaml）・79パラメータOptuna最適化・レジーム別TP/SL・固定金額TP・確定申告対応・週間レポート自動化により、24時間安定稼働中。
