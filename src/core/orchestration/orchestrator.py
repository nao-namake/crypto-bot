...
class TradingOrchestrator:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Phase 90β/90γ-②: trigger モード（Cloud Scheduler 駆動・min_instances=0）では
            # WebSocket 常駐不可。Phase 90β は cmdline_mode="trigger" で実装したが valid_modes
            # 不一致で EMERGENCY_STOP の原因に → Phase 90γ-② で env MODE 判定方式へ変更。
            # 設定 config.mode は "live" のまま（executor.py 等の判定整合性を維持）。
...
async def create_trading_orchestrator(
    ...
    try:
        ...
        from ...trading.execution import OrderStrategy, PositionRestorer, StopManager, TPSLManager
        ...
        tp_sl_manager = TPSLManager()  # Phase 64: TP/SL統合管理
        position_restorer = PositionRestorer()  # Phase 64: ポジション復元
        ...
        execution_service.inject_services(
            ...
            tp_sl_manager=tp_sl_manager,  # Phase 64: TP/SL統合管理
            position_restorer=position_restorer,  # Phase 64: ポジション復元
        )
        logger.info(
...
async def _get_actual_balance(config, logger) -> float:
    ...
    def _get_mode_balance(mode: str) -> float:
        ...
        # Phase 55.9: load_config()ではmode_balances属性が取得できないバグを修正
        if mode == "backtest":
            return get_threshold("mode_balances.backtest.initial_balance", 100000.0)
        elif mode == "paper":
            return get_threshold("mode_balances.paper.initial_balance", 100000.0)
        else:
            return get_threshold("mode_balances.live.initial_balance", 100000.0)
