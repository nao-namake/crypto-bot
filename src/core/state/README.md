# src/core/state/ - システム状態管理（Phase 52.4完了）

**Phase 52.4完了**のドローダウン状態永続化システム。
...
**主な変更**:
- `unified.yaml`パステンプレート対応（`local_path_template`・`gcs_path_template`）
- `get_config()`パターン適用によるハードコード値完全削除
- モード別パス生成の設定ファイル化完了

**更新ファイル**:
1. `config/core/unified.yaml`: パステンプレート設定追加
   ```yaml
   risk:
     drawdown_manager:
       persistence:
...
2. `src/core/state/drawdown_persistence.py`: `get_config()`パターン適用
3. `src/trading/risk/drawdown.py`: モード別パステンプレート使用
4. `scripts/management/run_safe.sh`: モード別パス対応
...
AI自動取引システムの実行状態を永続化・管理。各モード（paper/live/backtest）の状態を完全分離し、本番環境への影響を防止。ローカルファイル・Cloud Storage両対応の統一インターフェース提供。
...
**状態永続化システム（Phase 49完了）**: モード別状態分離により、ペーパーモードの実験が本番環境に影響しない安全な構造を実現。ローカル・Cloud Storage両対応で運用環境に最適化。DrawdownPersistence抽象クラスによる統一インターフェース提供。