...
class ExecutionService:
    ...
    async def _execute_live_trade(self, evaluation: TradeEvaluation) -> ExecutionResult:
        ...
        try:
            ...
            # Phase 90σ: 計測 slippage は実 PnL（Phase 69.7 bitbank API 直 PnL）に内包され
            # ライブ分析の損益で把握できるため、エントリー時の slippage 計算・記録を撤去
            # （Phase 62.16 のスリッページ計測経路は Phase 69.7 移行で形骸化していた）。
