...
**使用場面**:
- デプロイ後のインフラ基盤稼働確認
- Cloud Run・Secret Manager・Discord通知の健全性チェック
- Container安定性・API接続問題の早期検知
- 証拠金維持率80%遵守確認（Phase 49完了機能）
- 定期的なシステム基盤診断（毎時推奨）
...
- 確定申告対応システム（Phase 47）・Discord週間レポート（Phase 48）
...
- 📨 **Discord通知**: Webhook接続・通知システム（週間レポート含む）
- 🔥 **Container安定性**: プロセス管理・async/await問題
- 💰 **API接続**: bitbank API認証・データ取得・証拠金維持率取得
- ⚠️ **エラー監視**: ログ分析・警告検出
- 🛡️ **取引阻害エラー**: NoneType エラー・API応答異常・取引実行エラー検出
- 📊 **Phase 49機能**: 証拠金維持率80%遵守・4層検証システム稼働確認
...
# 2-2. 証拠金維持率80%確認（Phase 50.4完全修正）
echo ""
echo "📊 証拠金維持率80%遵守確認（Phase 50.4: API直接取得方式）"
MARGIN_RATIO_API_SUCCESS=$(count_logs_since_deploy "textPayload:\"Phase 50.4: API証拠金維持率取得成功\"" 10)
MARGIN_RATIO_500_FALLBACK=$(count_logs_since_deploy "textPayload:\"500%固定返却\" OR textPayload:\"500.0%\"" 10)
MARGIN_RATIO_80_REJECT=$(count_logs_since_deploy "textPayload:\"証拠金維持率.*80%未満\" OR textPayload:\"証拠金不足\"" 10)

echo "   Phase 50.4 API直接取得成功: $MARGIN_RATIO_API_SUCCESS回"
echo "   500%固定返却（旧問題）: $MARGIN_RATIO_500_FALLBACK回"
echo "   80%未満エントリー拒否: $MARGIN_RATIO_80_REJECT回"

if [ $MARGIN_RATIO_API_SUCCESS -gt 0 ] && [ $MARGIN_RATIO_500_FALLBACK -eq 0 ]; then
    echo "✅ 証拠金維持率取得: 完全修正済み（/user/margin/status API直接取得稼働中）"
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))

    if [ $MARGIN_RATIO_80_REJECT -gt 0 ]; then
        echo "   ℹ️ 80%未満エントリー拒否: ${MARGIN_RATIO_80_REJECT}回（正常動作）"
    fi
elif [ $MARGIN_RATIO_500_FALLBACK -gt 0 ]; then
    echo "❌ 証拠金維持率: Phase 50.4前の500%固定問題が残存（Critical）"
    CRITICAL_ISSUES=$((CRITICAL_ISSUES + 2))
else
    echo "⚠️ 証拠金維持率取得: 未確認（Phase 50.4機能が稼働していない可能性）"
    WARNING_ISSUES=$((WARNING_ISSUES + 1))
fi
...
# 3-2. 4層検証システム稼働確認（Phase 49.15）
echo ""
echo "🔍 4層検証システム稼働確認（Phase 49機能）"
VALIDATION_DOCKERFILE=$(count_logs_since_deploy "textPayload:\"Dockerfile整合性チェック\" OR textPayload:\"Layer 1.*OK\"" 5)
VALIDATION_FEATURES=$(count_logs_since_deploy "textPayload:\"特徴量順序チェック\" OR textPayload:\"Layer 2.*OK\"" 5)
VALIDATION_STRATEGIES=$(count_logs_since_deploy "textPayload:\"戦略信号チェック\" OR textPayload:\"Layer 3.*OK\"" 5)
VALIDATION_MODULES=$(count_logs_since_deploy "textPayload:\"モジュールインポートチェック\" OR textPayload:\"Layer 4.*OK\"" 5)

echo "   Layer 1 (Dockerfile整合性): $VALIDATION_DOCKERFILE回"
echo "   Layer 2 (特徴量順序): $VALIDATION_FEATURES回"
echo "   Layer 3 (戦略信号): $VALIDATION_STRATEGIES回"
echo "   Layer 4 (モジュールインポート): $VALIDATION_MODULES回"

TOTAL_VALIDATION=$((VALIDATION_DOCKERFILE + VALIDATION_FEATURES + VALIDATION_STRATEGIES + VALIDATION_MODULES))

if [ $TOTAL_VALIDATION -ge 4 ]; then
    echo "✅ 4層検証システム: 稼働中（全層確認済み）"
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
elif [ $TOTAL_VALIDATION -gt 0 ]; then
    echo "⚠️ 4層検証システム: 部分稼働（一部層のみ確認）"
    WARNING_ISSUES=$((WARNING_ISSUES + 1))
else
    echo "ℹ️ 4層検証システム: ログ未確認（起動時のみ実行される可能性）"
fi

# 3-3. 外部API特徴量取得状況確認（Phase 50.3・Phase 50.6）
echo ""
echo "🌐 外部API特徴量取得状況確認（Phase 50.3・Phase 50.6）"
EXTERNAL_API_SUCCESS=$(count_logs_since_deploy "textPayload:\"外部API指標取得成功\"" 10)
USDJPY_SUCCESS=$(count_logs_since_deploy "textPayload:\"USD/JPY: \"" 10)
NIKKEI_SUCCESS=$(count_logs_since_deploy "textPayload:\"日経平均: \"" 10)
USBOND_SUCCESS=$(count_logs_since_deploy "textPayload:\"米10年債利回り: \"" 10)
FEARGREED_SUCCESS=$(count_logs_since_deploy "textPayload:\"Fear & Greed: \"" 10)
EXTERNAL_API_ERROR=$(count_logs_since_deploy "textPayload:\"外部API取得エラー\" OR textPayload:\"asyncio.to_thread\" OR textPayload:\"yfinance.*エラー\"" 10)

echo "   外部API全体成功: $EXTERNAL_API_SUCCESS回"
echo "   USD/JPY取得成功: $USDJPY_SUCCESS回"
echo "   日経平均取得成功: $NIKKEI_SUCCESS回"
echo "   米10年債取得成功: $USBOND_SUCCESS回"
echo "   Fear & Greed取得成功: $FEARGREED_SUCCESS回"
echo "   外部APIエラー: $EXTERNAL_API_ERROR回"

# 外部API成功率計算（4指標 × 取得回数）
TOTAL_API_SUCCESS=$((USDJPY_SUCCESS + NIKKEI_SUCCESS + USBOND_SUCCESS + FEARGREED_SUCCESS))
EXPECTED_API_SUCCESS=$((EXTERNAL_API_SUCCESS * 4))

if [ $EXTERNAL_API_SUCCESS -gt 0 ] && [ $TOTAL_API_SUCCESS -ge $EXTERNAL_API_SUCCESS ]; then
    # macOS対応: 成功率計算
    if [ $EXPECTED_API_SUCCESS -gt 0 ]; then
        API_SUCCESS_RATE=$(python3 -c "print(int(($TOTAL_API_SUCCESS / $EXPECTED_API_SUCCESS) * 100))" 2>/dev/null || echo "0")
    else
        API_SUCCESS_RATE=0
    fi

    if [ $API_SUCCESS_RATE -ge 75 ]; then
        echo "✅ 外部API特徴量: 正常稼働（成功率${API_SUCCESS_RATE}%・Phase 50.6修正効果確認）"
        NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
    else
        echo "⚠️ 外部API特徴量: 部分稼働（成功率${API_SUCCESS_RATE}%・一部API障害の可能性）"
        WARNING_ISSUES=$((WARNING_ISSUES + 1))
    fi
elif [ $EXTERNAL_API_ERROR -gt 5 ]; then
    echo "❌ 外部API特徴量: 多数エラー（${EXTERNAL_API_ERROR}回・Phase 50.6修正未適用の可能性）"
    CRITICAL_ISSUES=$((CRITICAL_ISSUES + 1))
else
    echo "ℹ️ 外部API特徴量: 未確認（取引サイクル未実行の可能性）"
fi
...
if [ $NONETYPE_ERROR_COUNT -eq 0 ]; then
    ...
    echo "❌ NoneType エラー: 多発 ($NONETYPE_ERROR_COUNT回・対策必要)"
    CRITICAL_ISSUES=$((CRITICAL_ISSUES + 1))
fi
...
**🎯 重要**: このファイルは基盤システムのみをチェックします。Bot固有の機能（70特徴量・5戦略・ML統合）については **02_Bot機能診断.md** を実行してください。

**最終更新**: 2025年10月30日 - Phase 50完了対応（証拠金維持率80%完全修正・外部API特徴量取得・4段階Graceful Degradation）