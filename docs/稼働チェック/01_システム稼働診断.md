...
# 2-2. 証拠金維持率80%確認（Phase 49.5）
echo ""
echo "📊 証拠金維持率80%遵守確認（Phase 49機能）"
MARGIN_RATIO_SUCCESS=$(count_logs_since_deploy "textPayload:\"API直接取得成功: 保証金維持率\"" 10)
MARGIN_RATIO_80_REJECT=$(count_logs_since_deploy "textPayload:\"証拠金維持率.*80%未満\" OR textPayload:\"証拠金不足\"" 10)

echo "   維持率取得成功: $MARGIN_RATIO_SUCCESS回"
echo "   80%未満エントリー拒否: $MARGIN_RATIO_80_REJECT回"

if [ $MARGIN_RATIO_SUCCESS -gt 0 ]; then
    echo "✅ 証拠金維持率取得: 正常（bitbank API直接取得稼働中）"
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
    ...
    echo "⚠️ 証拠金維持率取得: 未確認（Phase 49機能が稼働していない可能性）"
    WARNING_ISSUES=$((WARNING_ISSUES + 1))
fi
...
**🎯 重要**: このファイルは基盤システムのみをチェックします。Bot固有の機能（55特徴量・5戦略・ML統合）については **02_Bot機能診断.md** を実行してください。

**最終更新**: 2025年10月25日 - Phase 49完了対応（証拠金維持率80%・4層検証システム）