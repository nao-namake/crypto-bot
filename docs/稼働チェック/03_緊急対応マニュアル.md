...
- 55特徴量Strategy-Aware ML・証拠金維持率80%確実遵守
- SELL Only問題解決・TradeTracker統合・4層検証システム実装
...
### 🌐 外部API障害時の対応手順（Phase 50.3・Phase 50.6）

**問題**: 外部API特徴量取得失敗・70特徴量システムが62特徴量にフォールバック継続

```bash
echo "🌐 外部API障害時の対応手順（Phase 50.3・Phase 50.6）"

# 1. 外部API障害状況確認
echo "外部API障害状況確認..."
EXTERNAL_API_ERROR=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND (textPayload:\"外部API取得エラー\" OR textPayload:\"yfinance.*エラー\" OR textPayload:\"Fear.*Greed.*エラー\")" --limit=20 --format="value(textPayload)" | grep -c . || echo "0")
LEVEL2_FALLBACK=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND (textPayload:\"Level 2\" OR textPayload:\"62特徴量\")" --limit=15 --format="value(textPayload)" | grep -c . || echo "0")

echo "外部APIエラー数: $EXTERNAL_API_ERROR回"
echo "Level 2フォールバック: $LEVEL2_FALLBACK回"

if [ $EXTERNAL_API_ERROR -gt 10 ] || [ $LEVEL2_FALLBACK -gt 5 ]; then
    echo "❌ 外部API障害検出 - 対応開始"

    # 2. Phase 50.6修正適用確認
    echo "Phase 50.6修正適用確認..."
    ASYNCIO_TO_THREAD=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"asyncio.to_thread\"" --limit=5 --format="value(textPayload)" | grep -c . || echo "0")

    if [ $ASYNCIO_TO_THREAD -eq 0 ]; then
        echo "⚠️ Phase 50.6修正未適用の可能性 - 最新リビジョンデプロイ推奨"
        echo "  → GitHub Actions CI/CDで最新コミットをデプロイしてください"
    else
        echo "✅ Phase 50.6修正適用済み - 外部API側の一時的障害の可能性"
    fi

    # 3. Level 2フォールバック動作確認
    echo ""
    echo "Level 2（62特徴量）フォールバック動作確認..."
    echo "最新Level 2フォールバックログ:"
    gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND (textPayload:\"Level 2\" OR textPayload:\"62特徴量\")" --limit=3 --format="value(timestamp.date(tz='Asia/Tokyo'),textPayload)"

    # 4. 対応方針
    echo ""
    echo "📊 外部API障害対応方針:"
    echo "   ✅ システム継続稼働: 62特徴量で自動フォールバック稼働中"
    echo "   ✅ 取引機能: 正常（外部API障害でも取引判断可能）"
    echo "   📈 性能影響: 軽微（70→62特徴量、-8個のマクロ経済指標のみ除外）"
    echo ""
    echo "   🔍 推奨アクション:"
    echo "   1. 外部API復旧待ち（Yahoo Finance・Alternative.me）"
    echo "   2. 1時間後に 01_システム稼働診断.md で外部API復旧確認"
    echo "   3. Level 1（70特徴量）復帰確認"
    echo ""
    echo "   ⚠️ Level 2継続が24時間以上の場合:"
    echo "   → 外部APIプロバイダー障害情報確認推奨"
    echo "   → Yahoo Finance Status: https://status.yahoo.com/"

elif [ $EXTERNAL_API_ERROR -eq 0 ] && [ $LEVEL2_FALLBACK -eq 0 ]; then
    echo "✅ 外部API: 正常稼働中（70特徴量システム稼働）"
    echo "対応不要"
else
    echo "ℹ️ 外部API: 軽微エラー（${EXTERNAL_API_ERROR}回）"
    echo "監視継続 - 緊急対応不要"
fi

echo ""
echo "次回確認: 1時間後に 01_システム稼働診断.md で外部API状況確認"
```
...
**最終更新**: 2025年10月25日 - Phase 49完了システム対応