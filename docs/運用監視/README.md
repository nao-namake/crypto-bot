...
| **02_Bot機能診断.md** | Bot固有機能のエントリーシグナルチェック | 10分 | 特徴量生成・2段階Graceful Degradation・戦略統合・Registry Pattern・ML統合・取引阻害エラー・TradeTracker統合 |
...
- **対象**: Bot固有のコア機能
- **重点**:
  - 特徴量生成完全性（feature_order.jsonに定義）
  - 2段階Graceful Degradation（Full→Basic→Dummy）
  - 戦略動的信頼度（strategies.yamlに定義・小数点第3位変動確認）
  - Registry Pattern動作確認（動的戦略管理基盤）
  - ML予測統合（thresholds.yamlに閾値定義）
  - 取引機能（最小ロット・TP/SL・ATR・クールダウン・Atomic Entry Pattern）
...
**最終更新**: 2025年11月15日 - Phase 52.4完了対応（設定一元化・ハードコード撲滅・Single Source of Truth参照）
