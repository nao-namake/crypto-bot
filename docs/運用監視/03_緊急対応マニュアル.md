...
**使用場面**:
- 01_稼働率診断.md・02_機能診断.mdで問題検出時の即座修正
- 99%稼働率未達時の緊急対応
- Secret Manager権限・Container問題・Discord通知停止対応
- Phase 53.8特有問題対応（Python 3.11・RandomForest・15分足データ）
...
**Phase 56.2完了後のシステム状態**（2025/11/30）:
- **Python 3.11.14**（GCP gVisor互換性・99%稼働率達成）
- **RandomForest n_jobs=1**（multiprocessingクラッシュ解消）
- **numpy 2.3.5 + pandas 2.3.3**（pandas-ta-classic 0.3.37互換性）
- **15分足データ取得最適化**（エラーコード10000完全消失）
- **55特徴量システム**（49基本+6戦略シグナル）
- **2段階Graceful Degradation**（ensemble_full.pkl → ensemble_basic.pkl → DummyModel）
- **Phase 56.2: 自動タイムアウト無効化**（main.pyの15分タイマー削除・継続稼働優先）
- **Phase 56.2: docker-entrypoint.sh自動再起動**（Container exit(1)削減・最大1000回再起動）
...
### Phase 53.8特有問題（新規追加）
- 🐍 **99%稼働率未達**: 稼働率90%未満時の段階的対応
- 🐍 **Python 3.11環境問題**: Python 3.13特有エラー残存・環境不整合
- 🌲 **RandomForest n_jobs問題**: multiprocessingクラッシュ・n_jobs=-1設定残存
- 📊 **15分足データ取得エラー**: エラーコード10000発生・200件未満取得
- 📦 **numpy 2.x互換性問題**: pandas-ta-classic互換性エラー
- 🔄 **ロールバック手順**: Phase 53.8 → 旧バージョン戻し
...
- **[01_稼働率診断.md](./01_稼働率診断.md)** - 99%稼働率目標確認（事前チェック）
- **[02_機能診断.md](./02_機能診断.md)** - インフラ基盤・Bot機能診断（詳細分析）
...
## 🚨 Phase 53.8特有問題対応（新規追加）

### 🐍 99%稼働率未達時の段階的対応

**問題**: 01_稼働率診断.mdで90%未満検出・Phase 53.8目標未達

```bash
echo "🐍 99%稼働率未達時の段階的対応"

# 1. 現在の稼働率確認
CONTAINER_STARTS_24H=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"Container\" AND textPayload:\"starting\" AND timestamp>=\"$(python3 -c "import datetime; print((datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1)).strftime('%Y-%m-%dT%H:%M:%S.%fZ'))")\"" --limit=100 --format=json | jq length 2>/dev/null || echo "0")

CONTAINER_CRASHES_24H=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"Container called exit(1)\" AND timestamp>=\"$(python3 -c "import datetime; print((datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1)).strftime('%Y-%m-%dT%H:%M:%S.%fZ'))")\"" --limit=50 --format=json | jq length 2>/dev/null || echo "0")

echo "24時間Container起動: ${CONTAINER_STARTS_24H}回"
echo "24時間クラッシュ: ${CONTAINER_CRASHES_24H}回"

# 2. 段階的対応判定
if [ "$CONTAINER_CRASHES_24H" -gt 10 ]; then
    echo "❌ クラッシュ多発（${CONTAINER_CRASHES_24H}回） - Phase 53.8修正確認"

    # RandomForest n_jobs確認
    RF_CRASH=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"RandomForest\" AND textPayload:\"multiprocessing\"" --limit=20 --format=json | jq length 2>/dev/null || echo "0")

    if [ "$RF_CRASH" -gt 0 ]; then
        echo "🌲 RandomForest multiprocessingクラッシュ検出: ${RF_CRASH}回"
        echo "   → Phase 53.8修正（n_jobs=1）が反映されていない可能性"
        echo "   → セクション: RandomForest n_jobs問題対応 を実行"
    fi

    # Python環境確認
    PYTHON_313=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"Python 3.13\"" --limit=10 --format=json | jq length 2>/dev/null || echo "0")

    if [ "$PYTHON_313" -gt 0 ]; then
        echo "🐍 Python 3.13検出: ${PYTHON_313}回"
        echo "   → Python 3.11移行が完了していない可能性"
        echo "   → セクション: Python 3.11環境問題対応 を実行"
    fi

elif [ "$CONTAINER_STARTS_24H" -gt 5 ]; then
    echo "⚠️ 再起動頻発（${CONTAINER_STARTS_24H}回） - 軽微な問題"
    echo "   → Container問題緊急対応セクションで詳細診断"
else
    echo "✅ Container安定性正常 - 他要因調査必要"
    echo "   → 02_機能診断.md で詳細分析"
fi

echo ""
echo "修正後は必ず 01_稼働率診断.md で効果確認"
```

### 🌲 RandomForest n_jobs問題対応

**問題**: RandomForest multiprocessingクラッシュ・n_jobs=-1設定残存

```bash
echo "🌲 RandomForest n_jobs問題対応（Phase 53.8重要修正）"

# 1. クラッシュ確認
RF_CRASH=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"RandomForest\" AND (textPayload:\"multiprocessing\" OR textPayload:\"n_jobs=-1\" OR textPayload:\"fork\")" --limit=20 --format=json | jq length 2>/dev/null || echo "0")

echo "RandomForest multiprocessingクラッシュ: ${RF_CRASH}回"

if [ "$RF_CRASH" -gt 0 ]; then
    echo "❌ Phase 53.8修正（n_jobs=1）が反映されていない"
    echo ""
    echo "🔧 対応手順:"
    echo "1. ローカル環境でMLモデル再訓練（n_jobs=1設定確認）"
    echo "   python3 scripts/ml/create_ml_models.py --n-classes 3 --threshold 0.005 --optimize --n-trials 50 --verbose"
    echo ""
    echo "2. モデルファイル確認"
    echo "   ls -lh models/production/ensemble_*.pkl"
    echo ""
    echo "3. Git commit & push（CI/CD自動デプロイ）"
    echo "   git add models/production/"
    echo "   git commit -m \"fix: RandomForest n_jobs=1モデル再訓練（Phase 53.8）\""
    echo "   git push origin main"
    echo ""
    echo "4. GCP デプロイ確認（約10分後）"
    echo "   gcloud run revisions list --service=crypto-bot-service-prod --region=asia-northeast1 --limit=3"
    echo ""
    echo "5. クラッシュ消失確認（30分後）"
    echo "   bash docs/稼働チェック/01_稼働率診断.sh"
else
    echo "✅ RandomForest n_jobs=1正常稼働"
fi
```

### 🐍 Python 3.11環境問題対応

**問題**: Python 3.13特有エラー残存・Python 3.11移行未完了

```bash
echo "🐍 Python 3.11環境問題対応"

# 1. Python環境確認
PYTHON_313=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"Python 3.13\"" --limit=10 --format=json | jq length 2>/dev/null || echo "0")

PYTHON_311=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"Python 3.11\"" --limit=10 --format=json | jq length 2>/dev/null || echo "0")

echo "Python 3.13検出: ${PYTHON_313}回"
echo "Python 3.11検出: ${PYTHON_311}回"

if [ "$PYTHON_313" -gt 0 ]; then
    echo "❌ Python 3.13混在検出 - Phase 53.8移行未完了"
    echo ""
    echo "🔧 対応手順:"
    echo "1. Dockerfileバージョン確認"
    echo "   cat Dockerfile | grep \"FROM python\""
    echo "   # 期待値: FROM python:3.11.14-slim"
    echo ""
    echo "2. requirements.txt確認"
    echo "   cat requirements.txt | grep \"numpy\\|pandas\\|scikit-learn\""
    echo "   # 期待値: numpy>=2.0.0, pandas>=2.2.0, scikit-learn==1.5.2"
    echo ""
    echo "3. 再デプロイ強制実行"
    echo "   gcloud run deploy crypto-bot-service-prod --source . --region=asia-northeast1"
    echo ""
    echo "4. Python 3.11確認（デプロイ10分後）"
    echo "   gcloud logging read \"textPayload:\\\"Python 3.11\\\"\" --limit=5"
elif [ "$PYTHON_311" -gt 0 ]; then
    echo "✅ Python 3.11環境正常"
else
    echo "ℹ️ Python環境ログ未確認（Container起動直後の可能性）"
fi

# 2. Python 3.13特有エラー確認
PANDAS_ABCINDEX=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"ABCIndex\"" --limit=20 --format=json | jq length 2>/dev/null || echo "0")

if [ "$PANDAS_ABCINDEX" -gt 0 ]; then
    echo "❌ pandas ABCIndexエラー検出: ${PANDAS_ABCINDEX}回（Python 3.13特有）"
    echo "   → Python 3.11移行で解消されるべきエラー"
    echo "   → 上記対応手順で再デプロイ実行"
fi
```

### 📊 15分足データ取得エラー対応（Phase 53.8.2）

**問題**: エラーコード10000発生・200件未満取得

```bash
echo "📊 15分足データ取得エラー対応（Phase 53.8.2修正）"

# 1. エラーコード10000確認
ERROR_10000=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND (textPayload:\"エラーコード.*10000\" OR textPayload:\"code.*10000\")" --limit=20 --format=json | jq length 2>/dev/null || echo "0")

echo "エラーコード10000: ${ERROR_10000}回"

if [ "$ERROR_10000" -gt 0 ]; then
    echo "❌ Phase 53.8.2修正（days_ago=1）が反映されていない"
    echo ""
    echo "🔧 対応手順:"
    echo "1. データ取得コード確認"
    echo "   cat src/data/bitbank_client.py | grep \"days_ago\""
    echo "   # 期待値: days_ago=1（Phase 53.8.2修正）"
    echo ""
    echo "2. 修正内容確認"
    echo "   git log --oneline --grep=\"15分足\" -5"
    echo "   # 期待値: Phase 53.8.2 commit存在"
    echo ""
    echo "3. 再デプロイ"
    echo "   git push origin main  # CI/CD自動デプロイ"
    echo ""
    echo "4. エラー消失確認（デプロイ30分後）"
    echo "   bash docs/稼働チェック/02_機能診断.sh"
else
    echo "✅ エラーコード10000ゼロ（Phase 53.8.2修正成功）"
fi

# 2. 15分足データ件数確認
DATA_COUNT=$(gcloud logging read "resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"crypto-bot-service-prod\" AND textPayload:\"15分足\" AND textPayload:\"件\"" --limit=5 --format="value(textPayload)" 2>/dev/null)

echo ""
echo "最新15分足データ取得ログ:"
echo "$DATA_COUNT"

if echo "$DATA_COUNT" | grep -q "200件"; then
    echo "✅ 15分足200件取得正常"
else
    echo "⚠️ 15分足データ件数要確認"
fi
...
**問題**: Phase 53.8修正後も99%稼働率未達・重大な問題発生

```bash
echo "🔄 ロールバック手順（Phase 53.8 → 旧バージョン戻し）"
echo ""
echo "⚠️ 警告: この操作はPhase 53.8の全修正を戻します"
echo ""
read -p "本当にロールバックしますか？ (yes/no): " CONFIRM

if [ "$CONFIRM" = "yes" ]; then
    echo "🔄 Phase 53.8ロールバック開始..."

    # 1. Phase 53.7最終コミット特定
    PHASE_537_COMMIT=$(git log --oneline --grep="Phase 53.7" -1 --format="%H" 2>/dev/null)

    if [ -n "$PHASE_537_COMMIT" ]; then
        echo "Phase 53.7最終コミット: $PHASE_537_COMMIT"

        # 2. ロールバック実行
        echo "Git revert実行..."
        git revert --no-commit HEAD...$PHASE_537_COMMIT
        git commit -m "revert: Phase 53.8ロールバック → Phase 53.7復帰（緊急対応）"
        ...
        echo ""
        echo "📋 次のステップ:"
        echo "1. CI/CDデプロイ完了待機（約10分）"
        echo "2. Phase 53.7環境確認"
        echo "   gcloud run revisions list --service=crypto-bot-service-prod --limit=3"
        echo "3. 稼働率確認（デプロイ24時間後）"
        echo "   bash docs/稼働チェック/01_稼働率診断.sh"
        echo "4. Phase 53.8問題原因分析・再修正計画"
    else
        echo "❌ Phase 53.7コミット未検出 - 手動ロールバック必要"
        echo "   git log --oneline -20  # コミット履歴確認"
    fi
else
    echo "ℹ️ ロールバック中断"
fi
...
**🎯 重要**: このマニュアルは緊急時の迅速な対応を目的としています。根本原因の特定には 01_稼働率診断.md・02_機能診断.md と併用してください。

**Phase 53.8特有問題**: 99%稼働率未達・Python 3.11環境問題・RandomForest n_jobs問題・15分足データエラーに対応しています。

**最終更新**: 2025年11月30日 - Phase 56.2完了対応（自動タイムアウト無効化・docker-entrypoint.sh自動再起動・99%稼働率達成）