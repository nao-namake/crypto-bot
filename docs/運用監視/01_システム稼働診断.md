...
- 🎯 **Atomic Entry Pattern**: Entry/TP/SL一体性保証・ロールバック動作確認（Phase 51.6）
- 📊 **Market Regime Classification**: 市場状況分類・動的戦略選択確認（Phase 51.1-51.4）
...
# 3-2. Atomic Entry Pattern動作確認（Phase 51.6）
echo ""
echo "🎯 Atomic Entry Pattern動作確認（Phase 51.6）"
ATOMIC_ENTRY_SUCCESS=$(count_logs_since_deploy "textPayload:\"Phase 51.6: Atomic Entry完了\"" 10)
ATOMIC_ROLLBACK_COUNT=$(count_logs_since_deploy "textPayload:\"Phase 51.6: ロールバック実行\" OR textPayload:\"Atomic Entry rollback\"" 10)
TP_PLACEMENT_RETRY=$(count_logs_since_deploy "textPayload:\"TP配置リトライ\" OR textPayload:\"_place_tp_with_retry\"" 10)
SL_PLACEMENT_RETRY=$(count_logs_since_deploy "textPayload:\"SL配置リトライ\" OR textPayload:\"_place_sl_with_retry\"" 10)

echo "   Atomic Entry成功: $ATOMIC_ENTRY_SUCCESS回"
echo "   ロールバック実行: $ATOMIC_ROLLBACK_COUNT回"
echo "   TP配置リトライ: $TP_PLACEMENT_RETRY回"
echo "   SL配置リトライ: $SL_PLACEMENT_RETRY回"

if [ $ATOMIC_ENTRY_SUCCESS -gt 0 ] && [ $ATOMIC_ROLLBACK_COUNT -eq 0 ]; then
    echo "✅ Atomic Entry Pattern: 正常稼働（Entry/TP/SL一体性保証・ロールバック不要）"
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
elif [ $ATOMIC_ENTRY_SUCCESS -gt 0 ] && [ $ATOMIC_ROLLBACK_COUNT -le 2 ]; then
    echo "⚠️ Atomic Entry Pattern: 軽微ロールバック（${ATOMIC_ROLLBACK_COUNT}回・監視推奨）"
    WARNING_ISSUES=$((WARNING_ISSUES + 1))
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
elif [ $ATOMIC_ROLLBACK_COUNT -gt 5 ]; then
    echo "❌ Atomic Entry Pattern: 頻繁ロールバック（${ATOMIC_ROLLBACK_COUNT}回・TP/SL配置問題）"
    CRITICAL_ISSUES=$((CRITICAL_ISSUES + 1))
else
    echo "ℹ️ Atomic Entry Pattern: 未確認（エントリー未実行の可能性）"
fi

# 3-3. Market Regime Classification動作確認（Phase 51.1-51.4）
echo ""
echo "📊 Market Regime Classification動作確認（Phase 51.1-51.4）"
REGIME_CLASSIFICATION=$(count_logs_since_deploy "textPayload:\"市場状況:\" OR textPayload:\"RegimeType\"" 10)
...
echo "   市場状況分類実行: $REGIME_CLASSIFICATION回"
echo "   TIGHT_RANGE検出: $TIGHT_RANGE_COUNT回"
echo "   NORMAL_RANGE検出: $NORMAL_RANGE_COUNT回"
echo "   TRENDING検出: $TRENDING_COUNT回"
echo "   HIGH_VOLATILITY検出: $HIGH_VOLATILITY_COUNT回"
...
if [ $REGIME_CLASSIFICATION -gt 0 ] && [ $TOTAL_REGIME_DETECTED -gt 0 ]; then
    echo "✅ Market Regime Classification: 正常稼働（4段階市場状況分類動作中）"
    NORMAL_CHECKS=$((NORMAL_CHECKS + 1))
elif [ $REGIME_CLASSIFICATION -gt 0 ]; then
    echo "⚠️ Market Regime Classification: 部分稼働（分類未確定状態の可能性）"
    WARNING_ISSUES=$((WARNING_ISSUES + 1))
else
    echo "ℹ️ Market Regime Classification: 未確認（取引サイクル未実行の可能性）"
fi
...
**🎯 重要**: このファイルは基盤システムのみをチェックします。Bot固有の機能（55特徴量・6戦略・ML統合）については **02_Bot機能診断.md** を実行してください。

**最終更新**: 2025年11月08日 - Phase 51.7完了対応（Atomic Entry Pattern・Market Regime Classification・Dynamic Strategy Management）