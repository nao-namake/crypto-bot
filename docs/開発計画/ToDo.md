...
| 項目 | 値 |
|------|-----|
| 最新成果 | Phase 65.16完了: Maker約定修正 + ML Signal Recovery |
...
### Phase 66: ライブ取引再開 — 3つの根本原因修正 🔧進行中

**目的**: Phase 65.16デプロイ後24時間で取引ゼロ → 3つの根本原因を特定・修正

| 修正 | 内容 | 状態 |
|------|------|------|
| **修正1: HOLD信頼度** | `SignalBuilder.create_hold_signal()`のconfidence=0.5ハードコード修正。ATRBased(5箇所)+BBReversal(1箇所)でconfig値を使用するよう変更 | ✅ 完了 |
| **修正2: レジーム分類** | EMA slope閾値 0.01→0.003に緩和。ADX>25でもEMA slopeが厳しすぎてtrending未判定だった問題を修正 | ✅ 完了 |
| **バックテスト検証(修正1+2)** | 180日分: PF 1.82、勝率85.0%、DD 0.97%、535件（取引発生を確認） | ✅ 完了 |
| **修正3: ML再学習** | 最新180日分データで再学習（Optuna n-trials=50）。min_ml_confidence 0.55→0.45、recovery_cap 0.35→0.30 | ✅ 完了 |
| **バックテスト検証(全修正)** | 全修正込みの最終検証（180日分） | ⏳ 実行中 |

**修正1+2バックテスト結果**:
- 取引0件→535件に回復。PF 2.47→1.82（低下だが目標1.5+クリア）
- 信頼度帯別で逆転現象（低信頼度¥+100K / 高信頼度¥-1.8K）→ ML再学習の必要性を裏付け

**修正3 ML再学習結果**:
- 平均信頼度0.425、最大0.494（旧min_ml_confidence=0.55では最大でもRecovery発動不能だった）
- 閾値を0.45に調整（平均+0.025、平均以上に自信がある時のみ発動）
...
**最終更新**: 2026年2月26日 - Phase 66進行中（3修正全完了、全修正込みバックテスト検証中）
