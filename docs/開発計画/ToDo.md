...
| **バックテスト検証** | 修正1+2の効果確認（180日分） | ⏳ 実行中 |
| **修正3: ML再学習** | モデル19日前作成で古い。再学習+Recovery閾値調整（min_ml_confidence=0.55が高すぎ） | 📋 待機 |

**発見した問題の詳細**:
- ATRBasedはnormal_rangeで最大重み(0.50)を持つ主力戦略。HOLD信頼度0.5のハードコードにより、HOLD加重(0.250)が常にSELL加重(0.164)に勝ち、取引が発生しなかった
- 修正後: HOLD信頼度0.20 → HOLD加重0.100 < SELL加重0.164で取引可能に
- BBReversalも同じ問題があり併せて修正（hold_confidence 0.5→0.20）
...
**最終更新**: 2026年2月26日 - Phase 66進行中（HOLD信頼度+レジーム分類修正完了、バックテスト検証中）
