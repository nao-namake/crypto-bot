...
| 項目 | 値 |
|------|-----|
| デッドコード削除 | 未使用定数4件・`_check_tp_sl_orders_exist()`メソッド |
| 整合性バグ修正 | TP/SL検出boolean→数量ベース95%カバレッジ・regime不統一修正 |
| 重複統合 | TP/SL価格計算・SL超過成行決済→共通ヘルパー2件抽出 |
...
**やること**:
- executor.virtual_positionsとposition_trackerの二重管理を解消
