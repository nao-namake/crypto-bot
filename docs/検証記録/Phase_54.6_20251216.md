...
Phase 54.6実装の効果検証を目的としたバックテスト実行。
...
| 指標 | 値 | 判定基準 |
|------|-----|---------|
| シャープレシオ | 21.05 | 優秀（≥1.0で良好） |
| 期待値 | ¥+3 | 良好（>0で収益期待） |
| リカバリーファクター | 18.69 | 優秀（≥1.0でDD回復） |

### 参考指標

| 指標 | 値 | 説明 |
|------|-----|------|
| ソルティノレシオ | 37.69 | 下方リスク調整リターン |
| カルマーレシオ | 17.01 | 年率リターン/最大DD |
