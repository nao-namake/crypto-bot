# Phase Phase53.2 バックテスト記録

**実施日**: 2025/12/11

---

## 実施目的

Phase Phase53.2実装の効果検証を目的としたバックテスト実行。
過去180日間のBTC/JPY 15分足データを使用し、本番環境と同一ロジックで検証。

---

## 実行概要

- **バックテスト期間**: 2025/06/13 ~ 2025/12/10 (0日間)
- **データソース**: CSV (15分足 + 4時間足)
- **初期残高**: ¥100,000
- **取引ペア**: BTC/JPY
- **レバレッジ**: 1.0倍

---

## 設定値

### TP/SL設定

レジーム別動的TP/SL設定（Phase 52.0実装）:

| レジーム | TP | SL | 適用ケース |
|----------|----|----|-----------|
| tight_range | 0.8% | 0.6% | レンジ相場（狭い値動き） |
| normal_range | 1.0% | 0.7% | 通常相場 |
| trending | 1.5% | 1.0% | トレンド相場 |

### レジーム別エントリー制限

- tight_range: 最大1ポジション（Phase 51.8実装）
- normal_range: 最大2ポジション
- trending: 最大3ポジション

### ML統合設定

- アンサンブルモデル: LightGBM (50%) + XGBoost (30%) + RandomForest (20%)
- 最小信頼度閾値: 0.45
- 高信頼度閾値: 0.60

---

## エントリー統計

- **総エントリー数**: 16件
- **勝ちトレード**: 5件
- **負けトレード**: 11件
- **勝率**: 31.2%

---

## レジーム別パフォーマンス（Phase 51.8-J4-G）

| レジーム | エントリー数 | 勝率 | 平均損益/取引 | 総損益 |
|---------|------------|------|-------------|--------|
| レンジ（狭） | 16件 | 31.2% | ¥-3 | ¥-44 |


---

## パフォーマンス指標

### 損益サマリー

- **総損益**: ¥-44
- **総利益**: ¥+78
- **総損失**: ¥-122
- **平均損益/取引**: ¥-3

### リスク指標

- **最大ドローダウン**: ¥74 (0.07%)
- **プロフィットファクター**: 0.64
- **平均勝ちトレード**: ¥+16
- **平均負けトレード**: ¥-11

### 取引品質

- **勝率**: 31.2%
- **リスクリワード比**: 1.40:1 (平均)

---

## 結論

### 総合評価: 損失発生

- **プロフィットファクター 0.64**: 要改善
- **勝率 31.2%**: 低い
- **最大DD 0.07%**: 許容範囲内

### レジーム別評価

最も収益性が高いレジーム: **レンジ（狭）** (平均¥-3/取引)

### Phase 52.0実装効果

レジーム別動的TP/SL調整の効果を検証。

- tight_rangeでのTP 0.8%/SL 0.6%設定による早期利確・損切り
- trendingでのTP 1.5%/SL 1.0%設定によるトレンドフォロー

---

**レポート生成日時**: 2025/12/11 00:55:10