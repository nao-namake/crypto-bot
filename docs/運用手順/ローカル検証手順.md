...
Claude のクールダウン中でもローカルで実行と検証ができる環境の使用方法を説明します。

## 🎯 概要

Phase 12 完了時点で、以下の4つの主要検証・実行機能が自動レポート生成に対応しています：

1. **CI前チェック** (`dev_check.py`) - 品質チェック・システム検証
2. **CI後チェック** (`ops_monitor.py`) - 運用診断・稼働状況確認  
3. **バックテスト** (`main.py --mode backtest`) - 過去データでの戦略検証
4. **ペーパートレード** (`main.py --mode paper`) - リアルタイム仮想取引

すべての実行結果は自動的にマークダウンレポートとして保存され、他のAIツールとの情報共有が可能です。

---

## 1. 🔍 CI前チェック（品質チェック・システム検証）
...
# 個別チェック
python3 scripts/management/dev_check.py phase-check    # Phase実装状況確認
python3 scripts/management/dev_check.py validate      # 品質チェック（flake8・pytest）
python3 scripts/management/dev_check.py ml-models     # MLモデル作成・検証
python3 scripts/management/dev_check.py data-check    # データ層基本確認
python3 scripts/management/dev_check.py status        # システム状態確認

# レポート生成無効化（必要時のみ）
python3 scripts/management/dev_check.py full-check --no-report
```

### 期待結果

```
🚀 新システム品質チェック開始 (Phase 12対応)
=================================================
>>> 📂 新システムディレクトリ構造確認
✅ src/ディレクトリ確認完了
>>> 🤖 MLモデル整合性チェック
...
>>> ⚫ black: コード整形チェック
✅ blackチェック完了
>>> 🧪 pytest: 新システム316テスト実行
============================= test session starts ==============================
...
✅ 316 passed in 25.67s
...
📋 インフラ・基盤確認: 90.0/100 (healthy)
  ...
  ✅ test_system_health: 316 tests passed successfully
...
```
...
```
🚀 暗号資産取引Bot v12.0・CI/CDワークフロー最適化・手動実行監視・段階的デプロイ対応 起動 - モード: BACKTEST
📊 バックテストモード開始
...
# または明示的に指定
python3 main.py --mode paper --config config/base.yaml
...
```
🚀 暗号資産取引Bot v12.0・CI/CDワークフロー最適化・手動実行監視・段階的デプロイ対応 起動 - モード: PAPER
📝 ペーパートレードモード開始
🔄 取引サイクル開始 (1分間隔)
...
```bash
# ログディレクトリの権限確認・作成
mkdir -p logs/{dev_check_reports,ops_monitor_reports,backtest_reports,paper_trading_reports}
chmod 755 logs/
```
...
1. **レポートファイルを開く**:
   ```bash
   # 最新のレポートを確認
   ls -la logs/dev_check_reports/ | head -5
   cat logs/dev_check_reports/dev_check_full-check_20250821_040907.md
   ```
...
**🎉 これでClaude のクールダウン中でも、すべての主要機能をローカルで実行・検証し、結果を他のAIツールと共有できます！**