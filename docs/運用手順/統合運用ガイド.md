...
#### **🚨 ドローダウン状態問題（2025/09/20対処法追加）**
**問題**: 「取引拒否: リスクスコア=100.0%, 理由=ドローダウン制限: paused_consecutive_loss」で取引が実行されない
**原因**: ドローダウン状態ファイル（.cache/data/drawdown_state.json）が「paused_consecutive_loss」状態で取引を拒否

**🔧 定期対処法（推奨）**:
```bash
# === 毎日または取引されない場合の定期チェック ===
# 1. ドローダウン状態確認
cat .cache/data/drawdown_state.json 2>/dev/null || echo "ドローダウン状態ファイル未存在"
...
# ✅ 新しいピーク残高更新: 11000.00
# ✅ 取引承認: リスクスコア=XX.X%, ポジションサイズ=0.0001
```

**🎯 症状別対処法**:
```bash
# 症状1: "paused_consecutive_loss" 状態
# → ドローダウン状態ファイル削除で即座解決
rm -f .cache/data/drawdown_state.json

# 症状2: peak_balance異常値（例：1,000,000 vs current_balance: 10,000）
# → 初期化で残高リセット・正常値設定
rm -f .cache/data/drawdown_state.json

# 症状3: 取引停止期間（pause_until未来日時）
# → 状態リセットで即座取引再開
rm -f .cache/data/drawdown_state.json
```
