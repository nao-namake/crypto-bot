...
# 実行例:
bash scripts/backtest/run_backtest.sh                      # 180日間バックテスト実行
bash scripts/backtest/run_backtest.sh --days 30            # 30日間バックテスト実行
bash scripts/backtest/run_backtest.sh --collect --days 90  # 90日間データ収集のみ
bash scripts/backtest/run_backtest.sh --skip-collect       # データ収集スキップ
```
...
# 4. 結果確認（実行後）
# 最新レポート確認
LATEST_REPORT=$(ls -t src/backtest/logs/ | grep backtest_report | head -1)
echo "📊 最新バックテストレポート: $LATEST_REPORT"
cat "src/backtest/logs/$LATEST_REPORT"
...
- **BacktestRunner**: 戦略実行・ML予測・リスク管理・統計計算
- **Reporter**: JSON生成・パフォーマンス分析・レポート出力
