...
- ✅ **最新状態反映**: 1,056テスト・64.99%カバレッジ達成済み
...
# 実行時間: 約4分（50 trials）
# 学習データ: 180日分・1,032サンプル・57特徴量（戦略信号なし）
```
...
| オプション | 説明 | 標準で有効 |
|-----------|------|----------|
| `--level 1` | full モデル（62特徴量） | ✅ 推奨 |
| `--level 2` | basic モデル（57特徴量・フォールバック） | ⏳ 必要時 |
...
# 期待: ensemble_full_YYYYMMDD_HHMMSS.pkl 存在
```
...
#
# ✅ アンサンブルモデル保存完了: models/production/ensemble_full.pkl (4.7M)
# ✅ メタデータ保存: production_model_metadata.json
# 🎉 ML学習完了 - 実取引準備完了
#
# 実行時間: 約4分（50 trials）
# F1スコア: 0.56-0.59（62特徴量対応・3クラス分類として良好）
```
...
**目的**: 67パラメータ自動最適化（戦略・ML統合・MLハイパーパラメータ）
...
# 3. 検証項目チェック（5-10分実行）
# ✅ MLモデル初期化: ProductionEnsemble読み込み成功（62特徴量対応）
# ✅ 62特徴量生成: 基本57（ATR・EMA・RSI・MACD・BB・Donchian・ADX・ラグ・移動統計・交互作用・時間等） + 戦略信号5
...
# 4. ログ確認（期待出力）
# [INFO] 🤖 MLモデル初期化成功: ProductionEnsemble (4.7M・62特徴量対応)
# [INFO] 📊 62特徴量完全生成成功
...
```bash
# ML学習（約4分・50 trials・62特徴量）
python3 scripts/ml/create_ml_models.py --level 1 --n-classes 3 --threshold 0.005 --optimize --n-trials 50 --verbose
...
**📅 最終更新**: 2025年11月01日 - 62特徴量システム対応・シンプル化完了
