...
4. **第3部: GitHub Actions自動化** - CI/CD・週次バックテスト
5. **第4部: ローカル開発環境** - セットアップ・実行方法
6. **第5部: 本番デプロイメント** - GCP Cloud Run構築
7. **第6部: 日常運用・監視** - ログ確認・パフォーマンス監視
8. **第7部: 緊急時対応** - トラブルシューティング
9. **付録: スクリプトリファレンス** - 全スクリプト一覧
...
# 2. 品質チェック（約80秒・1,191テスト・65.42%カバレッジ）
bash scripts/testing/checks.sh

# 3. バックテスト実行（約2-3時間・Phase 52影響確認）
python main.py --mode backtest

# または GitHub Actions自動実行（推奨）
gh workflow run "Weekly Backtest Automation" -f phase_name="52.5"
```

**使用タイミング**:
- ML学習: 新機能追加後・週次定期再学習
- 品質チェック: コード変更後・デプロイ前必須
- バックテスト: パラメータ変更後・戦略検証時
...
### Step 5.5: GitHub週次バックテスト実行（推奨・自動化）

#### 🚀 **GitHub Actions手動実行**

```bash
# 方法1: GitHub CLIで実行（推奨）
gh workflow run "Weekly Backtest Automation"

# Phase名指定（オプション）
gh workflow run "Weekly Backtest Automation" -f phase_name="52.4"

# 実行確認
gh run list --workflow="Weekly Backtest Automation" --limit 5
```

#### 🌐 **GitHub Webインターフェースで実行**

1. **GitHubリポジトリを開く**: https://github.com/YOUR_USERNAME/YOUR_REPO
2. **Actionsタブをクリック**
3. **左サイドバーで "Weekly Backtest Automation" を選択**
4. **"Run workflow" ボタンをクリック**
5. **Phase名を入力**（デフォルト: 52.4）
6. **"Run workflow" を実行**

#### ⏱️ **実行時間目安**

- **データ収集**: 約5-10分（180日分）
- **バックテスト実行**: 約2時間43分（Phase 52.4実測）
- **レポート生成**: 約1-2分
- **合計**: 約3時間
- **タイムアウト設定**: 6時間（十分なバッファ確保・Phase 53.8.2）
...
# 期待出力例:
# ✓ main  Weekly Backtest Automation  #123  workflow_dispatch  3h2m15s
#   Status: completed
#   Conclusion: success
...
# 3. 生成されたMarkdownレポート確認
ls -lt docs/バックテスト記録/Phase_*.md | head -3

# 4. レポート内容確認
cat docs/バックテスト記録/Phase_52.4_YYYYMMDD.md
```

#### ✅ **成功確認項目**

- ✅ ワークフローステータス: `success`
- ✅ Markdownレポート生成: `docs/バックテスト記録/Phase_*.md`
- ✅ Gitコミット完了: レポートがリポジトリに追加
- ✅ JSONレポート生成: `src/backtest/logs/backtest_*.json`

#### 🔄 **ローカル同期**

```bash
# GitHub Actionsで生成されたレポートを取得
git pull origin main

# 生成されたレポート確認
ls -lt docs/バックテスト記録/ | head -5
```

#### 📅 **自動実行スケジュール**

- **頻度**: 毎週日曜日 00:00 JST（土曜 15:00 UTC）
- **実行内容**: データ収集 → バックテスト → レポート生成 → Git自動コミット
- **通知**: GitHub Actions成功/失敗メール

**役割**: GitHub Actions上でバックテストを自動実行・Markdownレポート自動生成・リポジトリ自動更新
...
**📅 最終更新**: 2025年11月26日 - Phase 55.2対応完了（バックテストスクリプト拡張・進捗表示機能・結果サマリー自動表示追加）
