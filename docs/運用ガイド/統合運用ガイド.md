...
**最終更新**: 2026年1月4日 - Phase 57.11バックテストスクリプト改修
...
### バックテスト実行（Phase 57.11）

```bash
# 基本実行（180日・CSV収集あり・Markdownレポート自動生成）
bash scripts/backtest/run_backtest.sh
...
├── backtest/
│   ├── run_backtest.sh       # バックテスト実行（Phase 57.11改修）
...
bash scripts/backtest/run_backtest.sh                     # 180日・フル機能
bash scripts/backtest/run_backtest.sh --days 30           # 30日間テスト
bash scripts/backtest/run_backtest.sh --days 60 --skip-collect  # 既存CSV使用
