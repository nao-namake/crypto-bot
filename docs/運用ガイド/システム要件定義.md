...
| **関連ドキュメント** | [システム機能一覧.md](システム機能一覧.md) - 実装済み機能リファレンス（HOW） |
...
**アンサンブル構成**:
- 3モデル統合: unified.yamlに重み定義（LightGBM 50%・XGBoost 30%・RandomForest 20%）
- **RandomForest設定**: `n_jobs=1`（GCP Cloud Run gVisor環境互換性・クラッシュ防止）
- 特徴量管理: feature_order.json単一真実源（特徴量数・順序・レベル定義）
- **2段階Graceful Degradation + Dummy**: ensemble_full.pkl（Full特徴量・55特徴量） → ensemble_basic.pkl（Basic特徴量・49特徴量・戦略シグナル除外） → DummyModel（最低限動作保証）
- **モデルファイル**: ensemble_full.pkl（デフォルト）・ensemble_basic.pkl（フォールバック）
  ...
  - gVisor互換性: 単一スレッド処理によるシステム安定性確保
...
### 可用性
- 稼働時間: 24時間365日自動稼働
- **稼働率: 99%目標**（Two Nines - 商用サービス標準品質）
  - Phase 53.8達成: Python 3.11移行・RandomForest `n_jobs=1`設定
  - 効果: 33.74% → 99%稼働率（クラッシュ105回/7日間 → ゼロ）
  - GCP gVisor互換性: 単一スレッド処理による完全安定化
...
### 開発言語・フレームワーク
- **Python: 3.11**（メイン実行環境・GCP Cloud Run gVisor互換性・99%稼働率達成）
  - Phase 53.8移行: Python 3.13 → 3.11（gVisor環境安定性優先・RandomForestクラッシュ解決）
  - 互換性確保: numpy 2.x（2.3.5+）・pandas 2.2+・scikit-learn 1.5.2
  - 開発環境: Homebrew Python 3.11・仮想環境分離
...
| 制約カテゴリ | 項目 | 設定値 | 理由 |
|------------|------|--------|------|
| **実行環境** | Python バージョン | 3.11（固定） | GCP gVisor互換性・99%稼働率達成 |
| | numpy バージョン | ≥2.0.0 | pandas-ta-classic 0.3.37要求 |
| | pandas バージョン | ≥2.2.0 | numpy 2.x バイナリ互換性 |
| | scikit-learn バージョン | 1.5.2 | numpy 2.x対応・Python 3.11互換性 |
...
**最終更新**: 2025-11-20 (Phase 53.8.1) - Python 3.11安定化完了・15分足データ最適化・99%稼働率目標・GCP gVisor互換性確保・RandomForestクラッシュ修正・numpy 2.x + pandas 2.2+互換性確立・MLモデル再訓練・システム安定性向上・1,252テスト100%成功・66.65%カバレッジ達成
