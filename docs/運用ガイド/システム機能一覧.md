...
| 項目 | 設定 |
|------|------|
| SL注文タイプ | stop_limit（指値）|
| スリッページバッファ | 0.1% |
| 手数料削減効果 | taker 0.12% → maker 0.02%（83%削減）|
...
**概要**: バックテスト結果を統一フォーマットで分析

| 機能 |
|------|
| CI結果自動取得（`--from-ci`） |
| ローカル結果自動検出（`--local`） |
| 84項目の固定指標（基本・戦略別・ML別・レジーム別） |
| 改善提案自動生成 |
| 履歴CSV追記（変更前後比較用） |
...
python3 scripts/backtest/standard_analysis.py <path>      # 特定ファイル
```

**出力先**: `results/analysis_YYYYMMDD_HHMMSS.json/md`、`results/analysis_history.csv`

**実装ファイル**: `scripts/backtest/standard_analysis.py`

---

### 11.7 ライブモード標準分析スクリプト

**概要**: ライブモードの固定指標分析（39項目）

| 機能 |
|------|
| bitbank API連携（アカウント・ポジション・注文取得） |
| GCPログ取得（稼働率・エラー分析） |
| SQLite取引履歴連携（税務DB） |
| 39固定指標（バックテストと同等の評価基準） |
| JSON/Markdown/CSV出力 |
...
| カテゴリ | 指標数 | 主な内容 |
|---------|--------|---------|
| アカウント状態 | 5 | 証拠金維持率・残高・使用証拠金・未実現損益・マージンコール |
| ポジション状態 | 5 | オープン数・詳細・未約定注文・注文内訳・ロスカット価格 |
| 取引履歴分析 | 12 | 取引数・勝率・損益・戦略別分析・TP/SL発動数 |
| システム健全性 | 6 | API応答時間・エラー数・稼働状態・デプロイ時刻 |
| TP/SL適切性 | 4 | 現在価格距離・設置有無・設定値ズレ |
| 稼働率 | 3 | 稼働時間率・ダウンタイム・障害発生時刻 |
| MLモデル状態 | 4 | モデルタイプ・フォールバックレベル・特徴量数・Stacking設定 |

**使用方法**:
```bash
python3 scripts/live/standard_analysis.py                    # 基本（24時間）
python3 scripts/live/standard_analysis.py --hours 48         # 期間指定
python3 scripts/live/standard_analysis.py --output results/  # 出力先指定
```

**出力先**: `results/live/live_analysis_YYYYMMDD_HHMMSS.json/md/csv`

**実装ファイル**: `scripts/live/standard_analysis.py`
