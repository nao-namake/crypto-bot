...
| `config/core/strategies.yaml` | 戦略定義 | 6戦略の有効化・重み・優先度 |
| `config/core/strategies/active.yaml` | プリセット選択 | アクティブプリセット指定 |
| `config/core/strategies/presets/` | 戦略プリセット | 各種パラメータセット |
| `config/optimization/results/` | 最適化結果 | Optuna最適化の生データ |
...
### 2.1 Atomic Entry Pattern

**概要**: Entry・TP・SL注文を一体化して配置（全成功 or 全ロールバック）

| 項目 | 値 |
|------|-----|
| リトライ方式 | Exponential Backoff（1秒、2秒、4秒） |
| 最大リトライ | 3回 |
| 失敗時動作 | 全注文キャンセル（ロールバック） |

**実装ファイル**: `src/trading/execution/atomic_entry_manager.py`
...
### 2.3 TP/SL再計算（3段階ATRフォールバック）

**概要**: 実約定価格ベースでTP/SLを再計算

| 優先度 | 取得元 |
|--------|--------|
| 1 | evaluation.market_conditions |
| 2 | DataService経由で直接取得 |
| 3 | デフォルトATR値 |

**実装ファイル**: `src/trading/execution/tp_sl_calculator.py`
...
**設定**: `config/core/strategies.yaml`
...
**実装ファイル**:
- `.github/workflows/optuna-optimization.yml`
- `scripts/optimization/run_github_optimization.py`
- `scripts/optimization/save_as_preset.py`

**結果保存先**:
- `config/optimization/results/latest_optimization.json`（生データ）
- `config/core/strategies/presets/optuna_YYYYMMDD.yaml`（プリセット）

---

### 12.2 プリセットシステム

**概要**: 戦略設定のプリセット管理・切り替え機能

| ファイル | 役割 |
|---------|------|
| `config/core/strategies/active.yaml` | アクティブプリセット指定 |
| `config/core/strategies/presets/*.yaml` | 各種プリセット定義 |

**実装ファイル**: `src/core/config/threshold_manager.py`

**使用方法**:
1. `active.yaml`の`active_preset`を変更
2. システム再起動で適用

---

### 12.3 GitHub Actionsワークフロー

**概要**: CI/CD統合による自動化

| ワークフロー | 用途 |
|-------------|------|
| `optuna-optimization.yml` | パラメータ最適化（手動実行） |
| `backtest.yml` | バックテスト（手動実行） |
| `weekly-ml-training.yml` | MLモデル週次学習 |
| `weekly-report.yml` | 週間レポート送信 |

**実装ファイル**: `.github/workflows/`
...
| 戦略定義 | `config/core/strategies.yaml` |
| プリセット選択 | `config/core/strategies/active.yaml` |
| 戦略プリセット | `config/core/strategies/presets/` |
| 最適化結果 | `config/optimization/results/` |
