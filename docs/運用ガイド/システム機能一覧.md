...
|------|-----|
| 目標純利益 | 1,000円（デフォルト） |
| 手数料考慮 | エントリー手数料・利息・決済リベート |
| API連携 | bitbank APIから実際の手数料データ取得 |
| フォールバック | API失敗時は推定値（Taker 0.12%/Maker -0.02%） |
...
fixed_amount:
  ...
  fallback_exit_fee_rate: -0.0002   # 決済時（Maker -0.02%リベート）
```
