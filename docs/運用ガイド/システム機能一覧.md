...
### 4.5 TP/SL欠損自動復旧（Phase 62.20）

**概要**: Atomic Entry完了後、5分後にTP/SL設置状態を自動検証し、欠損時は自動再構築
...
| Step 4 | 欠損があればtight_rangeのTP/SL幅で再構築 |
...
|------|-----|
| 目標純利益 | 500円 |
| 手数料考慮 | エントリー手数料・利息・決済手数料 |
| API連携 | bitbank APIから実際の手数料データ取得 |
| フォールバック | Maker 0%（改定後） |
