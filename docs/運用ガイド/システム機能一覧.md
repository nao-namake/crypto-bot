...
**概要**: TP/SL注文の配置・管理・到達判定・タイムアウト制御
...
**概要**: ポジション決済後に残ったTP/SL注文を自動削除。孤児ポジション（TP/SL欠損）を検出して自動復旧。
...
**孤児ポジション検出**: TP/SL注文の合計数量がポジション数量の95%未満の場合に「欠損」と判定。1件でもあればOKとするboolean方式ではなく、数量ベースで正確に判定。
...
**概要**: Atomic Entry完了後、10分後にTP/SL設置状態を自動検証し、欠損時は自動再構築。10分間隔の定期チェックでも欠損を検出・復旧。
...
**TP/SL検出方式**: TP/SL注文の合計数量がポジション数量の95%以上あれば「カバー済み」と判定。boolean方式（1件でもあればOK）ではなく、数量ベースで正確に判定。

**SL超過フォールバック**: 復旧時にSLトリガー価格を現在価格が既に超過している場合、SL注文配置ではなく成行決済を実行。

**復旧用TP/SL価格計算**: デフォルトtight_range（最保守）のTP/SL比率で計算。共通ヘルパー`calculate_recovery_tp_sl_prices()`で一元化。
