...
## 6.4 ML統合ロジック（3段階信頼度ベース + ML Signal Recovery）
...
**Phase 65.16: ML Signal Recovery**: 戦略の加重投票がHOLDでも、MLが方向性を持ち（BUY/SELL）かつ個別戦略が1つ以上同方向に投票している場合、HOLDをオーバーライドしてシグナルを回復する。MLを7つ目の擬似戦略として活用する機能。
...
**実装ファイル**: `src/core/services/trading_cycle_manager.py` → `_integrate_ml_with_strategy()`

**設定**: `config/core/thresholds.yaml` → `ml.strategy_integration.ml_signal_recovery`
