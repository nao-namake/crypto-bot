...
## 2.1 設定管理システム（1ファイル体系）

すべての設定をハードコード排除し、設定ファイルで一元管理。

| 設定ファイル | 役割 | 主な設定項目 |
|-------------|------|-------------|
| `config/core/thresholds.yaml` | 全設定一元管理 | 環境設定、残高、API設定、ML閾値、リスク設定、戦略定義+パラメータ、TP/SL、機能トグル |
| `config/core/feature_order.json` | 特徴量定義（Single Source of Truth） | 37特徴量リスト |
...
**設定**: `config/core/thresholds.yaml` → `mode_balances`
...
**Phase 65.16: 1円スプレッド対応**: BTC/JPYのスプレッドが最小1円（1tick）の場合、従来の`_calculate_maker_price()`はinside配置不可→return 0→Takerフォールバックとなっていた。修正後はスプレッド≤1tickの場合にbest_bid（買い）/ best_ask（売り）に配置し、`post_only=True`でMaker確定。

**実装ファイル**: `src/trading/execution/executor.py`、`src/trading/execution/order_strategy.py`
...
## 4.1 ポジションサイジング（Phase 67.4: 固定テーブル方式）

ML信頼度に応じた固定ポジションサイズテーブル。動的計算の複雑さと不安定性を解消。
...
- BTC価格・残高に依存せず安定したポジションサイズ
- `thresholds.yaml`の3つの数値を変更するだけで調整可能
- フォールバック: `mode: dynamic`で従来のKelly/Dynamic加重平均に戻せる

**実装ファイル**: `src/trading/risk/sizer.py`

**設定**: `config/core/thresholds.yaml` → `position_sizing`
...
**設定**: `config/core/thresholds.yaml` → `feature_flags.trading.cooldown`
...
**データファイル**: `logs/orphan_sl_orders.json`（`position_restorer.py:90, 715`）
...
| レベル | モデル | 特徴量数 | 状態 |
|--------|--------|---------|------|
| Level 0 | StackingEnsemble | 37 + メタ | **無効化**（stacking_enabled: false） |
| Level 1 | ensemble_full.pkl | 37 | **デフォルト使用** |
| Level 2 | ensemble_basic.pkl | 37 | フォールバック |
...
## 6.3 メタラベリング（取引品質フィルタ）

Phase 73-Dで方向予測からメタラベリングに転換。MLは「この取引は成功するか」を判定（Go/No-Go）。
...
**Triple Barrier Method**: threshold=0.003で学習。戦略が方向を100%決定し、MLは品質判定のみ。

## 6.4 ML品質フィルタ閾値（Phase 79調整）

quality_filterモードの3段階判定:

| 判定 | 条件 | 動作 |
|------|------|------|
| **ACCEPT** | ml_pred=1 AND 信頼度≥0.65 | 取引承認・通常実行 |
| **REJECT** | (ml_pred=0 AND 信頼度≥0.55) OR 信頼度<0.45 | HOLD変換・取引拒否 |
| **UNCERTAIN** | 上記以外（中間帯） | 信頼度を50%縮小して通過 |

| 閾値 | Phase 76 | Phase 79 | 根拠 |
|------|---------|---------|------|
| accept_threshold | 0.55 | **0.65** | モデル平均0.5928より上、上位約30%のみ通過 |
| reject_threshold | 0.35 | **0.45** | 明確に低品質を拒否 |
| uncertain_penalty | 0.7 | **0.5** | 中間帯を50%に縮小 |

**実装ファイル**: `src/core/services/trading_cycle_manager.py` → `_apply_quality_filter()`

**設定**: `config/core/thresholds.yaml` → `ml.quality_filter`

## 6.5 37特徴量システム（Phase 77: SHAP+Forward Selection最適化）
...
| Returns | 3 | returns_1/2/3（旧close_lag→定常性改善） |
| Volume | 4 | volume_lag_1/2/3, volume_ema |
| Rolling | 5 | close_ma_10/20, close_std_5/10/20 |
| Interaction | 5 | rsi_x_atr, macd_x_volume |
| Time | 2 | hour, day_of_week |

**Phase 77で削除**: 戦略シグナル6個（importance=0）、close_lag_1/2/3（returns変換）、day_cos、is_europe_session
**Phase 77で追加**: volume_ema（Forward Selection +0.006）

- **Full = Basic = 37特徴量**（単一モデル構成に統一）
...
**戦略選択方式**: レジーム別重み付け統合（2票ルールは無効化）+ ML Signal Recovery（Phase 65.16）

**実装ファイル**: `src/strategies/implementations/`

**設定**:
- `config/core/thresholds.yaml` → `strategies` - 戦略定義+パラメータ
...
tight_range:
  ...
  StochasticReversal: 0.10  # Phase 67: 重み削減
  ATRBased: 0.45            # Phase 67: 主力集中
  DonchianChannel: 0.30     # 補助
  ADXTrendStrength: 0.0     # トレンド型・タイトレンジ不向き
  MACDEMACrossover: 0.0     # トレンド型・タイトレンジ不向き
```

**注**: 2票ルール（consensus）は検証の結果、重み設定を無視する問題が判明し無効化。代わりにML Signal Recovery（Phase 65.16）でMLの方向性シグナルを活用。
...
## 8.3 固定金額TP（Phase 68.8: 信頼度別2段階）

%ベースではなく、目標純利益（固定金額）を保証するTP価格を計算。Phase 68.8で信頼度別2段階に拡張。
...
| 項目 | 値 |
|------|-----|
| 手数料計算 | エントリー: Taker 0.1% / 決済: Maker 0% |
...
**Phase 68修正**: エントリー手数料をTaker 0.1%で計算に含める（最悪ケース想定）。Maker約定時は手数料0%となり追加利益。
...
**後方互換性**: `enabled: false`で従来の%ベース計算に戻せる。`confidence_based.enabled: false`で固定500円に戻せる

## 8.4 固定金額SL（Phase 68.8: 信頼度別2段階）

%ベースではなく、最大損失額（固定金額）を保証するSL価格を計算。Phase 68.8で信頼度別2段階に拡張。
...
| 項目 | 値 |
|------|-----|
| 手数料計算 | エントリー: Taker 0.1% + 決済: Taker 0.1% |
...
**設定**: `config/core/thresholds.yaml` → `position_management.stop_loss.fixed_amount`
...
## 8.5 TP/SLレースコンディション対策（Phase 67.4 → 67.5で根本解決）

Phase 65.2の統合TP/SL再配置時に、SLキャンセル→再配置の間にSL不在となる問題を解消。

- **SL先行配置**: TP→SLの配置順序をSL→TPに変更（SL不在期間を最小化）
- **SL超過事前チェック（キャンセル前）**: 既存SLがまだ有効な状態で現在価格がSLラインを超過していないか確認。超過時は即成行決済してreturn（既存注文のキャンセル不要）
- **SL超過再チェック（キャンセル後）**: キャンセル中の価格変動に対するセーフティネット
- **消失ポジション強制クリーンアップ**: 1時間以上経過した消失ポジションをvirtual_positionsから自動削除

**実装ファイル**:
- `src/trading/execution/tp_sl_manager.py` → SL先行配置 + SL超過チェック（二重）
- `src/trading/execution/stop_manager.py` → 消失ポジション強制クリーンアップ

## 8.6 limit注文の約定ポーリング（Phase 67.5）

bitbankはlimit注文をfilled_amount=0で即時返却するが、数秒以内に約定することが多い。

- **約定ポーリング**: SUBMITTED状態のlimit注文に対し、最大10秒（2秒×5回）fetch_orderで約定確認
- **約定確認時**: filled_amount/filled_price更新→通常のTP/SL配置フローへ
- **未約定時**: `_last_tp_sl_check_time`リセット→次の5分サイクルで即TP/SLチェック実行
...
**設定**: `config/core/thresholds.yaml` → `data.timeframes`
...
├── ml/                     # 機械学習
│   └── create_ml_models.py        # MLモデル学習（37特徴量）
└── testing/                # 品質保証
...
| ファイル | パス |
|---------|------|
| 全設定一元管理 | `config/core/thresholds.yaml` |
