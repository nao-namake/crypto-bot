...
| 56.9 | ADX/MACD重み0化・RSIモード検証 | ✅ | トレンド型戦略をレンジで無効化 |
| 56.10 | ATR戦略BB条件化検証 | ✅ | BB主導化→PF低下→元に戻す |
...
## ⚙️ Phase 56.9: ADX/MACD重み0化・RSIモード検証【完了】

### 実施日: 2025/12/28

### 目的

ADXTrendStrength戦略の取引数を増加させる（目標: 60日180件）

### 検証内容

#### 1. RSI主導型レンジ逆張りモード

Web調査に基づき、ADXレンジモード（ADX < 20）でRSI+BB条件をメイントリガーに変更。

**実装内容（`adx_trend.py`）**:
```python
def _analyze_range_reversal(self, df, analysis, multi_timeframe_data=None):
    # 買いシグナル: RSI < 35 AND BB位置 < 0.25
    # 売りシグナル: RSI > 65 AND BB位置 > 0.75
```

**結果**:
| 設定 | 60日取引数 | 180日取引数 | PF |
|------|-----------|-------------|-----|
| RSI+BB AND条件 | 114件 | 270件 | 0.99 |
| RSI単独条件 | 120件 | 275件 | 0.89 |
| 元のDI条件 | 117件 | 275件 | 1.01 |

**結論**: RSIモードは取引数増加に寄与するが、PF低下を招く。

#### 2. データ分析

RSI+BB条件の発生頻度を分析:
- 条件満足件数: 4.5件/日（十分な頻度）
- 問題: 勝率38-40%、PF 0.79-0.88

**根本的問題**: ADXはトレンドフォロー型指標であり、レンジ相場での平均回帰には本質的に不向き。

### 最終決定

#### トレンド型戦略のレンジでの無効化

ADXTrendStrengthとMACDEMACrossoverはトレンドフォロー型戦略であり、
レンジ相場（tight_range/normal_range）では性能が出ない。
...
regime_strategy_mapping:
  tight_range:
    ...
    ADXTrendStrength: 0.0     # トレンド型→無効化
    MACDEMACrossover: 0.0     # トレンド型→無効化
  normal_range:
    ATRBased: 0.40
    BBReversal: 0.25
    ...
    MACDEMACrossover: 0.0     # トレンド型→無効化
  trending:
    ADXTrendStrength: 0.35    # トレンド時に有効
    MACDEMACrossover: 0.25    # トレンド時に有効
    # ...
```
...
| ファイル | 修正内容 |
|---------|---------|
| `config/core/thresholds.yaml` | tight_range/normal_rangeでADX/MACD重み0.0 |
| `src/strategies/implementations/adx_trend.py` | RSI信頼度計算メソッド追加（use_rsi_driven_mode: false） |

### 学習事項

1. **戦略と相場の適合性**: トレンドフォロー型戦略（ADX/MACD）をレンジ相場に無理に適用しても性能低下
2. **レンジ相場戦略の限界**: 全戦略で180件/60日達成は困難。戦略の特性に合った配分が重要
3. **RSI+BB条件の問題**: 条件発生頻度は十分だが、勝率が低く収益性に欠ける

---

## ⚙️ Phase 56.10: ATR戦略BB条件化検証【完了】

### 実施日: 2025/12/28

### 目的

ATRBased戦略の取引数を増加させる（目標: 180件/60日）
...
**問題**: シグナル数は増えたがPFが大幅に低下

### 最終決定

#### 元設定に戻す

BB位置メイン条件化はPF低下を招くため、元のRSI方向決定ロジックに戻す。

**最終設定（`thresholds.yaml`）**:
```yaml
atr_based:
  exhaustion_threshold: 0.55
  high_exhaustion_threshold: 0.70
  adx_range_threshold: 28
  bb_as_main_condition: false  # BB位置はボーナスとして使用
```
...
| ファイル | 修正内容 |
|---------|---------|
| `config/core/thresholds.yaml` | BB条件メイン化→元に戻す |
| `src/strategies/implementations/atr_based.py` | BB主導モード追加（`bb_as_main_condition: false`で無効） |

### 学習事項

1. **ATR消尽率+RSIがベスト**: BB位置メイン化は取引数増加するがPF低下
2. **品質vs量のトレードオフ**: 155件/60日でPF 1.06 > 893件/60日でPF 0.78
3. **既存ロジックの価値**: 無理な変更より現状維持が正解の場合もある
...
19. **戦略と相場の適合性**: トレンドフォロー型（ADX/MACD）をレンジ相場に無理適用しても性能低下
20. **無理な最適化の回避**: RSI+BB条件は頻度十分だが勝率低下、元ロジック維持が正解
21. **ATR消尽率+RSIがベスト**: BB位置メイン化は取引数増加するがPF低下を招く
22. **品質vs量のトレードオフ**: 155件/60日でPF 1.06 > 893件/60日でPF 0.78

---

**📅 最終更新**: 2025年12月28日 - Phase 56.10完了（ATR戦略BB条件化検証→元設定維持）
