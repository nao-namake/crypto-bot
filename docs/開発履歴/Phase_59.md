...
## Phase 59.10: 追加メタ特徴量実験（失敗）

### 背景

Phase 59.9のMeta-Learner改善（15特徴量）の効果を検証し、さらなる改善を目指して6つの追加メタ特徴量を実験。
...
|--------|------|
| `prob_std` | 全確率の標準偏差 |
| `prob_range` | 確率の最大-最小範囲 |
| `avg_max_conf` | 各モデルの最大信頼度平均 |
| `conf_std` | 信頼度の標準偏差 |
| `sell_prob_mean` | SELLクラス確率の平均 |
| `buy_prob_mean` | BUYクラス確率の平均 |

**メタ特徴量数**: 15 → 21（+6特徴量）

### 結果（❌失敗）

| 指標 | Phase 59.9 | Phase 59.10 | 変化 |
|------|-----------|-------------|------|
| Meta-Learner F1 | 0.43 | **0.425** | **-1.2%** |
| SELL予測率 | 42% | **49%** | +7% |
| BUY予測率 | 31% | **25%** | -6% |

**結論**: 追加特徴量による改善なし。過学習の兆候あり。

---

## Phase 59 最終比較: Stacking vs ProductionEnsemble

### 180日バックテスト結果

| 指標 | Phase 59.10 Stacking | ProductionEnsemble | 優位 |
|------|---------------------|-------------------|------|
| **総損益** | ¥+43,729 | **¥+54,526** | **PE (+24.7%)** |
| **PF** | 1.58 | **1.60** | PE |
| **勝率** | **54.5%** | 53.2% | Stacking |
| 取引数 | 477件 | 453件 | - |
| SR | **13.90** | 13.79 | Stacking |
| 最大DD | **1.10%** | 1.49% | Stacking |
| 一致率 | 45.3% | **51.2%** | PE |
| 一致時勝率 | 58.3% | **62.1%** | PE |
...
| 戦略 | Stacking | PE | 差額 |
|------|----------|------|------|
| ATRBased | ¥+37,761 | ¥+40,672 | +¥2,911 |
| StochasticReversal | ¥+1,561 | **¥+8,323** | **+¥6,762** |
| DonchianChannel | ¥+2,924 | ¥+3,158 | +¥234 |
| BBReversal | ¥+1,459 | ¥+1,503 | +¥44 |
| ADXTrendStrength | ¥+24 | ¥+870 | +¥846 |

### 結論

**ProductionEnsemble（Stacking無効）を採用**

理由:
1. **収益性**: +24.7%高い（¥54,526 vs ¥43,729）
2. **ML×戦略一致率**: +5.9pt高い（51.2% vs 45.3%）
3. **一致時勝率**: +3.8pt高い（62.1% vs 58.3%）
4. **シンプルさ**: Meta-Learner層を削減し、保守性向上

Stackingは勝率・DD面で若干優位だが、収益差が大きすぎる。
...
# config/core/thresholds.yaml
ensemble:
  stacking_enabled: false  # Phase 59完了: Stacking無効化
```
...
2026年1月13日 〜 2026年1月18日

### 成果一覧

| Phase | 内容 | 結果 |
|-------|------|------|
| 59.1 | BBReversal normal_range無効化 | ✅ +¥1,710改善 |
| 59.2 | 信頼度逆転問題対策 | ✅ penalty/bonus調整 |
| 59.3 | adjusted_confidence記録修正 | ✅ 統計記録正常化 |
| 59.4-A | 2票ルール無効化 | ✅ **PF 1.55、損益+¥44,506** |
| 59.4-B | レジーム別重み調整 | ❌ 失敗→リバート |
| 59.5 | ライブモード分析修正 | ✅ 3問題修正 |
| 59.6 | SL指値化 + 孤児SL修正 | ✅ 手数料83%削減 |
| 59.7 | Stacking Meta-Learner実装 | ✅ 基本実装完了 |
| 59.8 | Stacking本番環境統合 | ✅ フォールバック確立 |
| 59.9 | Meta-Learner改善 | ✅ F1 +19%改善 |
| 59.10 | 追加メタ特徴量実験 | ❌ 効果なし |
| **最終** | **Stacking vs PE比較** | **✅ PE採用（+24.7%収益）** |

### 最終パフォーマンス（ProductionEnsemble）

| 指標 | 値 | 評価 |
|------|-----|------|
| 総損益（180日） | **¥+54,526** | 過去最高 |
| PF | **1.60** | 優秀 |
| 勝率 | 53.2% | 良好 |
| 最大DD | 1.49% | 許容範囲 |
| 年間リターン（複利推定） | **22.9%** | 目標10%の2倍超 |

### 教訓

1. **シンプルな方が良い場合がある**: Stackingは理論的に優れているが、実際にはPEの固定重み平均が高収益
2. **ML改善と戦略は相互依存**: MLモデル改修後は戦略との統合効果を再評価する必要あり
3. **過学習リスク**: メタ特徴量を増やしすぎると過学習の危険性

---

### 次のステップ（Phase 60）

Phase 60では戦略関係の改修を実施予定。

ML改修（Phase 59）→ 戦略改修（Phase 60）の順で進め、必要に応じてML再調整を検討。

---

**最終更新**: 2026年1月18日
**ステータス**: ✅ **Phase 59 完了** - ProductionEnsemble採用、Stacking無効化
