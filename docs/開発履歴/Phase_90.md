...
# Phase 90ε: 土日TP/SL縮小（2026-05-30）

## 背景
日本の土日祝はBTCが特別イベントなき限り狭いレンジに収まりやすく、平日基準のTP1000-1200円（距離0.79%）では約定しないまま月曜を迎えるケースが多発（ユーザー指摘）。

## 実装内容
実運用の confidence_based（固定金額）経路に **JST土日判定** `is_weekend_jst`（`weekday()>=5`・Phase 83CのJST明示変換を踏襲）を追加し、信頼度判定の**後に最終上書き**。
- **土日は信頼度に関係なくTPを一律500円**（距離≒0.36%でレンジ内利確優先）に上書き。
- SLは target1000円へ下げるが **floor 0.7% 据え置き（A案）** のため実効SL≒0.70%（≒1,733円@0.015BTC・平日2000円より縮小しつつノイズ耐性維持）。
- 祝日は対象外（jpholiday不採用・追加依存ゼロ）。
- 設定は `thresholds.yaml` の `take_profit.fixed_amount.weekend` / `stop_loss.fixed_amount.weekend` でトグル化。ログに `(土日一律縮小)` ラベル出力で観察可能。

変更3ファイル（thresholds.yaml / strategy_utils.py / test_risk_manager.py）・テスト6件・black/isort/flake8 PASS・実config E2E確認済。

注: 既存 Phase 58.6 の `weekend_adjustment` は %ベース（dead code）で実運用無効。Phase 90εはそれと独立に、実際に動く固定金額経路へ土日縮小を導入したもの。

# Phase 90ζ: 観察可能化 + 整合性補修（2026-05-30）

## 背景
Phase 90εデプロイ後のライブ分析で観察性の穴が判明（取引挙動は不変・ログレベルと文言のみの補修）。

## 実装内容
- ①**SL土日縮小が本番ログで見えない**（SL適用ログが INFO で LOG_LEVEL=WARNING に抑制）→ `strategy_utils.py` の `🛡️ 固定金額SL適用` を INFO→WARNING 昇格（Phase 90γ-⑥のTP昇格と対称）。`confidence_label` に `(土日縮小→N円)` を含むため土日SL縮小が検証可能に。
- ②**特徴量数の37/55不整合**（README図・checks.shログが旧37のまま）→ 現在形参照を55に統一。
- ③**ライブ分析 `standard_analysis.py` を直近修正に追従**（5点）: 90δ post_only実Taker約定WARNING監視 / 90ε土日TP500監視 / 90ζ SL適用ログ監視 + SL grep空振り修正（`Phase 70.2`→`Phase 86`）/ 特徴量grep 37→55 修正（`feature_37_count`→`feature_full_count`）。

テスト1+5件追加・quick分析で実行確認済（土日TP500を50件検出）。

# Phase 90η: 稼働中Bot健全性監査（2026-06-01）

## 背景
土日縮小（90ε/ζ）デプロイ後の Day 1 検証として稼働中Botを監査。

## 結果
土日縮小正常・GCPコスト最適化（I3 scale-to-zero）完璧・取引/設定/Maker正常を確認、致命的バグなし。不整合3件を修正:
- (A) 稼働率46.8%誤表示 → 15分ベースに訂正
- (B) WebSocket NOT_STARTED誤警告 → triggerモードSKIPPED扱い
- (C) SLMonitor誤発火率100% → ポジ残量ガード `position_close_guard` 導入

※ (C) のガードはデプロイ後も誤発火継続と判明し、**Phase 90θ で真因修正**（このηのガードでは止まらなかった）。
...
# Phase 90λ: bitbank 50026を成功扱い + GCPログ精査の所見（2026-06-05）

## 背景
GCPログ精査で検出した低重大度3件に対応。

## 実装内容
- ①**bitbank 50026「該当注文なし」を孤児SL解消済み=成功扱い**（CRITICALノイズ解消）。孤児SLキャンセル時の50026は「注文が既に消滅＝解消済み」で正常。`tp_sl_manager.py` + `thresholds.yaml` の `already_resolved_patterns` で判定しCRITICAL化しない。テスト2件。
- ②CI/CD SetIamPolicy権限拒否（要IAM付与・別途対応）。
- ③デプロイは可能な限りフラット（ポジション0）時に実施する運用メモ（建玉保有中のデプロイは再起動時に孤児ポジ/孤児SL検出が集中・最終的に自己解消だがノイズ）。

checks.sh全PASS。

# Phase 90μ: SLMonitor誤発火 Fire #2 の真因修正（2026-06-05 デプロイ）

## 背景
6/5ライブ分析で `SLMonitor DRY_RUN誤発火:2回` を検出。GCPログ精査（06/04 19:30-20:30 UTC）で**誤発火2回は別々の2メカニズム**と確定。
- **Fire #1**（reason=`canceled_unfilled`・90θ 3/3連続検出）は15分stuckのSLを正しく昇格し同サイクルでPhase 64.12も実決済＝**設計どおり（誤発火ではない・現状維持）**。
- **Fire #2**（reason=`fetch_error_persistent`）が**クリーンな誤発火**で2バグの複合。

## 真因（2バグ複合）
- ① `stop_manager._check_tp_sl_execution` が合成ID `market_close_*`（Phase 64.12 成行決済済み）に `fetch_order` して例外を握り潰し、**幽霊VPが除去されず**毎サイクル C5/C7 の対象に。
- ② `sl_monitor.check_sl_health` の C7 昇格パスが他の昇格（canceled_unfilled/expired/timeout）と違い `_position_already_closed` 残量ガードを通さず、実0でも3サイクルで無条件発火。

## 修正
- **Fix 1**（根本・`stop_manager.py`）: `market_close_*` を `fetch_order` せず `stop_loss` 確定で幽霊VP即除去。
- **Fix 2**（防御・`sl_monitor.py`）: C7昇格前に残量ガードを対称適用し実0なら抑止。
- `dry_run:true` は変更なし（本番で誤発火0を確認後に別タスクで解除判断）。

テスト4件追加（sl_monitor 3 + stop_manager 1）・checks.sh全PASS。**デプロイ後 Fire #2 誤発火0 を確認**。
...
# Phase 90ξ: ライブ分析スクリプトの本番WARNING誤検知3件を修正（2026-06-15）

## 背景
ライブ分析でBot機能診断が「致命的:1」を報告。GCPログを実査したところ、本番 `LOG_LEVEL=WARNING` 下で**構造的に誤検知する3件**と確定（本番botは健全・取引ロジックは不変）。真因は複数チェックが**INFOレベルのログをgrep**しており、本番では常に0件になること（`ML予測実行開始`/`統合シグナル生成`/`Atomic Entry完了` は7日間0件を実機確認）。Phase 90ζ（`feature_37`→`feature_full`・SL grep `Phase 70.2`→`Phase 86`）に続く同系統の再発で、[[live-analysis-warning-log-trap]] として記録。

## 修正（`scripts/live/standard_analysis.py`・分析スクリプトのみ）
| # | チェック | 問題 | 修正 |
|---|---|---|---|
| 1 | `_check_ml_prediction` | `ProductionEnsemble`/`ML予測`(INFO)を見て常に偽CRITICAL | WARNINGの `フル取引サイクル開始`(trigger_server.py:259) / `固定金額TP適用` を生存シグナル併用 |
| 2 | `_detect_silent_failure` (signal/order) | INFOログで常に0件 | `フル取引サイクル開始` / `TP Maker配置成功` 併用 + 拒否系(`取引拒否`/`品質フィルタ拒否`)を `cycle_resolved_count` 集計 |
| 3 | success_rate判定 | `_count_gcp_logs` の `--limit=30` 頭打ち + 正当拒否が大半で実行率が構造的に低い→偽CRITICAL | 実行率の段階判定（<20%→CRITICAL）を撤廃し「シグナル評価後に実行も拒否も無い時のみ真のSilent Failure(CRITICAL+=3)」に変更 |

## 結果
テスト9件追加（`test_standard_analysis_phase90xi_ml_signal.py`）・checks.sh全PASS。実機 `--quick` で **致命的1→0・正常13→16・判定🟠要注意→🟡監視継続**。
