...
| 項目 | 値 |
|------|-----|
| **Phase** | 66.8（根本原因修正8件 + 旧MLモデル復元 + TP/SL500円固定） |
...
**固定金額TP/SL（Phase 66.6追加）**: TP=500円 / SL=500円（RR比 1:1）
...
### Phase 66 成果（修正1-8完了・バックテストCI実行中）
| Phase | 内容 | 状態 |
|-------|------|------|
...
- **修正8**: 旧MLモデル復元（BUY54%偏り）→ 取引数回復見込み
- **バックテスト**: CI実行中
...
**最終更新**: 2026年3月1日
**ステータス**: Phase 66.8（修正1-8完了・バックテストCI実行中）
