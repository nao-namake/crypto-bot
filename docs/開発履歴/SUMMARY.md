...
| **63** | **TP/SL不具合修正期** | 2026/2/10 | **TP/SL関連6バグ修正・固定金額TP累積手数料バグ修正（✅63.2完了）** | [詳細](Phase_63.md) |
...
## 6. 現在の状態（Phase 63.2完了）

### システム仕様
| 項目 | 値 |
|------|-----|
| **Phase** | 63.2完了（固定金額TP累積手数料バグ修正） |
...
| 項目 | 成果 |
|------|------|
| **Bug 1: stop_limit検出** | SL検出ロジックに`stop_limit`タイプ追加 |
| **Bug 2: マッチング緩和** | ポジション集約対応・サイド一致のみでマッチング（3箇所） |
| **Bug 3: asyncio.create_task廃止** | pending_verifications+メインサイクル処理方式に変更 |
| **Bug 4: タイムアウト常時発火** | fetch_order()でSL状態確認・API一時エラー時安全側スキップ |
| **Bug 5: 消失検知マッチング** | virtual_positionsのサイド一致マッチングに修正 |
| **Bug 6: virtual_positions整合性** | 実ポジション0件時の孤立エントリ自動クリーンアップ |

### Phase 63.2 成果（✅完了）
| 項目 | 成果 |
|------|------|
| **累積手数料バグ修正** | `fee_data.unrealized_fee_amount`（集約ポジション累積値）のTP計算への混入を排除 |
| **累積利息バグ修正** | `fee_data.unrealized_interest_amount`（集約ポジション累積値）のTP計算への混入を排除 |
| **TP正常化** | 必要含み益: 500+累積手数料（平均741円）→ 500円固定 |
| **バックテスト/ライブ一致** | fee_data有無で完全一致（乖離解消） |
...
**最終更新**: 2026年2月10日
**ステータス**: Phase 63.2完了（固定金額TP累積手数料バグ修正）
