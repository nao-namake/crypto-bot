...
**期間**: 2025/12/29 - 2026/01/03
**状況**: Phase 57.13 年利10%目標・均一+34%拡大【バックテスト実行中】
...
| 57.11 | DD 5%許容・攻撃設定（失敗） | ❌ | 赤字化 -¥6,569（一律ポジション拡大の失敗） |
| 57.12 | 信頼度リンク方式・レポート改善 | ✅ | +¥6,671（57.11より改善だが57.10より低下） |
| 57.13 | 年利10%目標・均一+34%拡大 | 🔄 | 57.10ベース×1.34・バックテスト実行中 |
...
## ❌ Phase 57.11: DD 5%許容・攻撃設定【失敗】

### 実施日: 2026/01/02

### 目的

Phase 57.10のバックテスト結果（+¥18,632、PF 1.28）を踏まえ、年利10%達成に向けてポジションサイズを攻撃的に拡大。
...
**executor.py**:
- Rollbackリトライ追加（3回、Exponential Backoff）
...
# thresholds.yaml
production:
  max_order_size: 0.10        # 0.05→0.10（2倍）

position_integrator:
  kelly_weight: 0.7           # 0.5→0.7（Kelly重視）
  dynamic_weight: 0.2         # 0.3→0.2
  risk_manager_weight: 0.1    # 0.2→0.1

kelly_criterion:
  max_position_ratio: 0.60    # 0.35→0.60（約2倍）
  safety_factor: 0.95         # 0.9→0.95

max_position_ratio_per_trade:
  low_confidence: 0.40        # 0.25→0.40（+60%）
  medium_confidence: 0.55     # 0.35→0.55（+57%）
  high_confidence: 0.70       # 0.50→0.70（+40%）
```
...
| # | 原因 | 影響 |
|---|------|------|
| 1 | **低信頼度(42%)での攻撃的ポジション(40%)** | normal_rangeで-¥9,969の損失 |
| 2 | **Kelly基準70%重みの一律適用** | 不安定環境でも過度なポジション |
| 3 | **ポジション拡大の急激さ(+100%)** | 損失も2倍に拡大 |

#### レジーム別詳細

| レジーム | 取引数 | 勝率 | 総損益 |
|---------|--------|------|--------|
| tight_range | 502件 | 43.8% | +¥3,400 |
| normal_range | 73件 | **32.9%** | **-¥9,969** |

**問題点**: normal_rangeは勝率32.9%と低いにもかかわらず、低信頼度で40%ポジションを取得した結果、大きな損失を生んだ。

### 学習事項

1. **一律ポジション拡大は危険**: 信頼度に関係なく全帯域を拡大すると、低品質取引での損失が拡大
2. **勝率と信頼度の相関**: normal_range（勝率32.9%）は取引すべきではない可能性
3. **段階的なアプローチが必要**: 高信頼度のみ攻撃的に、低信頼度は保守的維持

---

## 🔄 Phase 57.12: 信頼度リンク方式・レポート改善【実行中】

### 実施日: 2026/01/03

### 目的

Phase 57.11の失敗を教訓に、「信頼度リンク方式」で段階的な攻撃設定を適用し、年利10%を目指す。

### Phase 57.11失敗からの学習

| 失敗点 | 対策 |
|--------|------|
| 一律ポジション拡大 | 低信頼度は据置（保守的維持） |
| normal_rangeでの損失 | 低信頼度での取引を制限 |
| Kelly 70%一律適用 | 50%に戻す（安定重視） |

### 修正内容

#### 1. レポート改善 (generate_markdown_report.py)

新規セクション追加:
- **信頼度帯別パフォーマンス**: 低/中/高の取引数・勝率・損益
- **ポジションサイズ統計**: 平均・最大・最小ポジションサイズ
- **戦略×レジーム クロス集計**: 各戦略のレジーム別パフォーマンス

```python
# Phase 57.12: 新規関数追加
def generate_confidence_stats(trades): ...
def generate_position_stats(trades): ...
def generate_strategy_regime_matrix(trades): ...
```

#### 2. 設定変更 (thresholds.yaml)

**Phase 57.10復元**:
```yaml
production:
  max_order_size: 0.05        # 0.10→0.05（復元）

position_integrator:
  kelly_weight: 0.5           # 0.7→0.5（復元）
  dynamic_weight: 0.3         # 0.2→0.3（復元）
  risk_manager_weight: 0.2    # 0.1→0.2（復元）

kelly_criterion:
  max_position_ratio: 0.35    # 0.60→0.35（復元）
  safety_factor: 0.9          # 0.95→0.9（復元）
```

**信頼度リンク方式（段階的攻撃）**:
```yaml
max_position_ratio_per_trade:
  low_confidence: 0.25        # 据置（保守的維持）
  medium_confidence: 0.40     # +14%（慎重に拡大）
  high_confidence: 0.60       # +20%（攻撃的に）
...
| 指標 | 57.10 | 57.12予測 | 根拠 |
|------|-------|-----------|------|
| 半年利益 | ¥18,632 | ¥22,000〜25,000 | 高信頼度取引（約30%）のみ拡大 |
| 年利 | 7.5% | 9〜10% | 利益+30%、リスク+50% |
| DD | 2.0% | 3〜4% | 高信頼度のみリスク増加 |
| PF | 1.28 | 1.2以上 | 低信頼度は据置で品質維持 |
...
**問題**: 57.10と「同じ設定」に戻したはずが、ポジションサイズが**29%減少**していた。

| 原因 | 詳細 |
|------|------|
| 信頼度リンク方式の限界 | 高信頼度23%のみ拡大、77%の低信頼度は据置 → 平均が低下 |
| tight_range利益激減 | +¥20,526 → +¥4,252（-80%） |

**結論**: 信頼度リンク方式は高信頼度取引が少ないため効果が限定的。

---

## 🚀 Phase 57.13: 年利10%目標・均一+34%拡大【実行中】

### 実施日: 2026/01/03

### 目的

Phase 57.10の成功設定をベースに、全帯域均一に+34%拡大して年利10%を達成する。

### Phase 57.12からの学習

| 問題 | Phase 57.13での対策 |
|------|-------------------|
| 信頼度リンク方式の限界（高信頼度23%のみ拡大） | 全帯域均一に拡大 |
| ポジションサイズ29%減少 | 57.10ベース×1.34で確実に拡大 |
| tight_range利益80%減 | 均一拡大で全取引の利益を増加 |
...
```yaml
# Phase 57.13: 57.10ベース × 1.34倍

production:
  max_order_size: 0.067        # 0.05 × 1.34

kelly_criterion:
  max_position_ratio: 0.47     # 0.35 × 1.34
  safety_factor: 0.9           # 57.10維持

trading:
  initial_position_size: 0.0067  # 0.005 × 1.34

max_position_ratio_per_trade:
  low_confidence: 0.34         # 0.25 × 1.34
  medium_confidence: 0.47      # 0.35 × 1.34
  high_confidence: 0.67        # 0.50 × 1.34

dynamic_position_sizing:
  low_confidence:
    min_ratio: 0.13            # 0.10 × 1.34
    max_ratio: 0.27            # 0.20 × 1.34
  medium_confidence:
    min_ratio: 0.20            # 0.15 × 1.34
    max_ratio: 0.34            # 0.25 × 1.34
  high_confidence:
    min_ratio: 0.27            # 0.20 × 1.34
    max_ratio: 0.47            # 0.35 × 1.34
```
...
| 項目 | 57.10 | 57.13 | 倍率 |
|------|-------|-------|------|
| max_order_size | 0.05 | 0.067 | ×1.34 |
| kelly_max_position_ratio | 0.35 | 0.47 | ×1.34 |
| initial_position_size | 0.005 | 0.0067 | ×1.34 |
| low_confidence | 0.25 | 0.34 | ×1.34 |
| medium_confidence | 0.35 | 0.47 | ×1.34 |
| high_confidence | 0.50 | 0.67 | ×1.34 |

### 期待効果

| 指標 | 57.10 | 57.13予測 | 目標 |
|------|-------|-----------|------|
| 総損益 | ¥18,632 | **¥25,000** | ¥25,000 ✅ |
| DD | 3.38% | 4.5% | ≤10% ✅ |
| PF | 1.15 | 1.15 | ≥1.1 ✅ |
| 年利 | 7.5% | **10%** | 10% ✅ |
...
| リスク | 57.11での影響 | 57.13での対策 |
|--------|---------------|---------------|
| normal_range大損失 | -¥9,969 | +34%拡大なら-¥2,500程度（許容） |
| DD急増 | 3.75% | 4.5%予測（目標10%内） |
| 勝率低下 | 42.4% | 57.10設定なら44.8%維持 |

### バックテスト実行

```bash
gh workflow run backtest.yml --ref main -f backtest_days=180
# Run ID: 20669867691
# 開始: 2026-01-03 09:53:33 JST
```
...
13. **一律ポジション拡大は危険**: 信頼度に関係なく全帯域を拡大すると、低品質取引での損失も拡大（Phase 57.11失敗教訓）
14. **信頼度リンク方式の限界**: 高信頼度取引が23%しかない場合、高信頼度のみ拡大しても効果が限定的。77%の低信頼度据置で平均ポジションが縮小する逆効果が発生（Phase 57.12教訓）
15. **均一拡大の有効性**: 57.11の「一律2倍」は過激すぎたが、57.10ベース×1.34の「適度な均一拡大」ならDD 4.5%で目標利益達成可能（Phase 57.13仮説）
...
## ✅ Phase 57.10 結論: バックテストとライブモードの一致性

### バックテスト信頼性の確認

Phase 57.10のバグ修正により、**バックテスト結果はライブモードの性能予測として高い信頼性を持つようになりました**。
...
|------|------------|------------|
| 戦略ロジック | ✅ 同一コード | ✅ 同一コード |
| MLモデル | ✅ 同一（ensemble_full.pkl） | ✅ 同一 |
| リスク管理 | ✅ DrawdownManager修正済み | ✅ 同一 |
| Kelly基準 | ✅ reference_timestamp使用 | ✅ 同一 |
| TP/SL設定 | ✅ レジーム別同一設定 | ✅ 同一 |
| ポジションサイズ計算 | ✅ 同一ロジック | ✅ 同一 |

#### ライブモード特有の差異（バックテストでは再現不可能）

| 項目 | 影響 | 推定誤差 |
|------|------|---------|
| **スリッページ** | 指値価格で約定しない場合がある | -2~5% |
| **API遅延** | シグナル発生から約定まで5-10秒 | -1~3% |
| **流動性** | 大口注文時の価格影響 | -1~2% |
| **スプレッド変動** | ボラティリティ高時にスプレッド拡大 | -1~3% |
| **API障害** | bitbankダウンタイムで機会損失 | -0~2% |

**推定総誤差: バックテスト結果の5~15%程度悪化の可能性**

### 結論

```
バックテスト結果 × 0.85~0.95 ≒ ライブモード期待値
```

Phase 57.10修正後のバックテスト:
- **取引数**: 500件/180日（ライブでも同等頻度を期待）
- **PF**: 1.28（ライブでは1.1~1.2程度を期待）
- **DD**: 2.0%（ライブでは2~3%程度を期待）

**バックテストは「傾向の予測」として信頼できる状態になりました。**

---

**📅 最終更新**: 2026年1月3日 - Phase 57.13 年利10%目標・均一+34%拡大（バックテスト実行中）
