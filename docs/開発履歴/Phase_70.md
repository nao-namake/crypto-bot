...
- Discord関連チェック削除（Phase 64で廃止済み）

---

## Phase 70.2: SL計算にentry_fee再包含（2026年3月24日）

### 問題

72時間ライブ実績でSL目標500円に対し実損653〜814円で着地。

| ケース | SL目標 | 実損 | 超過額 | 内訳 |
|--------|--------|------|--------|------|
| 3/22 buy 0.015BTC | 500円 | 712円 | +212円 | entry_fee 168円 + slippage 44円 |
| 3/23 buy 0.01BTC | 500円 | 653円 | +153円 | entry_fee 109円 + slippage 44円 |
| 3/23 sell 0.01BTC | 500円 | 814円 | +314円 | entry_fee 109円 + slippage 204円 |

原因:
1. **entry_fee未包含**: Phase 69で「サンクコスト」として除外したが、bitbank PnLにはentry_feeが含まれる
2. **成行スリッページ**: stop型（成行）のトリガー後滑り（44〜204円）

### 根本原因の再分析

Phase 69でentry_feeを除外した理由は「SL距離が極端に狭くなるバグ(0.09%)」だったが、真の原因はentry_feeの**二重計上バグ**だった。二重計上は既に修正済みのため、entry_fee自体の包含は正しく安全。

### 変更内容

**`config/core/thresholds.yaml`**:
- `stop_loss.fixed_amount.include_entry_fee`: false → **true**

**3つのSL計算パス全てを修正**:
- `strategy_utils.py`: メインパス（エントリー時SL計算）
- `tp_sl_manager.py`: リカバリパス（ポジション復旧時）
- `manager.py`: 再計算パス（リスク評価時）

### SL距離の変化（0.01BTC @ 11,000,000円の場合）

```
旧（entry_fee除外）:
  SL距離 = (500 - 110) / 0.01 = 39,000円 (0.35%)

新（entry_fee包含）:
  SL距離 = (500 - 110 - 110) / 0.01 = 28,000円 (0.25%)
```

SL距離は狭くなるが、Phase 69時代の二重計上(0.09%)とは異なり十分な距離を確保。
実損は500円+スリッページのみとなり、bitbank PnLとの整合性が回復。
