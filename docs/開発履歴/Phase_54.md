...
## ✅ Phase 54.10: tight_range戦略0ベース最適化【完了】
...
- カバレッジ向上テスト追加（test_runtime_flags.py、test_exceptions.py）
...
if (
    hasattr(self.orchestrator, "risk_manager")  # ← 存在しない属性名
...
**問題**:
- `orchestrator.py`では`self.risk_service`として保持
...
# 修正後（2箇所）
if (
    hasattr(self.orchestrator, "risk_service")  # risk_manager → risk_service
