...
## Phase 62.19: 手数料改定対応 ✅完了

**実施日**: 2026年2月7日

### 背景

**bitbank手数料改定（2026年2月2日適用）**

公式発表: https://bitbank.cc/blog/articles/726061459
...
1. **設定が旧手数料のまま**: Maker -0.02%、Taker 0.12%のまま
2. **TP価格計算が不正確**: 旧Taker 0.12%で計算している
3. **バックテスト損益計算も旧手数料**: 実際の手数料と乖離
4. **分析スクリプト**: Maker削減効果が旧料金でハードコード
5. **テストファイル**: モック値が旧料金のまま
6. **ドキュメント**: 運用ガイドに旧料金が記載

### 修正内容

| ファイル | 変更内容 |
|---------|----------|
| `config/core/thresholds.yaml` | 全手数料率を新体系に更新（11箇所） |
| `src/strategies/utils/strategy_utils.py` | デフォルト値・コメント・ログ修正 |
| `scripts/live/standard_analysis.py` | ハードコード→設定参照に一元化 |
| `src/trading/execution/stop_manager.py` | コメント一般化・デフォルト値更新 |
| `src/backtest/reporter.py` | デフォルト値更新（4箇所） |
| `tests/unit/strategies/utils/test_fixed_amount_tp.py` | テストモック値更新 |
| `tests/unit/trading/execution/test_stop_manager.py` | テストモック値更新 |
| `docs/運用ガイド/統合運用ガイド.md` | ドキュメント更新 |
| `docs/運用ガイド/システム機能一覧.md` | ドキュメント更新 |
| `CLAUDE.md` | 手数料設定テーブル更新 |

### 影響

| 項目 | 現状（旧手数料） | 修正後（新手数料） |
|------|-----------------|------------------|
| 必要含み益（TP計算） | ~1,100円 | ~500円 |
| TP到達難易度 | 高い | 本来の難易度 |
| 往復手数料（TP時） | -0.04%（リベート） | 0%（無料） |
| 往復手数料（SL時） | 0.10% | 0.1% |
| Maker成功時削減効果 | 0.14% | 0.1% |

### 計算例（修正前後比較）

**条件**: エントリー価格 10,000,000円、ポジション 0.025 BTC、目標純利益 500円

```
【修正前（旧Phase 62.11: Taker 0.12%想定）】
エントリー手数料 = 10,000,000 × 0.025 × 0.0012 = +300円
決済手数料 = 10,000,000 × 0.025 × 0.0012 = +300円
必要含み益 = 500 + 300 + 300 = 1,100円  ← 過大評価！

【修正後（新手数料: Maker 0%）】
エントリー手数料 = 10,000,000 × 0.025 × 0.0 = 0円
決済手数料 = 10,000,000 × 0.025 × 0.0 = 0円
必要含み益 = 500 + 0 + 0 = 500円  ← 正確！
```

### 設定一元化

将来の手数料変更時は `thresholds.yaml` のみ修正すれば全システムに反映される。

| 区分 | 変更前 | 変更後 |
|------|--------|--------|
| standard_analysis.py | `0.0014` ハードコード | `get_threshold()` 参照 |
| stop_manager.py | コメントに `0.0012` | 値を含まない一般的な説明 |
| 全get_threshold()呼び出し | 旧値デフォルト | 新値デフォルト |

---

## Phase 62 完了

**完了日**: 2026年2月5日（62.17追加: 2月6日、62.18追加: 2月6日、62.19追加: 2月7日）
...
| 62.9-62.10 | Maker戦略実装 | 往復手数料0%（Maker無料化後） |
...
## Phase 62.20: TP/SL欠損自動復旧（2026年2月7日完了）

### 背景

Phase 62.17のSLタイムアウトフォールバック時に、APIエラー50062が発生し、Bot内部状態とbitbank実態に乖離が生じた:

```
2026-02-06T22:19:52 - ❌ Phase 58.1: bitbank決済注文発行失敗: エラーコード 50062
```

結果として、ポジションは残存しているがSLが設定されていない状態が発生。

### 課題

| 問題 | 詳細 |
|------|------|
| APIエラー後の状態不整合 | bitbankにはポジションが残存、Botは「決済完了」と誤認 |
| SL欠損 | タイムアウト時にSL注文はキャンセル済み、失敗後に再設定されず |
| 無防備なポジション | SLなしのポジションが残存し、損失リスク増大 |

### 実装内容

Atomic Entry完了後、5分後にTP/SL設置状態を再確認し、欠損があれば自動的に再構築する。

| 処理 | 内容 |
|------|------|
| Step 1 | Atomic Entry完了後、5分後の検証をスケジュール |
| Step 2 | bitbank APIでポジション存在確認 |
| Step 3 | アクティブ注文からTP/SL存在確認 |
| Step 4 | 欠損があれば再構築（tight_rangeのTP/SL幅を使用） |

### 追加ファイル・変更

| ファイル | 変更内容 |
|---------|----------|
| `src/trading/execution/executor.py` | `_schedule_tp_sl_verification`、`_verify_and_rebuild_tp_sl`メソッド追加 |
| `config/core/thresholds.yaml` | `tp_sl_verification`設定追加 |
...
tp_sl_verification:
  enabled: true
  delay_seconds: 300      # 検証までの待機時間（5分）
  rebuild_on_missing: true
  default_regime: "tight_range"
```

### 動作ログ例

```
📋 Phase 62.20: TP/SL検証スケジュール - 300秒後 (Entry: xxx)
🔍 Phase 62.20: TP/SL検証開始 - Entry: xxx
⚠️ Phase 62.20: TP/SL欠損検出 - SL - Entry: xxx
✅ Phase 62.20: SL再構築成功 - ID: yyy, 価格: 11,000,000円
```
