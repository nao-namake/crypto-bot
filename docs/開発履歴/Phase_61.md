...
### 500円TP vs 1000円TP 比較テスト結果

| 指標 | 1000円TP | 500円TP | 変化 |
|------|----------|---------|------|
| 取引数 | 261件 | 298件 | **+14%** |
| 勝率 | 60.2% | **75.8%** | **+15.6pt** |
| 総損益 | ¥+131,564 | **¥+149,195** | **+13%** |
| PF | 2.06 | **2.68** | **+30%** |
| 最大DD | ¥6,477 | ¥5,860 | -10% |
| 最大連勝 | 8回 | **27回** | **+238%** |

**結論**: 500円TPを採用（thresholds.yaml更新済み）
...
#### 比較結果
| 項目 | 診断スクリプト | CSV分析 | 差異原因 |
|------|--------------|---------|----------|
| 取引数 | 95件 | 67件 | 期間が異なる（24h vs 18日） |
| 勝率 | N/A | 56.9% | DBにpnl記録なし |
| TP発動 | 0回 | 4件 | Phase 61.9以前の約定は検知対象外 |
| SL発動 | 0回 | 9件 | Phase 61.9以前の約定は検知対象外 |

#### 差異の原因
1. **期間の違い**: 診断スクリプトは24時間、CSVは18日間（1/12-1/29）
2. **Phase 61.9デプロイ時期**: 1/29 21:50 JST以降のみ自動執行検知が有効
3. **CSVのTP/SL（13件）**: 全てPhase 61.9デプロイ前に発生

#### 今後の期待
- 次回TP/SL約定時にPhase 61.9ログ + Phase 61.12 DB記録が両方機能
- 診断スクリプトで正確な勝率が表示される

### バックテスト結果（2026年1月30日）

#### 基本指標
| 指標 | Phase 60.7 | Phase 61.5 | **Phase 61.12** | 変化 |
|------|-----------|-----------|----------------|------|
| **総損益** | ¥86,639 | ¥100,629 | **¥131,564** | **+52%** |
| **PF** | 1.58 | 1.78 | **2.06** | **+30%** |
| 勝率 | - | - | 60.2% | - |
| 最大DD | - | - | 1.03% | ✅ 低リスク |
| 期待値 | - | - | +¥504/取引 | - |
| 平均ポジションサイズ | - | - | 0.027 BTC | ✅ 正常 |

#### 戦略別パフォーマンス
| 戦略 | 取引数 | 勝率 | 損益 | 評価 |
|------|--------|------|------|------|
| ATRBased | 218件 | 61.0% | +¥118,767 | ✅ 主力（83%） |
| DonchianChannel | 13件 | 69.2% | +¥10,608 | ✅ 良好 |
| StochasticReversal | 29件 | 51.7% | +¥3,383 | ⚪ 普通 |
| BBReversal | 1件 | 0.0% | -¥1,194 | ⚠️ 要監視 |
...
| レジーム | 取引数 | 勝率 | 損益 |
|----------|--------|------|------|
| tight_range | 220件 | 55.9% | +¥91,263 |
| normal_range | 41件 | 82.9% | +¥40,301 |

#### 改善要因
1. **Phase 61.10**: ポジションサイズ統一（平均0.027 BTC）
2. **Phase 61.5**: 低信頼度エントリー排除
3. **ATRBased戦略**: 安定したパフォーマンス
...
**最終成果**: PF 2.68・勝率75.8%・総損益¥149,195達成（500円TP採用）

| 成果 | 詳細 |
|------|------|
| **PF +70%** | 1.58 → 2.68 |
| **勝率 +21pt** | 54.8% → 75.8% |
| **総損益 +72%** | ¥86,639 → ¥149,195 |

### Phase別サマリー

| Phase | 結果 | 主な成果 |
|-------|------|----------|
| 61.1 | ❌→✅ | 失敗→ロールバック成功、閾値設定ファイル化 |
| 61.2 | ✅ | コードベース整理（500MB削減） |
| 61.3 | ✅ | TP/SL注文改善（UI「利確」「損切り」表示） |
| 61.4 | ✅ | MFE/MAE分析（What-if分析・改善方向性特定） |
| 61.5 | ✅ | 低信頼度対策（PF 1.78達成） |
| 61.6 | ✅ | バグ修正（ATR取得・TP注文タイプ） |
| 61.7 | ✅ | 固定金額TP実装（純利益保証） |
| 61.8 | ✅ | 固定金額TPバックテスト対応 |
| 61.9 | ✅ | TP/SL自動執行検知 |
| 61.10 | ✅ | ポジションサイズ統一 |
...
1. **500円TPが最適**: 1000円TPより勝率+15.6pt、PF+30%
2. **低信頼度エントリー排除**: 0.25未満を強制HOLDで品質向上
3. **ポジションサイズ統一**: バックテスト・ライブで同一ロジック使用

### 次Phase（Phase 62）への課題

| 課題 | 詳細 |
|------|------|
| ATRBased一強 | 298件中250件（84%）がATRBased |
| BBReversal不発 | 1件のみ（閾値が厳しすぎる） |
| StochasticReversal不足 | 30件（重み0.35の割に少ない） |
...
**最終更新**: 2026年1月31日 - Phase 61完了（500円TP採用・PF 2.68達成）
