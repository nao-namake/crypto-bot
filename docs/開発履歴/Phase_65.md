...
| **65.13** | SLキャンセル時の即時超過チェック + slippage_buffer拡大 | ✅ 完了 |
| **65.14** | TP/SL設置不全の残存バグ修正（float(None) + INACTIVEキャンセル漏れ） | ✅ 完了 |
| **65.15** | TP/SL設置フロー最終チェック（NoneType横展開 + take_profit型判定 + VP二重カウント防止） | ✅ 完了 |
...
## Phase 65.13: SLキャンセル時の即時超過チェック

**日付**: 2026年2月24日
**目的**: SLキャンセル検出時に即時SL超過チェックを実行し、成行決済へのフォールバックを追加

### 変更内容

| ファイル | 変更 |
|---------|------|
| `src/trading/execution/stop_manager.py` | SLキャンセル検出時の即時超過チェック追加 |
| `config/core/thresholds.yaml` | `slippage_buffer: 0.002→0.003` 拡大（SL指値の約定率向上） |

---

## Phase 65.14: TP/SL設置不全の残存バグ修正

**日付**: 2026年2月24日
**目的**: GCPログで繰り返し発生する「既存ポジションTP/SL確保失敗: float() argument must be a string or a real number, not 'NoneType'」エラーと、エントリー時のINACTIVE SLキャンセル漏れを修正

### 背景

Phase 65.13でSLキャンセル時の即時超過チェックを修正済みだが、GCPログ精査で**さらに2つの未修正バグ**が発見された。

**GCPログパターン（2/23）**:
```
15:48:46 ⚠️ Phase 56.5: 既存ポジションTP/SL確保失敗: float() argument must be a string or a real number, not 'NoneType'
（以降10分間隔で繰り返し、全サイクル失敗）
```

→ 10分定期ヘルスチェックが全て例外で失敗し、SL再配置が機能しない状態。

### 修正1【P0】: float(None)バグ修正（5箇所）

**原因**: bitbankのAPIレスポンスで`amount`や`stopPrice`がキーは存在するが値が`None`のケースがある。`dict.get("amount", 0)` はキーが存在すれば`None`を返す。

```python
# Before（安全でないパターン）:
float(o.get("amount", 0))       # amount=Noneの場合 → TypeError

# After（安全パターン）:
float(o.get("amount") or 0)     # amount=Noneの場合 → float(0) → OK
```
...
| # | 箇所 | 行 |
|---|------|-----|
| 1 | `_is_sl_price_valid` trigger価格取得 | ~L522 |
| 2 | `tp_sell_total` amount集計 | ~L531 |
| 3 | `sl_sell_total` amount集計 | ~L537 |
| 4 | `tp_buy_total` amount集計 | ~L545 |
| 5 | `sl_buy_total` amount集計 | ~L551 |

### 修正2【P1】: エントリー時のINACTIVE SLキャンセル漏れ

**原因**: `cleanup_old_tp_sl_before_entry`が`fetch_active_orders`で既存注文を取得するが、bitbankの**INACTIVE状態のstop_limit注文**（SLトリガー待ち）は返されない。

結果:
1. Entry 1 → SL1配置 (INACTIVE)
2. Entry 2 → cleanup → SL1はINACTIVEで見つからず → SL2配置
3. SL合計がポジション超過 → **bitbankが注文をキャンセル**

**修正内容**:
- VP追跡のTP/SL注文をorder ID指定でキャンセルする処理を追加（INACTIVEでもID指定でキャンセル可能）
- 同一sideの古いVP注文を全キャンセル（新エントリー後に全量カバーTP/SLが再配置される）
- VPの注文IDをキャンセル後にNullクリア

### 修正3: stop_limit type判定漏れ

`cleanup_old_tp_sl_before_entry`のSL判定で`order_type == "stop"`のみチェックしていたが、`stop_limit`も含める必要があった。
...
| ファイル | 修正内容 |
|---------|---------|
| `src/trading/execution/tp_sl_manager.py` | float(None)修正5箇所 + VP追跡INACTIVEキャンセル追加 + stop_limit type判定修正 |
| `tests/unit/trading/execution/test_tp_sl_manager.py` | float(None)耐性テスト2件 + INACTIVEキャンセルテスト4件 |
| `tests/unit/trading/execution/test_executor.py` | 既存テスト更新（Phase 65.14の動作変更に対応） |

### 品質チェック結果

```
pytest: 1758 passed, 1 skipped
Coverage: 71.55%
flake8: PASS
black: PASS
isort: PASS
