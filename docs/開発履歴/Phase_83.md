...
# Phase 83B: TP1000/SL500/floor撤廃 + ML再学習

**期間**: 2026年5月8日
**状態**: 設定変更・ML再学習完了・コミット待ち

## 背景

### Phase 83A 適用後14日（2026/04/24-05/07）の運用結果

- 総損益: **-8,522円**
- TP決済: 24件（純利益平均 +755円）
- SL決済: 11件（純損失平均 **-1,637円**）
- SL平均がTP平均の2.17倍 → SL拡大方針（1500円/floor0.7%）が裏目に

主因: SL distance を0.7%まで拡大した結果、1回SLで取り返しに3-4回TPが必要なRR比となり、勝率53%でも累積マイナス。

### sl_simulation.pyによる反実仮想検証

過去TP24件を bitbank 15分足で再現し、SL距離を変えた場合のヒット可否を検証。

| シナリオ | 勝率 | SL転落数 | TP×SL混合損益 |
|---------|------|----------|---------------|
| TP750/SL1500(floor0.7%)現状 | 100% | 0 | 基準 |
| **TP1000/SL500(floor無し)** | **95.5%** | **1** | **+25,000円改善** |
| TP750/SL500(floor無し) | 95.5% | 1 | +20,000円改善 |
| TP1000/SL800(floor0.4%) | 100% | 0 | +24,000円改善 |

→ SL500円まで縮小してもTP24件中1件しかSL転落せず、TP1000円まで拡大可能。
RR比2.0:1なら勝率33.3%で採算ライン、現状の53%でも黒字想定。

## 設定変更

### TP/SL（thresholds.yaml）

| 項目 | Phase 83A | Phase 83B |
|------|-----------|-----------|
| TP（高信頼度） | 750円 | **1,000円** |
| TP（低信頼度） | 600円 | **800円** |
| SL（高信頼度） | 1,500円 | **500円** |
| SL（低信頼度） | 1,200円 | **400円** |
| min_distance floor | 0.7%強制 | **撤廃（enabled: false）** |
| RR比 | 0.5:1 | **2.0:1** |

floor撤廃により、固定金額SL距離が直接適用される（`sl_amount / position_size + 手数料`）。
例: amount 0.015 BTC, SL目標500円 → SL距離 ≒ 33,333円 + 手数料 ≒ 約0.27%。

## ML再学習（83B-ML）

新運用条件（TP1000/SL500/floor撤廃）に合わせて再学習。

### 学習コマンド

```bash
python3 scripts/ml/create_ml_models.py --optimize --n-trials 50 --verbose \
    --meta-label --meta-tp-ratio 0.006 --meta-sl-ratio 0.0045
```

ratio算出根拠（amount 0.015 BTC, BTC ~12.5M JPY）:
- TP1000円 = (1000円 + entry手数料) / 0.015 / 12,500,000 ≒ 0.6%
- SL500円  = (500円 + entry+exit手数料) / 0.015 / 12,500,000 ≒ 0.45%

### 検証結果（全項目合格）

| 項目 | 値 | 評価 |
|------|-----|------|
| n_classes | 2（メタラベリング Go/No-Go） | ✅ |
| training_samples | **35,036**（2025-05-08〜2026-05-07の365日分） | ✅ Phase 83A-3 (34,970) と同等 |
| feature_count | 37（Phase 77 SHAP最適化維持） | ✅ |
| class_distribution | success 31.4% / failure 68.6% | ✅ Phase 83A-3 (33/67) と同等 |
| CV F1 (LightGBM) | **0.612**±0.032 | ✅ Phase 83A-3と完全一致 |
| CV F1 (XGBoost) | 0.583±0.060 | ✅ Phase 82 比+0.05 |
| CV F1 (RandomForest) | 0.552±0.075 | ✅ 安定 |
| Optuna Best F1 | LGB 0.560 / XGB 0.562 / RF 0.532 | ✅ 単独モデル F1 安定 |
| アンサンブル信頼度 | mean=0.652, std=0.149, range=[0.243-0.896] | ⚠️ Phase 83A-3 (mean 0.531) から上昇 |
| 高信頼度(>60%)比率 | 71% | ⚠️ Phase 83A-3 24% から大幅増 |
| 極端予測 | >0.75: 281/1000件、<0.25: 2/1000件 | ⚠️ 高信頼側にやや偏り |

### データ収集

学習前にBTC/JPY 15分足CSVを365日分（35,036行）に再収集:

```bash
mv src/backtest/data/historical/BTC_JPY_15m.csv src/backtest/data/historical/BTC_JPY_15m.csv.180d.bak
python3 src/backtest/scripts/collect_historical_csv.py --days 365 --symbol BTC/JPY
```

学習スクリプトはCSVに十分なデータがあれば（180日 × 96 × 0.9 = 15,552行以上）CSV全体を使う仕様のため、`--days` パラメータでなくCSV内容で学習データ量が決まる。

### 注意点

1. **信頼度上昇（mean 0.531→0.652）**: meta_sl_ratio が Phase 83A-3 (0.007) より厳しい (0.0045) ため、SL早期ヒットしにくい場面の識別が容易になり信頼度が上振れ。
2. **accept(>=0.58)率72%（前回24.1%）**: 同じ閾値設定では大半の取引が承認される。フィルタとしての選別効果が弱まる可能性 → デプロイ後 signal_simulation 実データで通過群と拒否群の勝率差を確認。差が小さい場合は閾値を 0.65 等に引き上げ要検討。
3. **>0.75の極端予測 28%**: 過学習の可能性あり。実データ推論で同水準なら閾値再調整。

### モデルバックアップ

- `models/production/ensemble_{full,basic}.phase82.pkl.bak` 保持中
- `models/production/ensemble_{full,basic}.phase83a.pkl.bak` 保持中（Phase 83A-3 時点のモデル）

---

## 期待される改善効果（Phase 83B目標）

| 指標 | Phase 83A 14日実績 | Phase 83B目標 |
|------|------------------|---------------|
| 平均SL損失 | -1,637円 | -500円前後（floor撤廃） |
| RR比（実効） | 0.46:1 | 2.0:1 |
| 採算ライン勝率 | 67% | 33% |
| 14日損益 | -8,522円 | プラス圏 |
