...
9. [Phase 58.8: ポジションカウント修正・孤児SL防止](#-phase-588-ポジションカウント修正孤児sl防止完了)
10. [ライブポジション検証記録](#-ライブポジション検証記録)
11. [学習事項](#-学習事項)
12. [修正ファイル一覧](#-修正ファイル一覧)
13. [Phase 58 進捗サマリー](#-phase-58-進捗サマリー)
...
| 58.8 | 01/13 | ポジションカウント修正・孤児SL防止 | ✅ | BTC/JPYフィルタ・リトライ機能・孤児検出 |
...
## 🔧 Phase 58.8: ポジションカウント修正・孤児SL防止【完了】
...
|------|------|------|
| ポジション数 | 10件 | 0件 |
| 維持率 | 500% 正常 | N/A ポジションなし |

**原因**: bitbank APIは全信用取引対応通貨ペアのポジションスロットを返却

```
btc_jpy (long/short) + xrp_jpy (long/short) + eth_jpy (long/short)
+ sol_jpy (long/short) + doge_jpy (long/short) = 10スロット
```

→ BTC/JPYフィルタ + Amount > 0 フィルタが必要

#### 問題2: 孤児SL注文

2件の孤児SL注文を検出（ID: 53707246136, 53707739408）

**発生原因**: ポジション決済時のSLキャンセル失敗 → 例外catchして処理継続 → ポジション削除 → SL孤児化

```python
# 問題のコード（stop_manager.py L296-297）
except Exception as e:
    self.logger.warning(f"⚠️ Phase 49.6: クリーンアップエラー（処理継続）: {e}")
    # ← 問題: 例外をcatchして処理継続、SLが残存
```
...
**ファイル**: `scripts/live/standard_analysis.py` L199-211
...
**ファイル**: `src/trading/execution/stop_manager.py` L578-637
...
async def cleanup_position_orders(...):
    ...
    async def _cancel_with_retry(order_id: str, order_type: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"✅ Phase 58.8: {order_type}注文キャンセル成功")
                return True
            except Exception as e:
    ...
    # SL注文キャンセル - 失敗時は例外を投げて処理中断
    if sl_order_id and reason in ["take_profit", "manual", "position_exit"]:
        if not await _cancel_with_retry(sl_order_id, "SL"):
            raise Exception(f"SL注文{sl_order_id}キャンセル失敗（孤児SL防止のため処理中断）")
```

#### 修正3: SLキャンセル失敗時は処理中断

**ファイル**: `src/trading/execution/stop_manager.py` L296-308
...
**ファイル**: `scripts/live/standard_analysis.py` L241-250
...
#### 修正5: Markdownレポートに孤児警告追加

**ファイル**: `scripts/live/standard_analysis.py` L669-671
...
| 影響 | 詳細 |
|------|------|
| **直接的影響** | なし（未約定のstop注文として残存） |
| **間接的リスク** | 約定時に意図しない新規ポジション構築の可能性 |
| **対応** | 検出時に手動キャンセル、または今後の自動クリーンアップ |
...
```bash
git commit -m "fix: Phase 58.8 ポジションカウント修正・孤児SL防止"
```
...
| ファイル | 修正内容 |
|---------|---------|
| `scripts/live/standard_analysis.py` | ポジションフィルタリング（L199-211）、孤児SL検出（L241-250）、レポート警告（L669-671） |
| `src/trading/execution/stop_manager.py` | SLキャンセルリトライ機能（L578-637）、処理中断ロジック（L296-308） |
...
- [x] ポジションカウント修正（Phase 58.8）
- [x] 孤児SL防止リトライ機能（Phase 58.8）
- [x] 孤児SL検出機能（Phase 58.8）
