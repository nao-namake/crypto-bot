...
| 58.5 | ポジション滞留問題対応 | 🔄 | tight_range TP/SL縮小・稼働率計算修正 |
...
1. **土日のみTP/SL縮小**: 低ボラ期間対応（調査完了・実装未着手）
2. **バックテストへの手数料・利息追加**: 現在未実装
3. **ポジションタイムアウト**: 一定時間後の強制決済
...
- [ ] Phase 58.5 バックテスト検証（GitHub Actions実行中）
...
- [ ] 土日のみTP/SL縮小（調査完了・実装未着手）
- [ ] バックテストへの手数料・利息追加

---

**📅 最終更新**: 2026年1月12日 - Phase 58.5進行中（TP/SL縮小・稼働率修正）
