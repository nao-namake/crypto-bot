...
class TestStateManager:
    ...
    def test_instance_key_generation(self):
        ...
        sm1 = StateManager(
            region="asia-northeast1", instance_id="primary", enable_cloud_storage=False
        )

        sm2 = StateManager(
            region="us-central1", instance_id="secondary", enable_cloud_storage=False
        )
    ...
    def test_state_persistence(self, state_manager):
        """状態の永続化をテスト"""
        # 初期状態
        initial_state = {"total_profit": 0.0, "trade_count": 0, "position": ""}

        state_manager.update_local_status(initial_state)

        # 状態を更新
        updated_state = {"total_profit": 1500.0, "trade_count": 10, "position": "SELL"}
