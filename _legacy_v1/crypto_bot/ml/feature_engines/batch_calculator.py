...
class FeatureBatch:
    ...
    def __init__(
        self, name: str, features: Dict[str, pd.Series], metadata: Optional[Dict] = None
    ):
        self.name = name
...
class BatchFeatureCalculator:
            ...
            for feature_name, values in feature_dict.items():
                if isinstance(values, pd.Series):
                    ...
                    if len(values) == len(base_index):
                        ...
                        if len(values) < len(base_index):
                            padded_values = list(values) + [np.nan] * (
                                len(base_index) - len(values)
                            )
                            series = pd.Series(
                                padded_values, index=base_index, name=feature_name
                            )
                        else:
                            series = pd.Series(
                                values[: len(base_index)],
                                index=base_index,
                                name=feature_name,
                            )
                else:
                    logger.error(
                        f"❌ Unsupported feature value type: {type(values)} for {feature_name}"
                    )
                    continue
    ...
    def merge_batches_efficient(
        self, base_df: pd.DataFrame, feature_batches: List[FeatureBatch]
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            for batch in feature_batches:
                if len(batch) > 0:
                    ...
                    if not batch_df.empty:
                        ...
                        logger.debug(
                            f"📦 Batch '{batch.name}': {len(batch_df.columns)} features"
                        )
            ...
            if batch_dataframes:
                ...
                self.batch_stats["memory_savings"] += max(
