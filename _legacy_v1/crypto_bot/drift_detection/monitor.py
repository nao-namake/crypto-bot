...
class DriftMonitor:
    ...
    def __init__(
        ...
        self.ensemble = DriftDetectionEnsemble(
            detectors=self.detection_config.get(
        ...
        self.alert_cooldown = timedelta(
            minutes=self.detection_config.get("alert_cooldown_minutes", 5)
        )
        self.last_alert_time = None

        # Data buffers
        self.sample_buffer = deque(
            maxlen=self.detection_config.get("buffer_size", 1000)
        )
        self.error_buffer = deque(
            maxlen=self.detection_config.get("error_buffer_size", 1000)
        )
    ...
    def start_monitoring(self, monitoring_interval: float = 1.0):
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitoring_loop, args=(monitoring_interval,), daemon=True
        )
    ...
    def update_sample(
        ...
        with self.lock:
            ...
            self.sample_buffer.append(
                {"data": sample, "timestamp": datetime.now(), "error": error}
            )

            if error is not None:
                self.error_buffer.append({"error": error, "timestamp": datetime.now()})

            if performance_metrics:
                self.performance_metrics.append(
                    {"metrics": performance_metrics, "timestamp": datetime.now()}
                )

            # Check for drift
            if len(self.sample_buffer) >= self.detection_config.get(
                "min_samples_for_detection", 100
            ):
        ...
        for metric_name in ["accuracy", "precision", "recall", "f1"]:
            ...
            if len(values) >= 5:
                ...
                if trend < -self.detection_config.get("performance_threshold", 0.1):
                    logger.warning(
                        f"Declining {metric_name} trend detected: {trend:.4f}"
                    )
    ...
    def _update_performance_metrics(self):
        ...
        if recent_errors:
            ...
            if len(self.performance_metrics) > 0:
                last_error_rate = self.performance_metrics[-1]["metrics"].get(
                    "error_rate", 0
                )
                if abs(error_rate - last_error_rate) > 0.05:
                    logger.info(
                        f"Error rate change: {last_error_rate:.3f} -> {error_rate:.3f}"
                    )
    ...
    def _cleanup_old_data(self):
        ...
        while self.drift_events and self.drift_events[0]["timestamp"] < cutoff_time:
    ...
    def export_drift_data(self, filepath: str, hours: int = 24) -> bool:
        ...
        try:
            cutoff_time = datetime.now() - timedelta(hours=hours)
            recent_events = [
                e for e in self.drift_events if e["timestamp"] > cutoff_time
            ]
